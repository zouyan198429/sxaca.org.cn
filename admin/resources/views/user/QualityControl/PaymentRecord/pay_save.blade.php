...
<div class="mm">
    ...
    @foreach ($pay_config_format as $pay_config_id => $pay_config)
        <?php
        $payment_record = $config_vod_list[$pay_config_id] ?? [];
        ...
        $totalPrice = 0;
        $data_ids = [];
        foreach($payment_record as $k => $data_info){
            $totalPrice += $data_info['final_amount'];
...
<script type="text/javascript">
    ...
    var SAVE_URL = "{{ url('api/user/payment_record/ajax_create_order') }}";// ajax保存记录地址
    ...
    var AJAX_QUERY_ORDER_WX_URL = "{{ url('api/user/vod_orders/ajax_wx_query_order') }}";// ajax查询微信扫码支付是否成功地址
...
<script src="{{ asset('/js/user/QualityControl/VodOrders_pay_save.js') }}?72"  type="text/javascript"></script>
