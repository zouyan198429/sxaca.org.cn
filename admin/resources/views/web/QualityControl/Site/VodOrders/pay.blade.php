...
<div class="mm">
    ...
    @foreach ($pay_config_format as $pay_config_id => $pay_config)
        ...
        ?>
        @foreach ($vod_orders as $company_id => $company_vod_orders)
            <?php
...
{{--                <col width="85">--}}
            ...
            @foreach ($company_vod_orders as $k => $data_info)
                ...
                $totalPrice = bcadd($totalPrice, $data_info['price'], 2);
                array_push($data_ids, $data_info['id']);
                ?>
            @endforeach
            <tr>
                <td colspan="2" style="text-align: right;" valign="top">
                    共<?php echo count($company_vod_orders)?>记录；总计：￥{{ $totalPrice ?? '' }}元
                </td>
...
<script type="text/javascript">
    ...
    var SAVE_URL = "{{ url('api/web/vod_orders/ajax_join_class_save') }}";// ajax保存记录地址
    var LIST_URL = "{{url('web/vod_orders')}}";//保存成功后跳转到的地址

    var PAY_SAVE_URL = "{{url('web/vod_orders/pay_save')}}";// 收款页面
</script>
<script src="{{ asset('/js/web/QualityControl/Site/VodOrders_pay.js') }}?11"  type="text/javascript"></script>
</body>
</html>
