...
{{--    <meta name="keywords" content="会员中心-陕西省质量认证认可协会"><meta name="description" content="陕西质量认证认可协会---官网----陕西省质量认证咨询中心-陕西省质量认证认可协会">--}}
    ...
    <title>陕西质量认证咨询中心_{{ $info['company_name'] ?? '' }}资质认定获证机构_陕西{{ $info['company_name'] ?? '' }}资质认定获证机构_检验检测能力</title>
    <meta name="keywords" content="陕西质量认证咨询中心,{{ $info['company_name'] ?? '' }}资质认定获证机构,陕西{{ $info['company_name'] ?? '' }}资质认定获证机构,检验检测能力" />
    <meta name="description" content="陕西质量认证咨询中心,{{ $info['company_name'] ?? '' }}资质认定获证机构,陕西{{ $info['company_name'] ?? '' }}资质认定获证机构,检验检测能力" />
...
<div class="article">
    ...
    <div id="left">
        <div class="sidebox subnav">
            <div class="subnavtitle hd"><h2>会员服务</h2></div>
            <div class="left_body">
                <ul class="subchannellist">
                    @foreach ($companyGrade as $k=>$txt)
                        <li class="depth1"  @if(isset($defaultCompanyGrade) && $defaultCompanyGrade == $k)  id="current" @endif><a href="{{url('web/company/0_0_20_1')}}?qkey=1&company_grade={{ $k }}">{{ $txt }}单位</a></li>
                    @endforeach
                </ul>
            </div>
            <div class="c"></div>
    ...
    <div id="right">
        <div id="location_1">
            ...
            <a href="{{url('web/company/0_0_20_1')}}?qkey=1&company_grade={{ $defaultCompanyGrade }}" target="_self" >{{ $company_grade_name ?? '' }}单位</a>&nbsp;&gt;&gt;&nbsp;
            <a href="javascript:void(0);" target="_self">查看详情</a>
        </div>
        <div class="right_title"><h2>{{ $company_grade_name ?? '' }}单位</h2></div>
        <div class="right_body">
            <div class="InfoTitle"><h1>{{ $info['company_name'] ?? '' }}</h1></div>
            <div class="info_from_wrap">
                <b>来源：</b
                ><a href="http://www.sxsrzrk.com/" target="_blank"
                >陕西省质量认证认可协会-陕西质量认证咨询中心</a
                >&nbsp; <b>日期：</b>{{ $info['created_at'] ?? '' }}&nbsp;
            </div>
            <!-- 频道/文章内容  开始-->
            <div class="InfoSContent"></div>
            <div class="InfoContent">
                {!! $info['company_content'] ?? '' !!}
            </div>
