...
<body class="layui-layout-body">
    ...
    <div class="layui-layout layui-layout-admin">
        ...
        <ul class="layui-nav layui-layout-right" lay-filter="layadmin-layout-right">
          ...
          <li class="layui-nav-item" lay-unselect>
            ...
            <dl class="layui-nav-child">
              <dd><a lay-href="{{ url('expert/info') }}">基本资料</a></dd>
              <dd><a lay-href="{{ url('expert/password') }}">修改密码</a></dd>
              <hr>
              <dd  style="text-align: center;"><a href="{{ url('expert/logout') }}">退出</a></dd>
            </dl>
          </li>
          ...
          <ul class="layui-nav layui-nav-tree" lay-shrink="all" id="LAY-system-side-menu" lay-filter="layadmin-system-side-menu">
            ...
            {{--</li>--}}
              ...
              <li data-name="user" class="layui-nav-item">
                  <a  lay-href="{{ url('expert/abilitys_admin/' . ($ability_id ?? 0)  . '/basic') }}" lay-tips="概况" lay-direction="2">
                      <i class="layui-icon layui-icon-component"></i>
                      <cite>概况</cite>
                  </a>
              </li>
              <li data-name="user" class="layui-nav-item">
                  <a  lay-href="{{ url('expert/abilitys_admin/' . ($ability_id ?? 0)  . '/ability_join_items_results') }}?retry_no=0&operate_num=1" lay-tips="参加单位" lay-direction="2">
                      <i class="layui-icon layui-icon-component"></i>
                      <cite>初测单位</cite>
                  </a>
              </li>
              <li data-name="user" class="layui-nav-item">
                  <a  lay-href="{{ url('expert/abilitys_admin/' . ($ability_id ?? 0)  . '/ability_join_items_results') }}?retry_no=0&submit_status=2&operate_num=2" lay-tips="上传数据" lay-direction="2">
                      <i class="layui-icon layui-icon-component"></i>
                      <cite>初测数据</cite>
                  </a>
              </li>
              <li data-name="user" class="layui-nav-item">
                  <a  lay-href="{{ url('expert/abilitys_admin/' . ($ability_id ?? 0)  . '/ability_join_items_results') }}?retry_no=1&operate_num=3" lay-tips="未合格单位" lay-direction="2">
                      <i class="layui-icon layui-icon-component"></i>
                      <cite>补测单位</cite>
                  </a>
              </li>
              <li data-name="user" class="layui-nav-item">
                  <a  lay-href="{{ url('expert/abilitys_admin/' . ($ability_id ?? 0)  . '/ability_join_items_results') }}?retry_no=1&submit_status=2&operate_num=4" lay-tips="补测数据" lay-direction="2">
                      <i class="layui-icon layui-icon-component"></i>
                      <cite>补测数据</cite>
                  </a>
              </li>
      ...
      <div class="layui-body" id="LAY_app_body">
        <div class="layadmin-tabsbody-item layui-show">
          ...
          <iframe src="{{ url('expert/abilitys_admin/' . ($ability_id ?? 0)  . '/basic') }}" frameborder="0" class="layadmin-iframe"></iframe>
        </div>
      </div>
  ...
  <script>
      ...
      var GET_TABLE_UPDATE_TIME_URL = "{{ url('api/expert/ajax_getTableUpdateTime') }}";
  </script>
