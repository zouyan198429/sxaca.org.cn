      ...
      <div class="msearch fr">
        ...
        <select style="width:80px; height:28px;" name="field">
          <option value="tax_num">设备编号</option>
            ...
            <option value="app_secret">应用密匙</option>
        </select>
...
{{--      <th>ID</th>--}}
  ...
  <script type="text/javascript">
      ...
      var AJAX_URL = "{{ url('api/admin/invoice_config_hydzfp/ajax_alist') }}";//ajax请求的url
      var ADD_URL = "{{ url('admin/invoice_config_hydzfp/add/0') }}"; //添加url

      var IFRAME_MODIFY_URL = "{{url('admin/invoice_config_hydzfp/add/')}}/";//添加/修改页面地址前缀 + id
      var IFRAME_MODIFY_URL_TITLE = "发票配置沪友" ;// 详情弹窗显示提示  [添加/修改] +  栏目/主题
      var IFRAME_MODIFY_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面

      var SHOW_URL = "{{url('admin/invoice_config_hydzfp/info/')}}/";//显示页面地址前缀 + id
      var SHOW_URL_TITLE = "" ;// 详情弹窗显示提示
      var SHOW_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面
      var EDIT_URL = "{{url('admin/invoice_config_hydzfp/add/')}}/";//修改页面地址前缀 + id
      var DEL_URL = "{{ url('api/admin/invoice_config_hydzfp/ajax_del') }}";//删除页面地址
      var BATCH_DEL_URL = "{{ url('api/admin/invoice_config_hydzfp/ajax_del') }}";//批量删除页面地址
      var EXPORT_EXCEL_URL = "{{ url('admin/invoice_config_hydzfp/export') }}";//导出EXCEL地址
      var IMPORT_EXCEL_TEMPLATE_URL = "{{ url('admin/invoice_config_hydzfp/import_template') }}";//导入EXCEL模版地址
      var IMPORT_EXCEL_URL = "{{ url('api/admin/invoice_config_hydzfp/import') }}";//导入EXCEL地址
