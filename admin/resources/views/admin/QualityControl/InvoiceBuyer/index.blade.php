...
<div class="mm">
  <div class="mmhead" id="mywork">
    ...
    <div class="tabbox" >
      <a href="javascript:void(0);" class="on" onclick="action.iframeModify(0)">添加发票配置购买方</a>
    </div>
    <form onsubmit="return false;" class="form-horizontal" style="display: block;" role="form" method="post" id="search_frm" action="#">
      <div class="msearch fr">
        ...
        <select style="width:80px; height:28px;" name="field">
          <option value="gmf_mc">抬头名称[公司或个人]</option>
            <option value="gmf_nsrsbh">纳税人识别号</option>
            <option value="gmf_dz">企业地址</option>
            <option value="gmf_dh">企业电话</option>
            <option value="gmf_yh">企业银行</option>
            <option value="gmf_yhzh">企业银行账号</option>
...
{{--      <th>ID</th>--}}
  ...
  <script type="text/javascript">
      ...
      var AJAX_URL = "{{ url('api/admin/invoice_buyer/ajax_alist') }}";//ajax请求的url
      var ADD_URL = "{{ url('admin/invoice_buyer/add/0') }}"; //添加url

      var IFRAME_MODIFY_URL = "{{url('admin/invoice_buyer/add/')}}/";//添加/修改页面地址前缀 + id
      var IFRAME_MODIFY_URL_TITLE = "发票配置购买方" ;// 详情弹窗显示提示  [添加/修改] +  栏目/主题
      var IFRAME_MODIFY_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面

      var SHOW_URL = "{{url('admin/invoice_buyer/info/')}}/";//显示页面地址前缀 + id
      var SHOW_URL_TITLE = "" ;// 详情弹窗显示提示
      var SHOW_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面
      var EDIT_URL = "{{url('admin/invoice_buyer/add/')}}/";//修改页面地址前缀 + id
      var DEL_URL = "{{ url('api/admin/invoice_buyer/ajax_del') }}";//删除页面地址
      var BATCH_DEL_URL = "{{ url('api/admin/invoice_buyer/ajax_del') }}";//批量删除页面地址
      var EXPORT_EXCEL_URL = "{{ url('admin/invoice_buyer/export') }}";//导出EXCEL地址
      var IMPORT_EXCEL_TEMPLATE_URL = "{{ url('admin/invoice_buyer/import_template') }}";//导入EXCEL模版地址
      var IMPORT_EXCEL_URL = "{{ url('api/admin/invoice_buyer/import') }}";//导入EXCEL地址
  ...
  <script src="{{ asset('js/admin/QualityControl/InvoiceBuyer.js') }}?2"  type="text/javascript"></script>
</body>
</html>
