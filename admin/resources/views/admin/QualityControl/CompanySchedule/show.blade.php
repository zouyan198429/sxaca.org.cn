...
{{--      <th>更新时间</th>--}}
  ...
  <script type="text/javascript">
      ...
      var AJAX_URL = "{{ url('api/admin/company_schedule/ajax_alist') }}";//ajax请求的url
      var ADD_URL = "{{ url('admin/company_schedule/add/0') }}"; //添加url

      var IFRAME_MODIFY_URL = "{{url('admin/company_schedule/add/')}}/";//添加/修改页面地址前缀 + id
      var IFRAME_MODIFY_URL_TITLE = "能力附表" ;// 详情弹窗显示提示  [添加/修改] +  栏目/主题
      var IFRAME_MODIFY_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面

      var SHOW_URL = "{{url('admin/company_schedule/info/')}}/";//显示页面地址前缀 + id
      var SHOW_URL_TITLE = "" ;// 详情弹窗显示提示
      var SHOW_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面
      var EDIT_URL = "{{url('admin/company_schedule/add/')}}/";//修改页面地址前缀 + id
      var DEL_URL = "{{ url('api/admin/company_schedule/ajax_del') }}";//删除页面地址
      var BATCH_DEL_URL = "{{ url('api/admin/company_schedule/ajax_del') }}";//批量删除页面地址
      var EXPORT_EXCEL_URL = "{{ url('admin/company_schedule/export') }}";//导出EXCEL地址
      var IMPORT_EXCEL_TEMPLATE_URL = "{{ url('admin/company_schedule/import_template') }}";//导入EXCEL模版地址
      var IMPORT_EXCEL_URL = "{{ url('api/admin/company_schedule/import') }}";//导入EXCEL地址
  ...
  <script src="{{ asset('js/admin/QualityControl/CompanySchedule_show.js') }}?1"  type="text/javascript"></script>
</body>
</html>
