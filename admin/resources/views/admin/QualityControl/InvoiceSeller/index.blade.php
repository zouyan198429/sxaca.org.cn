...
<div class="mm">
  <div class="mmhead" id="mywork">
    ...
    <div class="tabbox" >
      <a href="javascript:void(0);" class="on" onclick="action.iframeModify(0)">添加发票配置销售方</a>
    </div>
    <form onsubmit="return false;" class="form-horizontal" style="display: none;" role="form" method="post" id="search_frm" action="#">
      <div class="msearch fr">
        ...
        <select style="width:80px; height:28px;" name="field">
          <option value="xsf_mc">销售方名称</option>
            <option value="xsf_dz">销售方地址</option>
            <option value="xsf_dh">销售方电话</option>
            <option value="xsf_nsrsbh">销售方纳税人识别号</option>
            <option value="xsf_yh">销售方银行</option>
            <option value="xsf_yhzh">销售方银行账号</option>
        </select>
...
{{--      <th>ID</th>--}}
      <th>收款帐号<hr/>销售方名称</th>
        <th>销售方地址<hr/>销售方电话</th>
        <th>销售方纳税人识别号</th>
        <th>销售方银行<hr/>销售方银行账号</th>
        <th>开启状态</th>
      <th>创建时间<hr/>更新时间</th>
      <th>操作</th>
    </tr>
  ...
  <script type="text/javascript">
      ...
      var AJAX_URL = "{{ url('api/admin/invoice_seller/ajax_alist') }}";//ajax请求的url
      var ADD_URL = "{{ url('admin/invoice_seller/add/0') }}"; //添加url

      var IFRAME_MODIFY_URL = "{{url('admin/invoice_seller/add/')}}/";//添加/修改页面地址前缀 + id
      var IFRAME_MODIFY_URL_TITLE = "发票配置销售方" ;// 详情弹窗显示提示  [添加/修改] +  栏目/主题
      var IFRAME_MODIFY_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面

      var SHOW_URL = "{{url('admin/invoice_seller/info/')}}/";//显示页面地址前缀 + id
      var SHOW_URL_TITLE = "" ;// 详情弹窗显示提示
      var SHOW_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面
      var EDIT_URL = "{{url('admin/invoice_seller/add/')}}/";//修改页面地址前缀 + id
      var DEL_URL = "{{ url('api/admin/invoice_seller/ajax_del') }}";//删除页面地址
      var BATCH_DEL_URL = "{{ url('api/admin/invoice_seller/ajax_del') }}";//批量删除页面地址
      var EXPORT_EXCEL_URL = "{{ url('admin/invoice_seller/export') }}";//导出EXCEL地址
      var IMPORT_EXCEL_TEMPLATE_URL = "{{ url('admin/invoice_seller/import_template') }}";//导入EXCEL模版地址
      var IMPORT_EXCEL_URL = "{{ url('api/admin/invoice_seller/import') }}";//导入EXCEL地址
  ...
  <script src="{{ asset('js/admin/QualityControl/InvoiceSeller.js') }}?1"  type="text/javascript"></script>
</body>
</html>
