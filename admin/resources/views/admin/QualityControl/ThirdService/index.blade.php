...
<div class="mm">
  <div class="mmhead" id="mywork">
    ...
    <div class="tabbox" >
      <a href="javascript:void(0);" class="on" onclick="action.iframeModify(0)">添加第三方服务商</a>
    </div>
...
{{--      <th>更新时间</th>--}}
  ...
  <script type="text/javascript">
      ...
      var AJAX_URL = "{{ url('api/admin/third_service/ajax_alist') }}";//ajax请求的url
      var ADD_URL = "{{ url('admin/third_service/add/0') }}"; //添加url

      var IFRAME_MODIFY_URL = "{{url('admin/third_service/add/')}}/";//添加/修改页面地址前缀 + id
      var IFRAME_MODIFY_URL_TITLE = "第三方服务商管理" ;// 详情弹窗显示提示  [添加/修改] +  栏目/主题
      var IFRAME_MODIFY_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面

      var SHOW_URL = "{{url('admin/third_service/info/')}}/";//显示页面地址前缀 + id
      var SHOW_URL_TITLE = "" ;// 详情弹窗显示提示
      var SHOW_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面
      var EDIT_URL = "{{url('admin/third_service/add/')}}/";//修改页面地址前缀 + id
      var DEL_URL = "{{ url('api/admin/third_service/ajax_del') }}";//删除页面地址
      var BATCH_DEL_URL = "{{ url('api/admin/third_service/ajax_del') }}";//批量删除页面地址
      var EXPORT_EXCEL_URL = "{{ url('admin/third_service/export') }}";//导出EXCEL地址
      var IMPORT_EXCEL_TEMPLATE_URL = "{{ url('admin/third_service/import_template') }}";//导入EXCEL模版地址
      var IMPORT_EXCEL_URL = "{{ url('api/admin/third_service/import') }}";//导入EXCEL地址
      var IMPORT_EXCEL_CLASS = "import_file";// 导入EXCEL的file的class
      var SMS_SEND_PAGE_URL = "{{url('admin/third_service/sms_send')}}";// 选择短信模板页面
      var SMS_SEND_URL = "{{url('api/admin/third_service/ajax_sms_send')}}";// 短信模板发送短信

      var OPEN_OPERATE_URL = "{{ url('api/admin/third_service/ajax_open') }}";//审核操作(通过/不通过)
      var ACCOUNT_STATUS_URL = "{{ url('api/admin/third_service/ajax_frozen') }}";//操作(冻结/解冻)

      // 列表数据每隔指定时间就去执行一次刷新【如果表有更新时】--定时执行
      var IFRAME_TAG_KEY = "";// "QualityControl\\CTAPIThirdService";// 获得模型表更新时间的关键标签，可为空：不获取
