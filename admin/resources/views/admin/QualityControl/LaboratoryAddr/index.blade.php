...
<div class="mm">
  <div class="mmhead" id="mywork">
    ...
    <div class="tabbox" >
      <a href="javascript:void(0);" class="on" onclick="action.iframeModify(0)">添加实验室地址</a>
    </div>
...
{{--      <th>ID</th>--}}
  ...
  <script type="text/javascript">
      ...
      var AJAX_URL = "{{ url('api/admin/laboratory_addr/ajax_alist') }}";//ajax请求的url
      var ADD_URL = "{{ url('admin/laboratory_addr/add/0') }}"; //添加url

      var IFRAME_MODIFY_URL = "{{url('admin/laboratory_addr/add/')}}/";//添加/修改页面地址前缀 + id
      var IFRAME_MODIFY_URL_TITLE = "实验室地址" ;// 详情弹窗显示提示  [添加/修改] +  栏目/主题
      var IFRAME_MODIFY_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面

      var SHOW_URL = "{{url('admin/laboratory_addr/info/')}}/";//显示页面地址前缀 + id
      var SHOW_URL_TITLE = "" ;// 详情弹窗显示提示
      var SHOW_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面
      var EDIT_URL = "{{url('admin/laboratory_addr/add/')}}/";//修改页面地址前缀 + id
      var DEL_URL = "{{ url('api/admin/laboratory_addr/ajax_del') }}";//删除页面地址
      var BATCH_DEL_URL = "{{ url('api/admin/laboratory_addr/ajax_del') }}";//批量删除页面地址
      var EXPORT_EXCEL_URL = "{{ url('admin/laboratory_addr/export') }}";//导出EXCEL地址
      var IMPORT_EXCEL_TEMPLATE_URL = "{{ url('admin/laboratory_addr/import_template') }}";//导入EXCEL模版地址
      var IMPORT_EXCEL_URL = "{{ url('api/admin/laboratory_addr/import') }}";//导入EXCEL地址
  ...
  <script src="{{ asset('js/admin/QualityControl/LaboratoryAddr.js') }}?2"  type="text/javascript"></script>
</body>
</html>
