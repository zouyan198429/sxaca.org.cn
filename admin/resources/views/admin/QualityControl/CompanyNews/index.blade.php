...
<div class="mm">
  <div class="mmhead" id="mywork">
    ...
    <div class="tabbox" >
      <a href="javascript:void(0);" class="on" onclick="action.iframeModify(0)">添加企业其它</a>
    </div>
    <form onsubmit="return false;" class="form-horizontal" style="display: block;" role="form" method="post" id="search_frm" action="#">
      <div class="msearch fr">
        ...
        <select style="width:80px; height:28px;" name="field">
          <option value="new_title">标题</option>
        </select>
...
{{--      </th>--}}
  ...
  <script type="text/javascript">
      ...
      var AJAX_URL = "{{ url('api/admin/company_news/ajax_alist') }}";//ajax请求的url
      var ADD_URL = "{{ url('admin/company_news/add/0') }}"; //添加url

      var IFRAME_MODIFY_URL = "{{url('admin/company_news/add/')}}/";//添加/修改页面地址前缀 + id
      var IFRAME_MODIFY_URL_TITLE = "企业其它" ;// 详情弹窗显示提示  [添加/修改] +  栏目/主题
      var IFRAME_MODIFY_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面

      var SHOW_URL = "{{url('admin/company_news/info/')}}/";//显示页面地址前缀 + id
      var SHOW_URL_TITLE = "" ;// 详情弹窗显示提示
      var SHOW_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面
      var EDIT_URL = "{{url('admin/company_news/add/')}}/";//修改页面地址前缀 + id
      var DEL_URL = "{{ url('api/admin/company_news/ajax_del') }}";//删除页面地址
      var BATCH_DEL_URL = "{{ url('api/admin/company_news/ajax_del') }}";//批量删除页面地址
      var EXPORT_EXCEL_URL = "{{ url('admin/company_news/export') }}";//导出EXCEL地址
      var IMPORT_EXCEL_TEMPLATE_URL = "{{ url('admin/company_news/import_template') }}";//导入EXCEL模版地址
      var IMPORT_EXCEL_URL = "{{ url('api/admin/company_news/import') }}";//导入EXCEL地址
      ...
      var PARENT_BUSINESS_FUN_NAME = "adminQualityControlCompanyNewsIndex";
  ...
  <script src="{{ asset('js/admin/QualityControl/CompanyNews.js') }}?3"  type="text/javascript"></script>
</body>
</html>
