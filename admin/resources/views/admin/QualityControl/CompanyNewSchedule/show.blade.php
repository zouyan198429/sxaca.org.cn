...
{{--      <th>更新时间</th>--}}
  ...
  <script type="text/javascript">
      ...
      var SHOW_URL = "{{url('admin/company_new_schedule/info/')}}/";//显示页面地址前缀 + id
      var SHOW_URL_TITLE = "" ;// 详情弹窗显示提示
      var SHOW_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面
      var EDIT_URL = "{{url('admin/company_new_schedule/add/')}}/";//修改页面地址前缀 + id
      var DEL_URL = "{{ url('api/admin/company_new_schedule/ajax_del') }}";//删除页面地址
      var BATCH_DEL_URL = "{{ url('api/admin/company_new_schedule/ajax_del') }}";//批量删除页面地址
      var EXPORT_EXCEL_URL = "{{ url('admin/company_new_schedule/export') }}";//导出EXCEL地址
      var IMPORT_EXCEL_TEMPLATE_URL = "{{ url('admin/company_new_schedule/import_template') }}";//导入EXCEL模版地址
      var IMPORT_EXCEL_URL = "{{ url('api/admin/company_new_schedule/import') }}";//导入EXCEL地址
