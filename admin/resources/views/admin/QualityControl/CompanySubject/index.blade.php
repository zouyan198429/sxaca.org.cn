...
<div class="mm">
  <div class="mmhead" id="mywork">
    ...
    <div class="tabbox" >
      <a href="javascript:void(0);" class="on" onclick="action.iframeModify(0)">添加试题</a>
    </div>
    <form onsubmit="return false;" class="form-horizontal" style="display: block;" role="form" method="post" id="search_frm" action="#">
      <div class="msearch fr">
          ...
          <select class="wmini" name="type_no" style="width: 80px;">
              <option value="">试题分类</option>
              @foreach ($type_no_kv as $k=>$txt)
                  <option value="{{ $k }}"  @if(isset($defaultTypeNo) && $defaultTypeNo == $k) selected @endif >{{ $txt }}</option>
              @endforeach
          </select>
          <select class="wmini" name="subject_type" style="width: 80px;">
              <option value="">题目类型</option>
              @foreach ($subjectType as $k=>$txt)
                  <option value="{{ $k }}"  @if(isset($defaultSubjectType) && $defaultSubjectType == $k) selected @endif >{{ $txt }}</option>
              @endforeach
          </select>
          <select class="wmini" name="answer" style="width: 80px;">
              <option value="">正确答案</option>
              @foreach ($answer as $k=>$txt)
                  <option value="{{ $k }}"  @if(isset($defaultAnswer) && $defaultAnswer == $k) selected @endif >{{ $txt }}</option>
              @endforeach
          </select>
...
--}}
  <table lay-even class="layui-table table2 tableWidthFixed"  lay-size="lg"  id="dynamic-table">
    <colgroup>
        <col width="50">
{{--        <col width="60">--}}
        <col  width="10%" @if (isset($hidden_option) && $hidden_option == 1 ) style="display: none;"  @endif>
        <col width="7%" >
        <col width="7%" >
        <col width="15%" >
        <col width="15%">
...
{{--      <th>ID</th>--}}
  ...
  <script type="text/javascript">
      ...
      var AJAX_URL = "{{ url('api/admin/company_subject/ajax_alist') }}";//ajax请求的url
      var ADD_URL = "{{ url('admin/company_subject/add/0') }}"; //添加url

      var IFRAME_MODIFY_URL = "{{url('admin/company_subject/add/')}}/";//添加/修改页面地址前缀 + id
      var IFRAME_MODIFY_URL_TITLE = "试题" ;// 详情弹窗显示提示  [添加/修改] +  栏目/主题
      var IFRAME_MODIFY_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面

      var SHOW_URL = "{{url('admin/company_subject/info/')}}/";//显示页面地址前缀 + id
      var SHOW_URL_TITLE = "" ;// 详情弹窗显示提示
      var SHOW_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面
      var EDIT_URL = "{{url('admin/company_subject/add/')}}/";//修改页面地址前缀 + id
      var DEL_URL = "{{ url('api/admin/company_subject/ajax_del') }}";//删除页面地址
      var BATCH_DEL_URL = "{{ url('api/admin/company_subject/ajax_del') }}";//批量删除页面地址
      var EXPORT_EXCEL_URL = "{{ url('admin/company_subject/export') }}";//导出EXCEL地址
      var IMPORT_EXCEL_TEMPLATE_URL = "{{ url('admin/company_subject/import_template') }}";//导入EXCEL模版地址
      var IMPORT_EXCEL_URL = "{{ url('api/admin/company_subject/import') }}";//导入EXCEL地址
  ...
  <script src="{{ asset('js/admin/QualityControl/CompanySubject.js') }}?8"  type="text/javascript"></script>
</body>
</html>
