...
<div class="mm">
  <div class="mmhead" id="mywork">
    ...
    <div class="tabbox" >
      <a href="javascript:void(0);" class="on" onclick="action.iframeModify(0)">添加付款/收款项目</a>
    </div>
    <form onsubmit="return false;" class="form-horizontal" style="display: block;" role="form" method="post" id="search_frm" action="#">
      <div class="msearch fr">
        ...
        <select class="wmini" name="specified_amount_status">
          <option value="">是否指定金额</option>
          @foreach ($specifiedAmountStatus as $k=>$txt)
            <option value="{{ $k }}"  @if(isset($defaultSpecifiedAmountStatus) && $defaultSpecifiedAmountStatus == $k) selected @endif >{{ $txt }}</option>
          @endforeach
        </select>
          <select class="wmini" name="pay_valid_status">
              <option value="">收费生效时间</option>
              @foreach ($payValidStatus as $k=>$txt)
                  <option value="{{ $k }}"  @if(isset($defaultPayValidStatus) && $defaultPayValidStatus == $k) selected @endif >{{ $txt }}</option>
              @endforeach
          </select>
          ...
          <select class="wmini" name="unique_user_standard">
              <option value="">唯一用户付款标准</option>
              @foreach ($uniqueUserStandard as $k=>$txt)
                  <option value="{{ $k }}"  @if(isset($defaultUniqueUserStandard) && $defaultUniqueUserStandard == $k) selected @endif >{{ $txt }}</option>
              @endforeach
          </select>
          ...
          <select class="wmini" name="handle_method">
              <option value="">记录处理方式</option>
              @foreach ($handleMethod as $k=>$txt)
                  <option value="{{ $k }}"  @if(isset($defaultHandleMethod) && $defaultHandleMethod == $k) selected @endif >{{ $txt }}</option>
              @endforeach
          </select>
        ...
        <select style="width:80px; height:28px;" name="field">
          <option value="title">收费名称</option>
            <option value="pay_explain">收款文字说明</option>
        </select>
...
{{--      <th>ID</th>--}}
      ...
      <th>收款说明图片</th>
        <th>收款文字说明</th>
      <th>是否指定金额<hr/>记录处理方式<hr/>收集字段</th>
        <th>开启状态<hr/>收费状态<hr/>排序[降序]</th>
        <th>收费生效时间<hr/>收费起止时间</th>
        <th>付款时限<hr/>唯一用户付款标准</th>
        <th>有效时长<hr/>有效时限</th>
        <th>收款开通类型</th>
        <th>收款帐号<hr/>发票开票模板<hr/>发票商品项目模板</th>
      <th>创建时间<hr/>更新时间</th>
      <th>操作</th>
    </tr>
    </thead>
  ...
  <script type="text/javascript">
      ...
      var AJAX_URL = "{{ url('api/admin/payment_project/ajax_alist') }}";//ajax请求的url
      var ADD_URL = "{{ url('admin/payment_project/add/0') }}"; //添加url

      var IFRAME_MODIFY_URL = "{{url('admin/payment_project/add/')}}/";//添加/修改页面地址前缀 + id
      var IFRAME_MODIFY_URL_TITLE = "付款/收款项目" ;// 详情弹窗显示提示  [添加/修改] +  栏目/主题
      var IFRAME_MODIFY_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面

      var SHOW_URL = "{{url('admin/payment_project/info/')}}/";//显示页面地址前缀 + id
      var SHOW_URL_TITLE = "" ;// 详情弹窗显示提示
      var SHOW_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面
      var EDIT_URL = "{{url('admin/payment_project/add/')}}/";//修改页面地址前缀 + id
      var DEL_URL = "{{ url('api/admin/payment_project/ajax_del') }}";//删除页面地址
      var BATCH_DEL_URL = "{{ url('api/admin/payment_project/ajax_del') }}";//批量删除页面地址
      var EXPORT_EXCEL_URL = "{{ url('admin/payment_project/export') }}";//导出EXCEL地址
      var IMPORT_EXCEL_TEMPLATE_URL = "{{ url('admin/payment_project/import_template') }}";//导入EXCEL模版地址
      var IMPORT_EXCEL_URL = "{{ url('api/admin/payment_project/import') }}";//导入EXCEL地址
      ...
      var IFRAME_ADD_PAY_URL = "{{url('admin/payment_project/add_pay/')}}/";//收款页面地址前缀 + id
      ...
      var PAY_URL = "{{ url('admin/payment_record/pay') }}";//操作(缴费)
...
<script src="{{asset('js/common/list.js')}}?2"></script>
  <script src="{{ asset('js/admin/QualityControl/PaymentProject.js') }}?18"  type="text/javascript"></script>
