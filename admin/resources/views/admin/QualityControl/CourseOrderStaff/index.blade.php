...
{{--        <button class="btn btn-success  btn-xs export_excel"  onclick="otheraction.paySelected(this)" >缴费[勾选]</button>--}}
  ...
  <script type="text/javascript">
      ...
      var AJAX_URL = "{{ url('api/admin/course_order_staff/ajax_alist') }}";//ajax请求的url
      var ADD_URL = "{{ url('admin/course_order_staff/add/0') }}"; //添加url

      var IFRAME_MODIFY_URL = "{{url('admin/course_order_staff/add/')}}/";//添加/修改页面地址前缀 + id
      var IFRAME_MODIFY_URL_TITLE = "报名学员" ;// 详情弹窗显示提示  [添加/修改] +  栏目/主题
      var IFRAME_MODIFY_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面

      var SHOW_URL = "{{url('admin/course_order_staff/info/')}}/";//显示页面地址前缀 + id
      var SHOW_URL_TITLE = "" ;// 详情弹窗显示提示
      var SHOW_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面
      var EDIT_URL = "{{url('admin/course_order_staff/add/')}}/";//修改页面地址前缀 + id
      var DEL_URL = "{{ url('api/admin/course_order_staff/ajax_del') }}";//删除页面地址
      var BATCH_DEL_URL = "{{ url('api/admin/course_order_staff/ajax_del') }}";//批量删除页面地址
      var EXPORT_EXCEL_URL = "{{ url('admin/course_order_staff/export') }}";//导出EXCEL地址
