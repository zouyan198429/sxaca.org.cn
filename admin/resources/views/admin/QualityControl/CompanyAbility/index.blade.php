...
{{--      </th>--}}
  ...
  <script type="text/javascript">
      ...
      var AJAX_URL = "{{ url('api/admin/company_ability/ajax_alist') }}";//ajax请求的url
      var ADD_URL = "{{ url('admin/company_ability/add/0') }}"; //添加url

      var IFRAME_MODIFY_URL = "{{url('admin/company_ability/add/')}}/";//添加/修改页面地址前缀 + id
      var IFRAME_MODIFY_URL_TITLE = "能力验证" ;// 详情弹窗显示提示  [添加/修改] +  栏目/主题
      var IFRAME_MODIFY_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面

      var SHOW_URL = "{{url('admin/company_ability/info/')}}/";//显示页面地址前缀 + id
      var SHOW_URL_TITLE = "" ;// 详情弹窗显示提示
      var SHOW_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面
      var EDIT_URL = "{{url('admin/company_ability/add/')}}/";//修改页面地址前缀 + id
      var DEL_URL = "{{ url('api/admin/company_ability/ajax_del') }}";//删除页面地址
      var BATCH_DEL_URL = "{{ url('api/admin/company_ability/ajax_del') }}";//批量删除页面地址
      var EXPORT_EXCEL_URL = "{{ url('admin/company_ability/export') }}";//导出EXCEL地址
      var IMPORT_EXCEL_TEMPLATE_URL = "{{ url('admin/company_ability/import_template') }}";//导入EXCEL模版地址
      var IMPORT_EXCEL_URL = "{{ url('api/admin/company_ability/import') }}";//导入EXCEL地址
      ...
      var PARENT_BUSINESS_FUN_NAME = "adminQualityControlCompanyAbilityIndex";


      var SELECT_COMPANY_URL = "{{url('admin/company/select')}}";// 选择所属企业

      var IFRAME_UPEXCEL_URL = "{{url('admin/company_ability/add_excel/')}}/";//添加/修改页面地址前缀 + id
