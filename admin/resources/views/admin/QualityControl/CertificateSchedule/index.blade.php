...
{{--      <button class="btn btn-success  btn-xs export_excel"  onclick="action.smsSelected(this, 0, 2, 0, 0)" >发送短信[勾选]</button>--}}
  ...
  <script type="text/javascript">
      ...
      var AJAX_URL = "{{ url('api/admin/certificate_schedule/ajax_alist') }}";//ajax请求的url
      var ADD_URL = "{{ url('admin/certificate_schedule/add/0') }}"; //添加url

      var IFRAME_MODIFY_URL = "{{url('admin/certificate_schedule/add/')}}/";//添加/修改页面地址前缀 + id
      var IFRAME_MODIFY_URL_TITLE = "证书能力范围" ;// 详情弹窗显示提示  [添加/修改] +  栏目/主题
      var IFRAME_MODIFY_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面

      var SHOW_URL = "{{url('admin/certificate_schedule/info/')}}/";//显示页面地址前缀 + id
      var SHOW_URL_TITLE = "" ;// 详情弹窗显示提示
      var SHOW_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面
      var EDIT_URL = "{{url('admin/certificate_schedule/add/')}}/";//修改页面地址前缀 + id
      var DEL_URL = "{{ url('api/admin/certificate_schedule/ajax_del') }}";//删除页面地址
      var BATCH_DEL_URL = "{{ url('api/admin/certificate_schedule/ajax_del') }}";//批量删除页面地址
      var EXPORT_EXCEL_URL = "{{ url('admin/certificate_schedule/export') }}";//导出EXCEL地址
      var IMPORT_EXCEL_TEMPLATE_URL = "{{ url('admin/certificate_schedule/import_template') }}";//导入EXCEL模版地址
      var IMPORT_EXCEL_URL = "{{ url('api/admin/certificate_schedule/import') }}";//导入EXCEL地址
      ...
      var IFRAME_UPEXCEL_URL = "{{url('admin/certificate_schedule/add_excel/')}}/";//添加/修改页面地址前缀 + id
