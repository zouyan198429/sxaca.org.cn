...
{{--        <th>开票状态<hr/>开票时间</th>--}}
  ...
  <script type="text/javascript">
      ...
      var AJAX_URL = "{{ url('api/company/vod_orders/ajax_alist') }}";//ajax请求的url
      var ADD_URL = "{{ url('company/vod_orders/add/0') }}"; //添加url

      var IFRAME_MODIFY_URL = "{{url('company/vod_orders/add/')}}/";//添加/修改页面地址前缀 + id
      var IFRAME_MODIFY_URL_TITLE = "课程订单" ;// 详情弹窗显示提示  [添加/修改] +  栏目/主题
      var IFRAME_MODIFY_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面

      var SHOW_URL = "{{url('company/vod_orders/info/')}}/";//显示页面地址前缀 + id
      var SHOW_URL_TITLE = "" ;// 详情弹窗显示提示
      var SHOW_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面
      var EDIT_URL = "{{url('company/vod_orders/add/')}}/";//修改页面地址前缀 + id
      var DEL_URL = "{{ url('api/company/vod_orders/ajax_del') }}";//删除页面地址
      var BATCH_DEL_URL = "{{ url('api/company/vod_orders/ajax_del') }}";//批量删除页面地址
      var EXPORT_EXCEL_URL = "{{ url('company/vod_orders/export') }}";//导出EXCEL地址
      var IMPORT_EXCEL_TEMPLATE_URL = "{{ url('company/vod_orders/import_template') }}";//导入EXCEL模版地址
      var IMPORT_EXCEL_URL = "{{ url('api/company/vod_orders/import') }}";//导入EXCEL地址
...
{{--      var SELECT_COMPANY_URL = "{{url('admin/company/select')}}";// 选择所属企业--}}
      ...
      var PAY_URL = "{{ url('company/vod_orders/pay') }}";//操作(缴费)
      var COURSE_SHOW_URL = "{{url('web/vods/info/')}}/";// 前端课程查看地址
  </script>
  <script src="{{asset('js/common/list.js')}}?2"></script>
  <script src="{{ asset('js/company/QualityControl/VodOrders.js') }}?13"  type="text/javascript"></script>
</body>
</html>
