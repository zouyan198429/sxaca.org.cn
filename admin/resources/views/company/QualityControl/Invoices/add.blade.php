...
<div class="mm">
    <form class="am-form am-form-horizontal" method="post"  id="addForm">
        ...
        <table class="table1">
            ...
            <tr>
                <th>发票名称<span class="must">*</span></th>
                <td>
                    <input type="text" class="inp wnormal"  name="invoices_name" value="{{ $info['invoices_name'] ?? '' }}" placeholder="请输入发票名称"/>
                </td>
            </tr>
            <tr>
...
<script type="text/javascript">
    ...
    var SAVE_URL = "{{ url('api/company/invoices/ajax_save') }}";// ajax保存记录地址
    var LIST_URL = "{{url('company/invoices')}}";//保存成功后跳转到的地址
    // hidden_option 8192:调用父窗品的方法：[public/js目录下的] 项目目录+数据功能目录+当前文件名称 【有_线，则去掉】
    // 其它地方弹出此窗，保存完成时调用的父窗口方法名称 参数(obj:当前表单值对像, result:保存接口返回的结果，operateNum:自己定义的一个编号【页面有多处用到时用--通知父窗口调用位置】)
    var PARENT_BUSINESS_FUN_NAME = "adminQualityControlInvoicesedit";

    var SELECT_COMPANY_URL = "{{url('company/company/select')}}";// 选择所属企业
</script>
<script src="{{ asset('/js/company/QualityControl/Invoices_edit.js') }}"  type="text/javascript"></script>
</body>
</html>
