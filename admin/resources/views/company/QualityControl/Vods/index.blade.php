...
{{--      <th>排序[降序]</th>--}}
  ...
  <script type="text/javascript">
      ...
      var AJAX_URL = "{{ url('api/company/vods/ajax_alist') }}";//ajax请求的url
      var ADD_URL = "{{ url('company/vods/add/0') }}"; //添加url

      var IFRAME_MODIFY_URL = "{{url('company/vods/add/')}}/";//添加/修改页面地址前缀 + id
      var IFRAME_MODIFY_URL_TITLE = "课程" ;// 详情弹窗显示提示  [添加/修改] +  栏目/主题
      var IFRAME_MODIFY_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面

      var SHOW_URL = "{{url('company/vods/info/')}}/";//显示页面地址前缀 + id
      var SHOW_URL_TITLE = "" ;// 详情弹窗显示提示
      var SHOW_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面
      var EDIT_URL = "{{url('company/vods/add/')}}/";//修改页面地址前缀 + id
      var DEL_URL = "{{ url('api/company/vods/ajax_del') }}";//删除页面地址
      var BATCH_DEL_URL = "{{ url('api/company/vods/ajax_del') }}";//批量删除页面地址
      var EXPORT_EXCEL_URL = "{{ url('company/vods/export') }}";//导出EXCEL地址
      var IMPORT_EXCEL_TEMPLATE_URL = "{{ url('company/vods/import_template') }}";//导入EXCEL模版地址
      var IMPORT_EXCEL_URL = "{{ url('api/company/vods/import') }}";//导入EXCEL地址
      var IMPORT_EXCEL_CLASS = "import_file";// 导入EXCEL的file的class
      var SMS_SEND_PAGE_URL = "{{url('company/rrr_dddd/sms_send')}}";// 选择短信模板页面
      var SMS_SEND_URL = "{{url('api/company/rrr_dddd/ajax_sms_send')}}";// 短信模板发送短信
      ...
      var VIDEO_LIST_URL = "{{url('company/vod_video')}}";// 课程课件管理地址
...
<script src="{{asset('js/common/list.js')}}?2"></script>
  <script src="{{ asset('js/company/QualityControl/Vods.js') }}?9"  type="text/javascript"></script>
