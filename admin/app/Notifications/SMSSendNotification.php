...
class SMSSendNotification extends BaseQueueNotifications
{
    // use Queueable;
   ...
   // public $SMSParams = [];
    ...
    public function via($notifiable)
    {
         ...
         *           "countryCode":"86",
         *           "mobile":["15829686962"],
         *           "operateKey":"reg",--去掉不要了
         *            "dataParams":{// 根据实际情况组织参数
         *                  "operateType":"注册",
         *                  "code":"1849",
         *                  "validMinute":3
         *             }
...
//            'qcloud', //腾讯云
//        ]
        $gateways = $notifiable->gateways ?? [];// $smsConfig['default']['gateways'] ?? [];
        $configs = $notifiable->smsConfig ?? [];// $smsConfig['gateways'] ?? [];
...
//        if(isset($dataParams['validMinute'])) $needCache = true;
        ...
        $smsLogId = $notifiable->smsLogId ?? 0;
...
//            }
            ...
            if(method_exists(new SendSMS,'sendCommonSmsLogStatusOperate'))  SendSMS::sendCommonSmsLogStatusOperate($notifiable, $smsLogId, 1);
            return true;
        } catch ( \Exception $e) {
            $errMsg = $e->getMessage();
            // Log::info('消息通知日志 --发送短信失败-->'  . date('Y-m-d H:i:s') . __CLASS__ . '->' . __FUNCTION__, [$mobiles, $countryCode, $code, $errMsg]);
            Log::info('消息通知日志 --发送短信失败-->'  . date('Y-m-d H:i:s') . __CLASS__ . '->' . __FUNCTION__, [$mobiles, $countryCode, $errMsg]);

            // 操作失败时，处理失败的业务逻辑 --可以不处理
            if(method_exists(new SendSMS,'sendCommonSmsLogStatusOperate')) SendSMS::sendCommonSmsLogStatusOperate($notifiable, $smsLogId, 2, $errMsg);
