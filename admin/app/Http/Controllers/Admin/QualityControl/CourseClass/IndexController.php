...
//        }, $this->errMethod, $reDataArr, $this->errorView);
        return $this->exeDoPublicFun($request, 1, 1, 'admin.QualityControl.CourseClassAdmin.index', true
            , 'doListPage', ['class_id' => $class_id], function (&$reDataArr) use ($request){
     ...
     * @author zouyan(305463219@qq.com)
     */
    public function basic(Request $request, $class_id = 0)
    {
...
//
//        }, $this->errMethod, $reDataArr, $this->errorView);
        return $this->exeDoPublicFun($request, 1, 1, 'admin.QualityControl.CourseClassAdmin.basic', true
            , '', [], function (&$reDataArr) use ($request, &$class_id){
                $reDataArr['class_id'] = $class_id;

                $classInfo = $this->getCourseClassInfo($class_id);
                // $relationFormatConfigs = CTAPICourseClassBusiness::getRelationConfigs($request, $this, ['project_standards_list' => '', 'project_submit_items_list' => ''], []);
                // CTAPICourseClassBusiness::formatRelationList( $request, $this, $classInfo, $relationFormatConfigs);
                $reDataArr['info'] = $classInfo;
                // pr($reDataArr);
    ...
    public function doListPage(Request $request, &$reDataArr, $extendParams = []){
        ...
        // $id = $extendParams['params']['id'];
         $class_id = $extendParams['params']['class_id'];
