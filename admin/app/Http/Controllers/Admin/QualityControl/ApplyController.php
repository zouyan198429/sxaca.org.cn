...
use App\Models\QualityControl\Apply;
...
//            return view('admin.QualityControl.Apply.index', $reDataArr);
//
//        }, $this->errMethod, $reDataArr, $this->errorView);
        return $this->exeDoPublicFun($request, 1, 1, 'admin.QualityControl.Apply.index', true
...
////            // $reDataArr = $this->reDataArr;
////            $reDataArr = array_merge($reDataArr, $this->reDataArr);
////            $reDataArr['province_kv'] = CTAPIApplyBusiness::getCityByPid($request, $this,  0);
////            $reDataArr['province_kv'] = CTAPIApplyBusiness::getChildListKeyVal($request, $this, 0, 1 + 0, 0);
////            $reDataArr['province_id'] = 0;
////            return view('admin.QualityControl.Apply.select', $reDataArr);
////
////        }, $this->errMethod, $reDataArr, $this->errorView);
//        return $this->exeDoPublicFun($request, 2048, 1, 'admin.QualityControl.Apply.select', true
//            , 'doListPage', [], function (&$reDataArr) use ($request){
//
//            });
//    }
...
//            if ($id > 0) { // 获得详情数据
//                $operate = "修改";
//                $info = CTAPIApplyBusiness::getInfoData($request, $this, $id, [], '', []);
//            }
//            // $reDataArr = array_merge($reDataArr, $resultDatas);
//            $reDataArr['info'] = $info;
//            $reDataArr['operate'] = $operate;
//            return view('admin.QualityControl.Apply.add', $reDataArr);
//
//        }, $this->errMethod, $reDataArr, $this->errorView);
        $pageNum = ($id > 0) ? 64 : 16;
        return $this->exeDoPublicFun($request, $pageNum, 1,'admin.QualityControl.Apply.add', true
    ...
    /**
     * @OA\Get(
     *     path="/api/admin/apply/ajax_info",
     *     tags={"大后台-第三方服务商-应用管理"},
     *     summary="应用管理--详情",
     *     description="根据单个id,查询详情记录......",
     *     operationId="adminQualityControlApplyAjax_info",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_apply_id_required"),
     *     @OA\Response(response=200,ref="#/components/responses/Response_QualityControl_info_apply"),
     *     @OA\Response(response=400,ref="#/components/responses/common_Response_err_400"),
     *     @OA\Response(response=404,ref="#/components/responses/common_Response_err_404"),
     * )
     *     请求主体对象
     *     requestBody={"$ref": "#/components/requestBodies/RequestBody_QualityControl_info_apply"}
     */
    /**
     * ajax获得详情数据
     *
...
//        return ajaxDataArr(1, $resultDatas, '');
    ...
    public function ajax_open(Request $request)
    {
        ...
        $organize_id = 0;
        $modifyNum = CTAPIApplyBusiness::openAjax($request, $this, $organize_id, $id, $open_status);
        return ajaxDataArr(1, ['modify_num' => $modifyNum], '');
    }

    /**
     * @OA\Post(
     *     path="/api/admin/apply/ajax_save",
     *     tags={"大后台-第三方服务商-应用管理"},
     *     summary="应用管理--新加/修改",
     *     description="根据单个id,新加/修改记录(id>0:修改；id=0:新加)......",
     *     operationId="adminQualityControlApplyAjax_save",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_apply_id_required"),
...
//        $this->InitParams($request);
        ...
        return $this->exeDoPublicFun($request, $pageNum, 4,'', true
            , '', [], function (&$reDataArr) use ($request){
                ...
                $app_name = CommonRequest::get($request, 'app_name');
                ...
                if($id <= 0) {// 新加;要加入的特别字段
                    $addNewData = [
                        'app_id' => CTAPIApplyBusiness::createAppId($request, $this),// 应用id[32位，唯一]-公匙
                        'app_secret' => Tool::generatePassword(32, 1 | 2 | 4),// 应用API密钥-私匙[32位]
                    ...
                    $info = CTAPIApplyBusiness::getInfoData($request, $this, $id, [], '', []);
                    // 如果改变了所属企业,需要重新统计数
                    if(isset($saveData['staff_id']) && $staff_id != $info['staff_id']) $saveData['force_company_num'] = 1;
                }
                $extParams = [
                    'judgeDataKey' => 'replace',// 数据验证的下标
                ];
                $resultDatas = CTAPIApplyBusiness::replaceById($request, $this, $saveData, $id, $extParams, true);
                return ajaxDataArr(1, $resultDatas, '');
            });
    }
    ...
    /**
     * @OA\Get(
     *     path="/api/admin/apply/ajax_alist",
     *     tags={"大后台-第三方服务商-应用管理"},
     *     summary="应用管理--列表",
     *     description="应用管理--列表......",
     *     operationId="adminQualityControlApplyAjax_alist",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_apply_id_optional"),
     *     @OA\Response(response=200,ref="#/components/responses/Response_QualityControl_list_apply"),
     *     @OA\Response(response=400,ref="#/components/responses/common_Response_err_400"),
     *     @OA\Response(response=404,ref="#/components/responses/common_Response_err_404"),
     * )
     *     请求主体对象
...
//        $this->InitParams($request);
//        return  CTAPIApplyBusiness::getList($request, $this, 2 + 4);
        return $this->exeDoPublicFun($request, 4, 4,'', true, '', [], function (&$reDataArr) use ($request){

            $handleKeyConfigArr = ['staff_info' => '', 'resource_list' => ''];
            $extParams = [
                // 'handleKeyArr' => $handleKeyArr,//一维数组，数数据需要处理的标记，每一个或类处理，根据情况 自定义标记，然后再处理函数中处理数据。
                'relationFormatConfigs'=> CTAPIApplyBusiness::getRelationConfigs($request, $this, $handleKeyConfigArr, []),
            ];
            return  CTAPIApplyBusiness::getList($request, $this, 2 + 4, [], [], $extParams);
        });
    }
...
//        $this->InitParams($request);
//        $result = CTAPIApplyBusiness::getList($request, $this, 1 + 0);
...
////        $this->InitParams($request);
////        CTAPIApplyBusiness::getList($request, $this, 1 + 0);
...
//                'relationFormatConfigs'=> CTAPIApplyBusiness::getRelationConfigs($request, $this, $handleKeyConfigArr, []),
//            ];
//            CTAPIApplyBusiness::getList($request, $this, 1 + 0, [], [], $extParams);
//        });
//    }
...
////        $this->InitParams($request);
////        CTAPIApplyBusiness::importTemplate($request, $this);
...
//    }
    ...
    /**
     * @OA\Post(
     *     path="/api/admin/apply/ajax_del",
     *     tags={"大后台-第三方服务商-应用管理"},
     *     summary="应用管理--删除",
     *     description="根据单个id,删除记录......",
     *     operationId="adminQualityControlApplyAjax_del",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_apply_id_required"),
    ...
    {
//        $this->InitParams($request);
//        return CTAPIApplyBusiness::delAjax($request, $this);
        ...
        return $this->exeDoPublicFun($request, $pageNum, 4,'', true, '', [], function (&$reDataArr) use ($request){
            $organize_id = CommonRequest::getInt($request, 'staff_id');// 可有此参数
            return CTAPIApplyBusiness::delCustomizeAjax($request,  $this, $organize_id, [], '');
//            return CTAPIApplyBusiness::delAjax($request, $this);
        });
    }
...
//        // 获得一级城市信息一维数组[$k=>$v]
//        $childKV = CTAPIApplyBusiness::getCityByPid($request, $this, $parent_id);
//        // $childKV = CTAPIApplyBusiness::getChildListKeyVal($request, $this, $parent_id, 1 + 0);
//
...
//        $resultDatas = CTAPIApplyBusiness::importByFile($request, $this, $fileName);
//        return ajaxDataArr(1, $resultDatas, '');
///
...
//    }
    ...
    public function up_file(Request $request)
    {
        ...
        // $this->company_id = 1;
        // 企业 的 个人--只能读自己的人员信息
//        $organize_id = $this->user_id;// CommonRequest::getInt($request, 'staff_id');
...
//        $reDataArr['defaultAdminType'] = -1;// 列表页默认状态
        ...
        if(is_numeric($staff_id) && $staff_id > 0){
            // 获得企业信息
            $companyInfo = CTAPIStaffBusiness::getInfoData($request, $this, $staff_id);
            if(empty($companyInfo)) throws('企业信息不存在！');
            $info['staff_id'] = $staff_id;
            $info['user_staff_name'] = $companyInfo['real_name'] ?? '';
            $staff_hidden = 1;
        }
        $reDataArr['info'] = $info;
        $reDataArr['staff_hidden'] = $staff_hidden;// =1 : 隐藏企业选择

        // 审核状态
        $reDataArr['openStatus'] =  Apply::$openStatusArr;
    ...
    public function doInfoPage(Request $request, &$reDataArr, $extendParams = []){
        ...
        if ($id > 0) { // 获得详情数据
            ...
            $info = CTAPIApplyBusiness::getInfoData($request, $this, $id, [], '', $extParams);
        }
        // $reDataArr = array_merge($reDataArr, $resultDatas);
        $reDataArr['info'] = $info;
        $reDataArr['operate'] = $operate;

        $staff_hidden = CommonRequest::getInt($request, 'staff_hidden');
        $reDataArr['staff_hidden'] = $staff_hidden;// =1 : 隐藏企业选择
