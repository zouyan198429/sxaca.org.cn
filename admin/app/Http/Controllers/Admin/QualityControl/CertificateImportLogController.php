...
use App\Business\Controller\API\QualityControl\CTAPICertificateImportLogBusiness;
...
class CertificateImportLogController extends BasicController
{
...
//            return view('admin.QualityControl.CertificateImportLog.index', $reDataArr);
//
//        }, $this->errMethod, $reDataArr, $this->errorView);
        return $this->exeDoPublicFun($request, 1, 1, 'admin.QualityControl.CertificateImportLog.index', true
...
////            // $reDataArr = $this->reDataArr;
////            $reDataArr = array_merge($reDataArr, $this->reDataArr);
////            $reDataArr['province_kv'] = CTAPICertificateImportLogBusiness::getCityByPid($request, $this,  0);
////            $reDataArr['province_kv'] = CTAPICertificateImportLogBusiness::getChildListKeyVal($request, $this, 0, 1 + 0, 0);
////            $reDataArr['province_id'] = 0;
////            return view('admin.QualityControl.CertificateImportLog.select', $reDataArr);
////
////        }, $this->errMethod, $reDataArr, $this->errorView);
//        return $this->exeDoPublicFun($request, 2048, 1, 'admin.QualityControl.CertificateImportLog.select', true
//            , 'doListPage', [], function (&$reDataArr) use ($request){
//
//            });
//    }
...
//            if ($id > 0) { // 获得详情数据
//                $operate = "修改";
//                $info = CTAPICertificateImportLogBusiness::getInfoData($request, $this, $id, [], '', []);
//            }
//            // $reDataArr = array_merge($reDataArr, $resultDatas);
//            $reDataArr['info'] = $info;
//            $reDataArr['operate'] = $operate;
//            return view('admin.QualityControl.CertificateImportLog.add', $reDataArr);
//
//        }, $this->errMethod, $reDataArr, $this->errorView);
        $pageNum = ($id > 0) ? 64 : 16;
        return $this->exeDoPublicFun($request, $pageNum, 1,'admin.QualityControl.CertificateImportLog.add', true
    ...
    {
        return $this->exeDoPublicFun($request, 17179869184, 1,'admin.QualityControl.CertificateImportLog.info', true
            , 'doInfoPage', ['id' => $id], function (&$reDataArr) use ($request){
     ...
     *     operationId="adminQualityControlCertificateImportLogAjax_info",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_certificate_import_log_id_required"),
     *     @OA\Response(response=200,ref="#/components/responses/Response_QualityControl_info_certificate_import_log"),
     *     @OA\Response(response=400,ref="#/components/responses/common_Response_err_400"),
     *     @OA\Response(response=404,ref="#/components/responses/common_Response_err_404"),
     * )
     *     请求主体对象
     *     requestBody={"$ref": "#/components/requestBodies/RequestBody_QualityControl_info_certificate_import_log"}
     */
    /**
     * ajax获得详情数据
     *
...
//        return ajaxDataArr(1, $resultDatas, '');
    ...
    /**
     * @OA\Post(
     *     path="/api/admin/certificate_import_log/ajax_save",
     *     tags={"大后台-系统设置-证书"},
     *     summary="证书--新加/修改",
     *     description="根据单个id,新加/修改记录(id>0:修改；id=0:新加)......",
     *     operationId="adminQualityControlCertificateImportLogAjax_save",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_certificate_import_log_id_required"),
...
//        $this->InitParams($request);
        ...
        return $this->exeDoPublicFun($request, $pageNum, 4,'', true
            , '', [], function (&$reDataArr) use ($request){
                ...
                $import_no = CommonRequest::get($request, 'import_no');
                $import_content = CommonRequest::get($request, 'import_content');
                $import_time = CommonRequest::get($request, 'import_time');
                $success_num = CommonRequest::getInt($request, 'success_num');
                $fail_num = CommonRequest::getInt($request, 'fail_num');
                // 判断导入时间
                Tool::judgeBeginEndDate($import_time, '', 1 , 1, date('Y-m-d'), '导入时间');

                $saveData = [
                    'import_no' => $import_no,
                    'import_time' => $import_time,
                    'import_content' => replace_enter_char($import_content, 1),
...
//        }
                ...
                $resultDatas = CTAPICertificateImportLogBusiness::replaceById($request, $this, $saveData, $id, $extParams, true);
                return ajaxDataArr(1, $resultDatas, '');
            });
    }
    ...
    /**
     * @OA\Get(
     *     path="/api/admin/certificate_import_log/ajax_alist",
     *     tags={"大后台-系统设置-证书"},
     *     summary="证书--列表",
     *     description="证书--列表......",
     *     operationId="adminQualityControlCertificateImportLogAjax_alist",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_certificate_import_log_id_optional"),
     *     @OA\Response(response=200,ref="#/components/responses/Response_QualityControl_list_certificate_import_log"),
     *     @OA\Response(response=400,ref="#/components/responses/common_Response_err_400"),
     *     @OA\Response(response=404,ref="#/components/responses/common_Response_err_404"),
     * )
     *     请求主体对象
...
//        $this->InitParams($request);
//        return  CTAPICertificateImportLogBusiness::getList($request, $this, 2 + 4);
        return $this->exeDoPublicFun($request, 4, 4,'', true, '', [], function (&$reDataArr) use ($request){

            $handleKeyConfigArr = ['company_info'];
            $extParams = [
                // 'handleKeyArr' => $handleKeyArr,//一维数组，数数据需要处理的标记，每一个或类处理，根据情况 自定义标记，然后再处理函数中处理数据。
                'relationFormatConfigs'=> CTAPICertificateImportLogBusiness::getRelationConfigs($request, $this, $handleKeyConfigArr, []),
            ];
            return  CTAPICertificateImportLogBusiness::getList($request, $this, 2 + 4, [], [], $extParams);
        });
    }
...
//        $this->InitParams($request);
//        $result = CTAPICertificateImportLogBusiness::getList($request, $this, 1 + 0);
...
//        $this->InitParams($request);
//        CTAPICertificateImportLogBusiness::getList($request, $this, 1 + 0);
...
//        $this->InitParams($request);
//        CTAPICertificateImportLogBusiness::importTemplate($request, $this);
...
//    }
    ...
    /**
     * @OA\Post(
     *     path="/api/admin/certificate_import_log/ajax_del",
     *     tags={"大后台-系统设置-证书"},
     *     summary="证书--删除",
     *     description="根据单个id,删除记录......",
     *     operationId="adminQualityControlCertificateImportLogAjax_del",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_certificate_import_log_id_required"),
    ...
    {
//        $this->InitParams($request);
//        return CTAPICertificateImportLogBusiness::delAjax($request, $this);
...
//        // 获得一级城市信息一维数组[$k=>$v]
//        $childKV = CTAPICertificateImportLogBusiness::getCityByPid($request, $this, $parent_id);
//        // $childKV = CTAPICertificateImportLogBusiness::getChildListKeyVal($request, $this, $parent_id, 1 + 0);
//
...
//        $resultDatas = CTAPICertificateImportLogBusiness::importByFile($request, $this, $fileName);
//        return ajaxDataArr(1, $resultDatas, '');
///
...
//        $reDataArr['defaultAdminType'] = -1;// 列表页默认状态
    ...
    public function doInfoPage(Request $request, &$reDataArr, $extendParams = []){
        ...
        if ($id > 0) { // 获得详情数据
            ...
            $info = CTAPICertificateImportLogBusiness::getInfoData($request, $this, $id, [], '', $extParams);
        }
        // $reDataArr = array_merge($reDataArr, $resultDatas);
        $reDataArr['info'] = $info;
        $reDataArr['operate'] = $operate;
