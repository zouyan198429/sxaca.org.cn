...
use App\Models\QualityControl\InvoiceTemplate;
use App\Services\Request\CommonRequest;
use App\Services\Tool;
use Illuminate\Http\Request;

class InvoiceTemplateController extends BasicController
{
...
//            return view('admin.QualityControl.InvoiceTemplate.index', $reDataArr);
//
//        }, $this->errMethod, $reDataArr, $this->errorView);
        return $this->exeDoPublicFun($request, 1, 1, 'admin.QualityControl.InvoiceTemplate.index', true
...
//            // $reDataArr = $this->reDataArr;
//            $reDataArr = array_merge($reDataArr, $this->reDataArr);
//            $reDataArr['province_kv'] = CTAPIInvoiceTemplateBusiness::getCityByPid($request, $this,  0);
//            $reDataArr['province_kv'] = CTAPIInvoiceTemplateBusiness::getChildListKeyVal($request, $this, 0, 1 + 0, 0);
//            $reDataArr['province_id'] = 0;
//            return view('admin.QualityControl.InvoiceTemplate.select', $reDataArr);
//
//        }, $this->errMethod, $reDataArr, $this->errorView);
...
//            // $reDataArr = $this->reDataArr;
//            $reDataArr = array_merge($reDataArr, $this->reDataArr);
//            return view('admin.QualityControl.InvoiceTemplate.add', $reDataArr);
//
//        }, $this->errMethod, $reDataArr, $this->errorView);

        $pageNum = ($id > 0) ? 64 : 16;
        return $this->exeDoPublicFun($request, $pageNum, 1,'admin.QualityControl.InvoiceTemplate.add', true
    ...
    {
        return $this->exeDoPublicFun($request, 17179869184, 1,'admin.QualityControl.InvoiceTemplate.info', true
            , 'doInfoPage', ['id' => $id], function (&$reDataArr) use ($request){
    ...
    /**
     * @OA\Get(
     *     path="/api/admin/invoice_template/ajax_info",
     *     tags={"大后台-电子发票管理-发票开票模板"},
     *     summary="发票开票模板--详情",
     *     description="根据单个id,查询详情记录......",
     *     operationId="adminQualityControlInvoiceTemplateAjax_info",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_invoice_template_id_required"),
     *     @OA\Response(response=200,ref="#/components/responses/Response_QualityControl_info_invoice_template"),
     *     @OA\Response(response=400,ref="#/components/responses/common_Response_err_400"),
     *     @OA\Response(response=404,ref="#/components/responses/common_Response_err_404"),
     * )
     *     请求主体对象
     *     requestBody={"$ref": "#/components/requestBodies/RequestBody_QualityControl_info_invoice_template"}
     */
    /**
     * ajax获得详情数据
     *
    ...
    public function ajax_info(Request $request){
        ...
        $info = CTAPIInvoiceTemplateBusiness::getInfoData($request, $this, $id, [], '', []);
...
//        });
    ...
    /**
     * @OA\Post(
     *     path="/api/admin/invoice_template/ajax_save",
     *     tags={"大后台-电子发票管理-发票开票模板"},
     *     summary="发票开票模板--新加/修改",
     *     description="根据单个id,新加/修改记录(id>0:修改；id=0:新加)......",
     *     operationId="adminQualityControlInvoiceTemplateAjax_save",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_invoice_template_id_required"),
...
//        $this->InitParams($request);
        ...
        return $this->exeDoPublicFun($request, $pageNum, 4,'', true
            , '', [], function (&$reDataArr) use ($request){
                ...
                $fhr = CommonRequest::get($request, 'fhr');
                $bz = CommonRequest::get($request, 'bz');
...
//        }
                ...
                $resultDatas = CTAPIInvoiceTemplateBusiness::replaceById($request, $this, $saveData, $id, $extParams, true);
                return ajaxDataArr(1, $resultDatas, '');
        });
    }
    ...
    /**
     * @OA\Get(
     *     path="/api/admin/invoice_template/ajax_alist",
     *     tags={"大后台-电子发票管理-发票开票模板"},
     *     summary="发票开票模板--列表",
     *     description="发票开票模板--列表......",
     *     operationId="adminQualityControlInvoiceTemplateAjax_alist",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_invoice_template_id_optional"),
     *     @OA\Response(response=200,ref="#/components/responses/Response_QualityControl_list_invoice_template"),
     *     @OA\Response(response=400,ref="#/components/responses/common_Response_err_400"),
     *     @OA\Response(response=404,ref="#/components/responses/common_Response_err_404"),
     * )
     *     请求主体对象
...
//        $this->InitParams($request);
//        return  CTAPIInvoiceTemplateBusiness::getList($request, $this, 2 + 4);
        return $this->exeDoPublicFun($request, 4, 4,'', true, '', [], function (&$reDataArr) use ($request){

            $extParams = [
                // 'handleKeyArr' => $handleKeyArr,//一维数组，数数据需要处理的标记，每一个或类处理，根据情况 自定义标记，然后再处理函数中处理数据。
                'relationFormatConfigs'=> CTAPIInvoiceTemplateBusiness::getRelationConfigs($request, $this,
                    [
                        // 'pay_company_name' => '',
                    ], []),
                // 'listHandleKeyArr' => ['priceIntToFloat'],
            ];
            return  CTAPIInvoiceTemplateBusiness::getList($request, $this, 2 + 4, [], [], $extParams);
        });
    }
...
//        $this->InitParams($request);
//        $result = CTAPIInvoiceTemplateBusiness::getList($request, $this, 1 + 0);
...
//        $this->InitParams($request);
//        CTAPIInvoiceTemplateBusiness::getList($request, $this, 1 + 0);
...
//                // 'listHandleKeyArr' => ['priceIntToFloat'],
//            ];
//            CTAPIInvoiceTemplateBusiness::getList($request, $this, 1 + 0, [], [], $extParams);
//        });
//    }
...
//        $this->InitParams($request);
//        CTAPIInvoiceTemplateBusiness::importTemplate($request, $this);
...
//    }
    ...
    /**
     * @OA\Post(
     *     path="/api/admin/invoice_template/ajax_del",
     *     tags={"大后台-电子发票管理-发票开票模板"},
     *     summary="发票开票模板--删除",
     *     description="根据单个id,删除记录......",
     *     operationId="adminQualityControlInvoiceTemplateAjax_del",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_invoice_template_id_required"),
    ...
    {
//        $this->InitParams($request);
//        return CTAPIInvoiceTemplateBusiness::delAjax($request, $this);
...
//        // 获得一级城市信息一维数组[$k=>$v]
//        $childKV = CTAPIInvoiceTemplateBusiness::getCityByPid($request, $this, $parent_id);
//        // $childKV = CTAPIInvoiceTemplateBusiness::getChildListKeyVal($request, $this, $parent_id, 1 + 0);
//
...
//        $resultDatas = CTAPIInvoiceTemplateBusiness::importByFile($request, $this, $fileName);
//        return ajaxDataArr(1, $resultDatas, '');
///
...
//        $reDataArr['defaultAdminType'] = -1;// 列表页默认状态
        ...
        // 开启状态(1开启2关闭)
        $reDataArr['openStatus'] =  InvoiceTemplate::$openStatusArr;
        ...
        $reDataArr['tspz'] =  InvoiceTemplate::$tspzArr;
...
//        $reDataArr['defaultAdminType'] = -1;// 列表页默认状态
        ...
        if ($id > 0) { // 获得详情数据
            ...
            $info = CTAPIInvoiceTemplateBusiness::getInfoData($request, $this, $id, [], '', $extParams);
        }
