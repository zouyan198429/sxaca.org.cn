...
use App\Models\QualityControl\VodType;
use App\Services\Request\CommonRequest;
use App\Services\Tool;
use Illuminate\Http\Request;

class VodTypeController extends BasicController
{
...
//            return view('admin.QualityControl.VodType.index', $reDataArr);
//
//        }, $this->errMethod, $reDataArr, $this->errorView);
        return $this->exeDoPublicFun($request, 1, 1, 'admin.QualityControl.VodType.index', true
...
//            // $reDataArr = $this->reDataArr;
//            $reDataArr = array_merge($reDataArr, $this->reDataArr);
//            $reDataArr['province_kv'] = CTAPIVodTypeBusiness::getCityByPid($request, $this,  0);
//            $reDataArr['province_kv'] = CTAPIVodTypeBusiness::getChildListKeyVal($request, $this, 0, 1 + 0, 0);
//            $reDataArr['province_id'] = 0;
//            return view('admin.QualityControl.VodType.select', $reDataArr);
//
//        }, $this->errMethod, $reDataArr, $this->errorView);
...
//            // $reDataArr = $this->reDataArr;
//            $reDataArr = array_merge($reDataArr, $this->reDataArr);
//            return view('admin.QualityControl.VodType.add', $reDataArr);
//
//        }, $this->errMethod, $reDataArr, $this->errorView);

        $pageNum = ($id > 0) ? 64 : 16;
        return $this->exeDoPublicFun($request, $pageNum, 1,'admin.QualityControl.VodType.add', true
    ...
    {
        return $this->exeDoPublicFun($request, 17179869184, 1,'admin.QualityControl.VodType.info', true
            , 'doInfoPage', ['id' => $id], function (&$reDataArr) use ($request){
    ...
    /**
     * @OA\Get(
     *     path="/api/admin/vod_type/ajax_info",
     *     tags={"大后台-视频点播-课程分类"},
     *     summary="课程分类--详情",
     *     description="根据单个id,查询详情记录......",
     *     operationId="adminQualityControlVodTypeAjax_info",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_vod_type_id_required"),
     *     @OA\Response(response=200,ref="#/components/responses/Response_QualityControl_info_vod_type"),
     *     @OA\Response(response=400,ref="#/components/responses/common_Response_err_400"),
     *     @OA\Response(response=404,ref="#/components/responses/common_Response_err_404"),
     * )
     *     请求主体对象
     *     requestBody={"$ref": "#/components/requestBodies/RequestBody_QualityControl_info_vod_type"}
     */
    /**
     * ajax获得详情数据
     *
    ...
    public function ajax_info(Request $request){
        ...
        $info = CTAPIVodTypeBusiness::getInfoData($request, $this, $id, [], '', []);
...
//        });
    ...
    /**
     * @OA\Post(
     *     path="/api/admin/vod_type/ajax_save",
     *     tags={"大后台-视频点播-课程分类"},
     *     summary="课程分类--新加/修改",
     *     description="根据单个id,新加/修改记录(id>0:修改；id=0:新加)......",
     *     operationId="adminQualityControlVodTypeAjax_save",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_vod_type_id_required"),
...
//        }
                ...
                $resultDatas = CTAPIVodTypeBusiness::replaceById($request, $this, $saveData, $id, $extParams, true);
                return ajaxDataArr(1, $resultDatas, '');
        });
    }
    ...
    /**
     * @OA\Get(
     *     path="/api/admin/vod_type/ajax_alist",
     *     tags={"大后台-视频点播-课程分类"},
     *     summary="课程分类--列表",
     *     description="课程分类--列表......",
     *     operationId="adminQualityControlVodTypeAjax_alist",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_vod_type_id_optional"),
     *     @OA\Response(response=200,ref="#/components/responses/Response_QualityControl_list_vod_type"),
     *     @OA\Response(response=400,ref="#/components/responses/common_Response_err_400"),
     *     @OA\Response(response=404,ref="#/components/responses/common_Response_err_404"),
     * )
     *     请求主体对象
...
//        $this->InitParams($request);
//        return  CTAPIVodTypeBusiness::getList($request, $this, 2 + 4);
        return $this->exeDoPublicFun($request, 4, 4,'', true, '', [], function (&$reDataArr) use ($request){

            $extParams = [
                // 'handleKeyArr' => $handleKeyArr,//一维数组，数数据需要处理的标记，每一个或类处理，根据情况 自定义标记，然后再处理函数中处理数据。
                'relationFormatConfigs'=> CTAPIVodTypeBusiness::getRelationConfigs($request, $this, ['resource_list' => ''], []),
                // 'infoHandleKeyArr' => ['resetPayMethod']
                // 'listHandleKeyArr' => ['initPayMethodText', 'priceIntToFloat']

            ];
            return  CTAPIVodTypeBusiness::getList($request, $this, 2 + 4, [], [], $extParams);
        });
    }
...
//        $this->InitParams($request);
//        $result = CTAPIVodTypeBusiness::getList($request, $this, 1 + 0);
...
//        $this->InitParams($request);
//        CTAPIVodTypeBusiness::getList($request, $this, 1 + 0);
...
//        $this->InitParams($request);
//        CTAPIVodTypeBusiness::importTemplate($request, $this);
...
//    }
    ...
    /**
     * @OA\Post(
     *     path="/api/admin/vod_type/ajax_del",
     *     tags={"大后台-视频点播-课程分类"},
     *     summary="课程分类--删除",
     *     description="根据单个id,删除记录......",
     *     operationId="adminQualityControlVodTypeAjax_del",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_vod_type_id_required"),
    ...
    {
//        $this->InitParams($request);
//        return CTAPIVodTypeBusiness::delAjax($request, $this);
        ...
        return $this->exeDoPublicFun($request, $pageNum, 4,'', true, '', [], function (&$reDataArr) use ($request){
            ...
            return CTAPIVodTypeBusiness::delCustomizeAjax($request,  $this, $organize_id, [], '');
        });
    }
...
//        // 获得一级城市信息一维数组[$k=>$v]
//        $childKV = CTAPIVodTypeBusiness::getCityByPid($request, $this, $parent_id);
//        // $childKV = CTAPIVodTypeBusiness::getChildListKeyVal($request, $this, $parent_id, 1 + 0);
//
...
//        $resultDatas = CTAPIVodTypeBusiness::importByFile($request, $this, $fileName);
//        return ajaxDataArr(1, $resultDatas, '');
///
...
//        $reDataArr['defaultAdminType'] = -1;// 列表页默认状态
        ...
        // 开启状态(1开启2关闭)
        $reDataArr['openStatus'] =  VodType::$openStatusArr;
...
//        $reDataArr['defaultAdminType'] = -1;// 列表页默认状态
        ...
        if ($id > 0) { // 获得详情数据
            ...
            $info = CTAPIVodTypeBusiness::getInfoData($request, $this, $id, [], '', $extParams);
        }
