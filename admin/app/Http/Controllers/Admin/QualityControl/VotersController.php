...
use App\Business\Controller\API\QualityControl\CTAPIVotersBusiness;
...
class VotersController extends BasicController
{
...
//            return view('admin.QualityControl.Voters.index', $reDataArr);
//
//        }, $this->errMethod, $reDataArr, $this->errorView);
        return $this->exeDoPublicFun($request, 1, 1, 'admin.QualityControl.Voters.index', false
...
//            // $reDataArr = $this->reDataArr;
//            $reDataArr = array_merge($reDataArr, $this->reDataArr);
//            $reDataArr['province_kv'] = CTAPIVotersBusiness::getCityByPid($request, $this,  0);
//            $reDataArr['province_kv'] = CTAPIVotersBusiness::getChildListKeyVal($request, $this, 0, 1 + 0, 0);
//            $reDataArr['province_id'] = 0;
//            return view('admin.QualityControl.Voters.select', $reDataArr);
//
//        }, $this->errMethod, $reDataArr, $this->errorView);
...
//            // $reDataArr = $this->reDataArr;
//            $reDataArr = array_merge($reDataArr, $this->reDataArr);
//            return view('admin.QualityControl.Voters.add', $reDataArr);
//
//        }, $this->errMethod, $reDataArr, $this->errorView);

        $pageNum = ($id > 0) ? 64 : 16;
        return $this->exeDoPublicFun($request, $pageNum, 1,'admin.QualityControl.Voters.add', true
    ...
    /**
     * @OA\Get(
     *     path="/api/admin/voters/ajax_info",
     *     tags={"大后台-系统设置-选民"},
     *     summary="选民--详情",
     *     description="根据单个id,查询详情记录......",
     *     operationId="adminQualityControlVotersAjax_info",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_voters_id_required"),
     *     @OA\Response(response=200,ref="#/components/responses/Response_QualityControl_info_voters"),
     *     @OA\Response(response=400,ref="#/components/responses/common_Response_err_400"),
     *     @OA\Response(response=404,ref="#/components/responses/common_Response_err_404"),
     * )
     *     请求主体对象
     *     requestBody={"$ref": "#/components/requestBodies/RequestBody_QualityControl_info_voters"}
     */
    /**
     * ajax获得详情数据
     *
    ...
    public function ajax_info(Request $request){
        ...
        $info = CTAPIVotersBusiness::getInfoData($request, $this, $id, [], '', []);
...
//        });
    ...
    /**
     * @OA\Post(
     *     path="/api/admin/voters/ajax_save",
     *     tags={"大后台-系统设置-选民"},
     *     summary="选民--新加/修改",
     *     description="根据单个id,新加/修改记录(id>0:修改；id=0:新加)......",
     *     operationId="adminQualityControlVotersAjax_save",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_voters_id_required"),
...
//        $this->InitParams($request);
        ...
        return $this->exeDoPublicFun($request, $pageNum, 4,'', true
            , '', [], function (&$reDataArr) use ($request){
                ...
                $group_type = CommonRequest::getInt($request, 'group_type');
                $group_no = CommonRequest::get($request, 'group_no');
                $real_name = CommonRequest::get($request, 'real_name');
                $sex_name = CommonRequest::get($request, 'sex_name');
                $id_number = CommonRequest::get($request, 'id_number');
                $birth_date = CommonRequest::get($request, 'birth_date');
                $relation_name = CommonRequest::get($request, 'relation_name');
                $politics_name = CommonRequest::get($request, 'politics_name');
                $addr = CommonRequest::get($request, 'addr');
                $contact = CommonRequest::get($request, 'contact');
                $remarks = CommonRequest::get($request, 'remarks');


                // 组序号(1：一组；2：二组；4：三组；8：四组；16：五组)
                $groupTypeArr =  Voters::$groupTypeArr;
                if(!in_array($group_type, array_keys($groupTypeArr))) throws('组序号不是有效值');

                $saveData = [
                    'group_type' => $group_type,
                    'group_name' => $groupTypeArr[$group_type] ?? '',
                    'group_no' => $group_no,
                    'real_name' => $real_name,
                    'sex_name' => $sex_name,
                    'id_number' => $id_number,
                ...
                if($birth_date != '' && judgeDate($birth_date) !== false ){
                    $saveData['birth_date'] = $birth_date;
                }
//        if($id <= 0) {// 新加;要加入的特别字段
//            $addNewData = [
...
//        }
                ...
                $resultDatas = CTAPIVotersBusiness::replaceById($request, $this, $saveData, $id, $extParams, true);
                return ajaxDataArr(1, $resultDatas, '');
        });
    }
    ...
    /**
     * @OA\Get(
     *     path="/api/admin/voters/ajax_alist",
     *     tags={"大后台-系统设置-选民"},
     *     summary="选民--列表",
     *     description="选民--列表......",
     *     operationId="adminQualityControlVotersAjax_alist",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_voters_id_optional"),
     *     @OA\Response(response=200,ref="#/components/responses/Response_QualityControl_list_voters"),
     *     @OA\Response(response=400,ref="#/components/responses/common_Response_err_400"),
     *     @OA\Response(response=404,ref="#/components/responses/common_Response_err_404"),
     * )
     *     请求主体对象
...
//        $this->InitParams($request);
//        return  CTAPIVotersBusiness::getList($request, $this, 2 + 4);
...
//        $this->InitParams($request);
//        $result = CTAPIVotersBusiness::getList($request, $this, 1 + 0);
...
//        $this->InitParams($request);
//        CTAPIVotersBusiness::getList($request, $this, 1 + 0);
...
//        $this->InitParams($request);
//        CTAPIVotersBusiness::importTemplate($request, $this);
...
//    }
    ...
    /**
     * @OA\Post(
     *     path="/api/admin/voters/ajax_del",
     *     tags={"大后台-系统设置-选民"},
     *     summary="选民--删除",
     *     description="根据单个id,删除记录......",
     *     operationId="adminQualityControlVotersAjax_del",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_voters_id_required"),
    ...
    {
//        $this->InitParams($request);
//        return CTAPIVotersBusiness::delAjax($request, $this);
...
//        // 获得一级城市信息一维数组[$k=>$v]
//        $childKV = CTAPIVotersBusiness::getCityByPid($request, $this, $parent_id);
//        // $childKV = CTAPIVotersBusiness::getChildListKeyVal($request, $this, $parent_id, 1 + 0);
//
...
//        $resultDatas = CTAPIVotersBusiness::importByFile($request, $this, $fileName);
//        return ajaxDataArr(1, $resultDatas, '');
///
...
//        return ajaxDataArr(1, $resultDatas, '');
        return $this->exeDoPublicFun($request, 32768, 4,'', true, '', [], function (&$reDataArr) use ($request){
            ...
            CTAPIVotersBusiness::mergeRequest($request, $this, [
                'group_type' => 16,// 组序号(1：一组；2：二组；4：三组；8：四组；16：五组)
                // 'group_name' => '',// 组序号名[如：一组..]
            ]);
            ...
            try{
                ...
                if ($resourceId > 0) {
                    CTAPIResourceBusiness::mergeRequest($request, $this, [
                        'id' => $resourceId,
                    ]);
...
//        $reDataArr['defaultAdminType'] = -1;// 列表页默认状态
        ...
        // 组序号(1：一组；2：二组；4：三组；8：四组；16：五组)
        $reDataArr['groupType'] =  Voters::$groupTypeArr;
        $reDataArr['defaultGroupType'] = -1;// 列表页默认状态
...
//        $reDataArr['defaultAdminType'] = -1;// 列表页默认状态
        ...
        // 组序号(1：一组；2：二组；4：三组；8：四组；16：五组)
        $reDataArr['groupType'] =  Voters::$groupTypeArr;
        $reDataArr['defaultGroupType'] = $info['group_type'] ?? -1;// 列表页默认状态
