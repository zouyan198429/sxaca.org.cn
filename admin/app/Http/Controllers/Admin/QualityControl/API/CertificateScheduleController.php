...
//        }, $this->errMethod, $reDataArr, $this->errorView);
     ...
     * @author zouyan(305463219@qq.com)
     */
    public function get_alist_api(Request $request,$id = 0)
    {
        $pageNum = 0;
        return $this->exeDoPublicFun($request, $pageNum, 1,'admin.QualityControl.API.CertificateSchedule.get_alist_api', false
            , 'doInfoPage', ['id' => $id], function (&$reDataArr) use ($request){
...
//        return  CTAPICertificateScheduleBusiness::getList($request, $this, 2 + 4);
        return $this->exeDoPublicFun($request, 4, 4,'', false, '', [], function (&$reDataArr) use ($request){
            ...
            return  CTAPICertificateScheduleBusiness::getList($request, $this, 2 , [], [], $extParams);
        });
    }
     ...
     * @author zouyan(305463219@qq.com)
     */
    public function ajax_alist_api(Request $request){
//        $this->InitParams($request);
//        return  CTAPICertificateScheduleBusiness::getList($request, $this, 2 + 4);
        return $this->exeDoPublicFun($request, 4, 4,'', true, '', [], function (&$reDataArr) use ($request){
            // throws('此接口暂不对外开放，请联系相关业务及领导！');
            $handleKeyConfigArr = ['company_info' => '','certificate_info' => ''];
            $extParams = [
                // 'handleKeyArr' => $handleKeyArr,//一维数组，数数据需要处理的标记，每一个或类处理，根据情况 自定义标记，然后再处理函数中处理数据。
                'relationFormatConfigs'=> CTAPICertificateScheduleBusiness::getRelationConfigs($request, $this, $handleKeyConfigArr, []),
            ];
            $res = CTAPICertificateScheduleBusiness::getList($request, $this, 2, [], [], $extParams);
            $result = $res['result'] ?? [];
            $page = $result['page'] ?? 1;
            $totalPage = $result['totalPage'] ?? 1;
            // 超过 50页 且 超过 60%
            $judgePage = ceil($totalPage * 0.6);// 最多获取页数
            if($totalPage > 50 && $judgePage < 50) $judgePage = 50;
             if($page > 50 && $judgePage < $page){
                 throws('数据已获取完，没有其它数据了！');
                 // throws('此接口暂时只开放60%，请联系相关业务及领导！');
             }else{
                 if($totalPage > 50){
                     $pageMax = $judgePage;
                     $pagesize = $result['pagesize'];
                     $result['totalPage'] = $pageMax;
                     $total = $pageMax * $pagesize;
                     $result['total'] = $total;
                     // $result['page'] =
                     if($page >= $pageMax) $result['has_page'] = false;
                     $res['result'] = $result;
                 }
             }
            return $res;
