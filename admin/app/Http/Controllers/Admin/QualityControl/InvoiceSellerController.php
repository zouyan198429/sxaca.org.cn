...
use App\Business\Controller\API\QualityControl\CTAPIInvoiceSellerBusiness;
...
use App\Models\QualityControl\InvoiceSeller;
use App\Services\Request\CommonRequest;
use App\Services\Tool;
use Illuminate\Http\Request;

class InvoiceSellerController extends BasicController
{
...
//            return view('admin.QualityControl.InvoiceSeller.index', $reDataArr);
//
//        }, $this->errMethod, $reDataArr, $this->errorView);
        return $this->exeDoPublicFun($request, 1, 1, 'admin.QualityControl.InvoiceSeller.index', true
...
//            // $reDataArr = $this->reDataArr;
//            $reDataArr = array_merge($reDataArr, $this->reDataArr);
//            $reDataArr['province_kv'] = CTAPIInvoiceSellerBusiness::getCityByPid($request, $this,  0);
//            $reDataArr['province_kv'] = CTAPIInvoiceSellerBusiness::getChildListKeyVal($request, $this, 0, 1 + 0, 0);
//            $reDataArr['province_id'] = 0;
//            return view('admin.QualityControl.InvoiceSeller.select', $reDataArr);
//
//        }, $this->errMethod, $reDataArr, $this->errorView);
...
//            // $reDataArr = $this->reDataArr;
//            $reDataArr = array_merge($reDataArr, $this->reDataArr);
//            return view('admin.QualityControl.InvoiceSeller.add', $reDataArr);
//
//        }, $this->errMethod, $reDataArr, $this->errorView);

        $pageNum = ($id > 0) ? 64 : 16;
        return $this->exeDoPublicFun($request, $pageNum, 1,'admin.QualityControl.InvoiceSeller.add', true
    ...
    /**
     * @OA\Get(
     *     path="/api/admin/invoice_seller/ajax_info",
     *     tags={"大后台-电子发票管理-发票配置销售方"},
     *     summary="发票配置销售方--详情",
     *     description="根据单个id,查询详情记录......",
     *     operationId="adminQualityControlInvoiceSellerAjax_info",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_invoice_seller_id_required"),
     *     @OA\Response(response=200,ref="#/components/responses/Response_QualityControl_info_invoice_seller"),
     *     @OA\Response(response=400,ref="#/components/responses/common_Response_err_400"),
     *     @OA\Response(response=404,ref="#/components/responses/common_Response_err_404"),
     * )
     *     请求主体对象
     *     requestBody={"$ref": "#/components/requestBodies/RequestBody_QualityControl_info_invoice_seller"}
     */
    /**
     * ajax获得详情数据
     *
    ...
    public function ajax_info(Request $request){
        ...
        $info = CTAPIInvoiceSellerBusiness::getInfoData($request, $this, $id, [], '', []);
...
//        });
    ...
    /**
     * @OA\Post(
     *     path="/api/admin/invoice_seller/ajax_save",
     *     tags={"大后台-电子发票管理-发票配置销售方"},
     *     summary="发票配置销售方--新加/修改",
     *     description="根据单个id,新加/修改记录(id>0:修改；id=0:新加)......",
     *     operationId="adminQualityControlInvoiceSellerAjax_save",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_invoice_seller_id_required"),
...
//        $this->InitParams($request);
        ...
        return $this->exeDoPublicFun($request, $pageNum, 4,'', true
            , '', [], function (&$reDataArr) use ($request){
                ...
                $xsf_mc = CommonRequest::get($request, 'xsf_mc');
                $xsf_dz = CommonRequest::get($request, 'xsf_dz');
                $xsf_dh = CommonRequest::get($request, 'xsf_dh');
                $xsf_nsrsbh = CommonRequest::get($request, 'xsf_nsrsbh');
                $xsf_yh = CommonRequest::get($request, 'xsf_yh');
                $xsf_yhzh = CommonRequest::get($request, 'xsf_yhzh');
                $open_status = 1;// CommonRequest::getInt($request, 'open_status');
...
//        }
                ...
                $resultDatas = CTAPIInvoiceSellerBusiness::replaceById($request, $this, $saveData, $id, $extParams, true);
                return ajaxDataArr(1, $resultDatas, '');
        });
    }
    ...
    /**
     * @OA\Get(
     *     path="/api/admin/invoice_seller/ajax_alist",
     *     tags={"大后台-电子发票管理-发票配置销售方"},
     *     summary="发票配置销售方--列表",
     *     description="发票配置销售方--列表......",
     *     operationId="adminQualityControlInvoiceSellerAjax_alist",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_invoice_seller_id_optional"),
     *     @OA\Response(response=200,ref="#/components/responses/Response_QualityControl_list_invoice_seller"),
     *     @OA\Response(response=400,ref="#/components/responses/common_Response_err_400"),
     *     @OA\Response(response=404,ref="#/components/responses/common_Response_err_404"),
     * )
     *     请求主体对象
...
//        $this->InitParams($request);
//        return  CTAPIInvoiceSellerBusiness::getList($request, $this, 2 + 4);
        return $this->exeDoPublicFun($request, 4, 4,'', true, '', [], function (&$reDataArr) use ($request){

            $extParams = [
                // 'handleKeyArr' => $handleKeyArr,//一维数组，数数据需要处理的标记，每一个或类处理，根据情况 自定义标记，然后再处理函数中处理数据。
                'relationFormatConfigs'=> CTAPIInvoiceSellerBusiness::getRelationConfigs($request, $this,
                    [
                        'pay_company_name' => '',
                    ], []),
                // 'listHandleKeyArr' => ['priceIntToFloat'],
            ];
            return  CTAPIInvoiceSellerBusiness::getList($request, $this, 2 + 4, [], [], $extParams);
        });
    }
...
//        $this->InitParams($request);
//        $result = CTAPIInvoiceSellerBusiness::getList($request, $this, 1 + 0);
...
//        $this->InitParams($request);
//        CTAPIInvoiceSellerBusiness::getList($request, $this, 1 + 0);
...
//                // 'listHandleKeyArr' => ['priceIntToFloat'],
//            ];
//            CTAPIInvoiceSellerBusiness::getList($request, $this, 1 + 0, [], [], $extParams);
//        });
//    }
...
//        $this->InitParams($request);
//        CTAPIInvoiceSellerBusiness::importTemplate($request, $this);
...
//    }
    ...
    /**
     * @OA\Post(
     *     path="/api/admin/invoice_seller/ajax_del",
     *     tags={"大后台-电子发票管理-发票配置销售方"},
     *     summary="发票配置销售方--删除",
     *     description="根据单个id,删除记录......",
     *     operationId="adminQualityControlInvoiceSellerAjax_del",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_invoice_seller_id_required"),
    ...
    {
//        $this->InitParams($request);
//        return CTAPIInvoiceSellerBusiness::delAjax($request, $this);
...
//        // 获得一级城市信息一维数组[$k=>$v]
//        $childKV = CTAPIInvoiceSellerBusiness::getCityByPid($request, $this, $parent_id);
//        // $childKV = CTAPIInvoiceSellerBusiness::getChildListKeyVal($request, $this, $parent_id, 1 + 0);
//
...
//        $resultDatas = CTAPIInvoiceSellerBusiness::importByFile($request, $this, $fileName);
//        return ajaxDataArr(1, $resultDatas, '');
///
...
//        $reDataArr['defaultAdminType'] = -1;// 列表页默认状态
        ...
        // 开启状态(1开启2关闭)
        $reDataArr['openStatus'] =  InvoiceSeller::$openStatusArr;
...
//        $reDataArr['defaultAdminType'] = -1;// 列表页默认状态
        ...
        if($id <= 0 && $pay_config_id > 0){
            // 获得记录
            $dataInfo = CTAPIInvoiceSellerBusiness::getFVFormatList( $request,  $this, 4, 1
        ...
        if ($id > 0) { // 获得详情数据
            $operate = "修改";
            $info = CTAPIInvoiceSellerBusiness::getInfoData($request, $this, $id, [], '', $extParams);
        }
