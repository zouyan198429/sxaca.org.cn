...
use App\Models\QualityControl\OrderPayMethod;
use App\Models\QualityControl\OrderPayConfig;
use App\Services\Request\CommonRequest;
use App\Services\Tool;
use Illuminate\Http\Request;

class OrderPayMethodController extends BasicController
{
...
//            return view('admin.QualityControl.OrderPayMethod.index', $reDataArr);
//
//        }, $this->errMethod, $reDataArr, $this->errorView);
        return $this->exeDoPublicFun($request, 1, 1, 'admin.QualityControl.OrderPayMethod.index', true
...
//            // $reDataArr = $this->reDataArr;
//            $reDataArr = array_merge($reDataArr, $this->reDataArr);
//            $reDataArr['province_kv'] = CTAPIOrderPayMethodBusiness::getCityByPid($request, $this,  0);
//            $reDataArr['province_kv'] = CTAPIOrderPayMethodBusiness::getChildListKeyVal($request, $this, 0, 1 + 0, 0);
//            $reDataArr['province_id'] = 0;
//            return view('admin.QualityControl.OrderPayMethod.select', $reDataArr);
//
//        }, $this->errMethod, $reDataArr, $this->errorView);
...
//            // $reDataArr = $this->reDataArr;
//            $reDataArr = array_merge($reDataArr, $this->reDataArr);
//            return view('admin.QualityControl.OrderPayMethod.add', $reDataArr);
//
//        }, $this->errMethod, $reDataArr, $this->errorView);

        $pageNum = ($id > 0) ? 64 : 16;
        return $this->exeDoPublicFun($request, $pageNum, 1,'admin.QualityControl.OrderPayMethod.add', true
    ...
    /**
     * @OA\Get(
     *     path="/api/admin/order_pay_method/ajax_info",
     *     tags={"大后台-收款设置-收款方式配置"},
     *     summary="收款方式配置--详情",
     *     description="根据单个id,查询详情记录......",
     *     operationId="adminQualityControlOrderPayMethodAjax_info",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_order_pay_method_id_required"),
     *     @OA\Response(response=200,ref="#/components/responses/Response_QualityControl_info_order_pay_method"),
     *     @OA\Response(response=400,ref="#/components/responses/common_Response_err_400"),
     *     @OA\Response(response=404,ref="#/components/responses/common_Response_err_404"),
     * )
     *     请求主体对象
     *     requestBody={"$ref": "#/components/requestBodies/RequestBody_QualityControl_info_order_pay_method"}
     */
    /**
     * ajax获得详情数据
     *
    ...
    public function ajax_info(Request $request){
        ...
        $info = CTAPIOrderPayMethodBusiness::getInfoData($request, $this, $id, [], '', []);
...
//        });
    ...
    /**
     * @OA\Post(
     *     path="/api/admin/order_pay_method/ajax_save",
     *     tags={"大后台-收款设置-收款方式配置"},
     *     summary="收款方式配置--新加/修改",
     *     description="根据单个id,新加/修改记录(id>0:修改；id=0:新加)......",
     *     operationId="adminQualityControlOrderPayMethodAjax_save",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_order_pay_method_id_required"),
...
//        $this->InitParams($request);
        ...
        return $this->exeDoPublicFun($request, $pageNum, 4,'', true
            , '', [], function (&$reDataArr) use ($request){
                ...
                $pay_name = CommonRequest::get($request, 'pay_name');
                $pay_method = CommonRequest::getInt($request, 'pay_method');
                $pay_remarks = CommonRequest::get($request, 'pay_remarks');
                ...
                if(!in_array($pay_method, $methodArr)) throws('支付方式代码值必须为：' . implode(',', $methodArr));
                ...
                $saveData = [
                    ...
                    'pay_remarks' => replace_enter_char($pay_remarks, 1),
...
//        }
                ...
                $resultDatas = CTAPIOrderPayMethodBusiness::replaceById($request, $this, $saveData, $id, $extParams, true);
                return ajaxDataArr(1, $resultDatas, '');
        });
    }
    ...
    /**
     * @OA\Get(
     *     path="/api/admin/order_pay_method/ajax_alist",
     *     tags={"大后台-收款设置-收款方式配置"},
     *     summary="收款方式配置--列表",
     *     description="收款方式配置--列表......",
     *     operationId="adminQualityControlOrderPayMethodAjax_alist",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_order_pay_method_id_optional"),
     *     @OA\Response(response=200,ref="#/components/responses/Response_QualityControl_list_order_pay_method"),
     *     @OA\Response(response=400,ref="#/components/responses/common_Response_err_400"),
     *     @OA\Response(response=404,ref="#/components/responses/common_Response_err_404"),
     * )
     *     请求主体对象
...
//        $this->InitParams($request);
//        return  CTAPIOrderPayMethodBusiness::getList($request, $this, 2 + 4);
        return $this->exeDoPublicFun($request, 4, 4,'', true, '', [], function (&$reDataArr) use ($request){

            $extParams = [
                // 'handleKeyArr' => $handleKeyArr,//一维数组，数数据需要处理的标记，每一个或类处理，根据情况 自定义标记，然后再处理函数中处理数据。
                'relationFormatConfigs'=> CTAPIOrderPayMethodBusiness::getRelationConfigs($request, $this, ['resource_list'], []),

            ];
            return  CTAPIOrderPayMethodBusiness::getList($request, $this, 2 + 4, [], [], $extParams);
        });
    }
...
//        $this->InitParams($request);
//        $result = CTAPIOrderPayMethodBusiness::getList($request, $this, 1 + 0);
...
//        $this->InitParams($request);
//        CTAPIOrderPayMethodBusiness::getList($request, $this, 1 + 0);
...
//        $this->InitParams($request);
//        CTAPIOrderPayMethodBusiness::importTemplate($request, $this);
...
//    }
    ...
    /**
     * @OA\Post(
     *     path="/api/admin/order_pay_method/ajax_del",
     *     tags={"大后台-收款设置-收款方式配置"},
     *     summary="收款方式配置--删除",
     *     description="根据单个id,删除记录......",
     *     operationId="adminQualityControlOrderPayMethodAjax_del",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_order_pay_method_id_required"),
    ...
    {
//        $this->InitParams($request);
//        return CTAPIOrderPayMethodBusiness::delAjax($request, $this);
        ...
        return $this->exeDoPublicFun($request, $pageNum, 4,'', true, '', [], function (&$reDataArr) use ($request){
            ...
            return CTAPIOrderPayMethodBusiness::delCustomizeAjax($request,  $this, $organize_id, [], '');
...
//        // 获得一级城市信息一维数组[$k=>$v]
//        $childKV = CTAPIOrderPayMethodBusiness::getCityByPid($request, $this, $parent_id);
//        // $childKV = CTAPIOrderPayMethodBusiness::getChildListKeyVal($request, $this, $parent_id, 1 + 0);
//
...
//        $resultDatas = CTAPIOrderPayMethodBusiness::importByFile($request, $this, $fileName);
//        return ajaxDataArr(1, $resultDatas, '');
///
...
//        $reDataArr['defaultAdminType'] = -1;// 列表页默认状态
        ...
        // 状态(1正常(报名中)  2下架)
        $reDataArr['statusOnline'] =  OrderPayMethod::$statusOnlineArr;
...
//        $reDataArr['defaultAdminType'] = -1;// 列表页默认状态
        ...
        if ($id > 0) { // 获得详情数据
            ...
            $info = CTAPIOrderPayMethodBusiness::getInfoData($request, $this, $id, [], '', $extParams);
