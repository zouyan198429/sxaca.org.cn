...
use App\Business\Controller\API\QualityControl\CTAPICompanyInspectBusiness;
...
class CompanyInspectController extends BasicController
{
...
//            return view('web.QualityControl.Market.CompanyInspect.index', $reDataArr);
//
//        }, $this->errMethod, $reDataArr, $this->errorView);
        return $this->exeDoPublicFun($request, 1, 1, 'web.QualityControl.Market.CompanyInspect.index', false
...
//            // $reDataArr = $this->reDataArr;
//            $reDataArr = array_merge($reDataArr, $this->reDataArr);
//            $reDataArr['province_kv'] = CTAPICompanyInspectBusiness::getCityByPid($request, $this,  0);
//            $reDataArr['province_kv'] = CTAPICompanyInspectBusiness::getChildListKeyVal($request, $this, 0, 1 + 0, 0);
//            $reDataArr['province_id'] = 0;
//            return view('web.QualityControl.Market.CompanyInspect.select', $reDataArr);
//
//        }, $this->errMethod, $reDataArr, $this->errorView);
...
////            // $reDataArr = $this->reDataArr;
////            $reDataArr = array_merge($reDataArr, $this->reDataArr);
////            return view('web.QualityControl.Market.CompanyInspect.add', $reDataArr);
////
////        }, $this->errMethod, $reDataArr, $this->errorView);
//
//        $pageNum = ($id > 0) ? 64 : 16;
//        return $this->exeDoPublicFun($request, $pageNum, 1,'web.QualityControl.Market.CompanyInspect.add', true
...
//    }
    ...
    {
        return $this->exeDoPublicFun($request, 17179869184, 1,'web.QualityControl.Market.CompanyInspect.info', false
            , 'doInfoPage', ['id' => $id], function (&$reDataArr) use ($request){
    ...
    /**
     * @OA\Get(
     *     path="/api/web/market/company_inspect/ajax_info",
     *     tags={"陕西省市场监督管理局-检验机构信息管理-监督检查"},
     *     summary="监督检查--详情",
     *     description="根据单个id,查询详情记录......",
     *     operationId="webmarketQualityControlCompanyInspectAjax_info",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_company_inspect_id_required"),
     *     @OA\Response(response=200,ref="#/components/responses/Response_QualityControl_info_company_inspect"),
     *     @OA\Response(response=400,ref="#/components/responses/common_Response_err_400"),
     *     @OA\Response(response=404,ref="#/components/responses/common_Response_err_404"),
     * )
     *     请求主体对象
     *     requestBody={"$ref": "#/components/requestBodies/RequestBody_QualityControl_info_company_inspect"}
     */
    /**
     * ajax获得详情数据
     *
    ...
    public function ajax_info(Request $request){
        ...
        $info = CTAPICompanyInspectBusiness::getInfoData($request, $this, $id, [], '', []);
...
//        });
    ...
    /**
     * @OA\Post(
     *     path="/api/web/market/company_inspect/ajax_save",
     *     tags={"陕西省市场监督管理局-检验机构信息管理-监督检查"},
     *     summary="监督检查--新加/修改",
     *     description="根据单个id,新加/修改记录(id>0:修改；id=0:新加)......",
     *     operationId="webmarketQualityControlCompanyInspectAjax_save",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_company_inspect_id_required"),
...
//                $exist_problem = CommonRequest::get($request, 'exist_problem');
//                $breach_clause = CommonRequest::get($request, 'breach_clause');
//                $link_url = CommonRequest::get($request, 'link_url');
//
//                $saveData = [
//                    'company_id' => $company_id,
//                    'exist_problem' => $exist_problem,
//                    'breach_clause' => replace_enter_char($breach_clause, 1),
//                    'link_url' => $link_url,
//                ];
//
//                if($id <= 0) {// 新加;要加入的特别字段
...
//                ];
//                $resultDatas = CTAPICompanyInspectBusiness::replaceById($request, $this, $saveData, $id, $extParams, true);
//                return ajaxDataArr(1, $resultDatas, '');
//        });
//    }
    ...
    /**
     * @OA\Get(
     *     path="/api/web/market/company_inspect/ajax_alist",
     *     tags={"陕西省市场监督管理局-检验机构信息管理-监督检查"},
     *     summary="监督检查--列表",
     *     description="监督检查--列表......",
     *     operationId="webmarketQualityControlCompanyInspectAjax_alist",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_company_inspect_id_optional"),
     *     @OA\Response(response=200,ref="#/components/responses/Response_QualityControl_list_company_inspect"),
     *     @OA\Response(response=400,ref="#/components/responses/common_Response_err_400"),
     *     @OA\Response(response=404,ref="#/components/responses/common_Response_err_404"),
     * )
     *     请求主体对象
...
//        $this->InitParams($request);
//        return  CTAPICompanyInspectBusiness::getList($request, $this, 2 + 4);
        return $this->exeDoPublicFun($request, 4, 4,'', false, '', [], function (&$reDataArr) use ($request){

            $handleKeyConfigArr = ['company_info' => ''];
            $extParams = [
                // 'handleKeyArr' => $handleKeyArr,//一维数组，数数据需要处理的标记，每一个或类处理，根据情况 自定义标记，然后再处理函数中处理数据。
                'relationFormatConfigs'=> CTAPICompanyInspectBusiness::getRelationConfigs($request, $this, $handleKeyConfigArr, []),
            ];
            $result = CTAPICompanyInspectBusiness::getList($request, $this, 2 + 4, [], [], $extParams);
...
//        $this->InitParams($request);
//        $result = CTAPICompanyInspectBusiness::getList($request, $this, 1 + 0);
...
////        $this->InitParams($request);
////        CTAPICompanyInspectBusiness::getList($request, $this, 1 + 0);
...
//                'relationFormatConfigs'=> CTAPICompanyInspectBusiness::getRelationConfigs($request, $this, $handleKeyConfigArr, []),
//            ];
//            CTAPICompanyInspectBusiness::getList($request, $this, 1 + 0, [], [], $extParams);
//        });
//    }
...
//        $this->InitParams($request);
//        CTAPICompanyInspectBusiness::importTemplate($request, $this);
...
//    }
    ...
    /**
     * @OA\Post(
     *     path="/api/web/market/company_inspect/ajax_del",
     *     tags={"陕西省市场监督管理局-检验机构信息管理-监督检查"},
     *     summary="监督检查--删除",
     *     description="根据单个id,删除记录......",
     *     operationId="webmarketQualityControlCompanyInspectAjax_del",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_company_inspect_id_required"),
...
//    {
////        $this->InitParams($request);
////        return CTAPICompanyInspectBusiness::delAjax($request, $this);
//
...
//            return CTAPICompanyInspectBusiness::delCustomizeAjax($request,  $this, $organize_id, [], '');
//            // return CTAPICompanyInspectBusiness::delAjax($request, $this);
//        });
//    }
...
//        // 获得一级城市信息一维数组[$k=>$v]
//        $childKV = CTAPICompanyInspectBusiness::getCityByPid($request, $this, $parent_id);
//        // $childKV = CTAPICompanyInspectBusiness::getChildListKeyVal($request, $this, $parent_id, 1 + 0);
//
...
//        $resultDatas = CTAPICompanyInspectBusiness::importByFile($request, $this, $fileName);
//        return ajaxDataArr(1, $resultDatas, '');
///
...
//        $reDataArr['defaultAdminType'] = -1;// 列表页默认状态
        ...
        if ($id > 0) { // 获得详情数据
            ...
            $extParams = [
                // 'handleKeyArr' => $handleKeyArr,//一维数组，数数据需要处理的标记，每一个或类处理，根据情况 自定义标记，然后再处理函数中处理数据。
                'relationFormatConfigs'=> CTAPICompanyInspectBusiness::getRelationConfigs($request, $this,
                    [
                        'company_info' => '',
                    ], []),
                // 'listHandleKeyArr' => ['priceIntToFloat'],
            ];
            $info = CTAPICompanyInspectBusiness::getInfoData($request, $this, $id, [], '', $extParams);
        }
