...
//            return view('web.QualityControl.Site.Vods.index', $reDataArr);
//
//        }, $this->errMethod, $reDataArr, $this->errorView);
        return $this->exeDoPublicFun($request, 1, 1, 'web.QualityControl.Site.Vods.index', true
...
//            return view('web.QualityControl.Site.Vods.select', $reDataArr);
//
//        }, $this->errMethod, $reDataArr, $this->errorView);
...
//    }
     ...
     * @author zouyan(305463219@qq.com)
     */
    public function vods(Request $request, $vod_type_id = 0, $recommend_status = 0, $pagesize = 20, $page = 1)
    {
        return $this->exeDoPublicFun($request, 0, 8, 'web.QualityControl.Site.Vods.index', false
            , '', [], function (&$reDataArr) use ($request, &$vod_type_id, &$recommend_status, &$pagesize, &$page){

                $keyArr = CTAPIVodsBusiness::getVodsList($request, $this, $reDataArr, 'web/vods/', $vod_type_id, $recommend_status, $pagesize, $page, 1);
                ...
                $reDataArr['defaultVodType'] = $vod_type_id;// $info['vod_type_id'] ?? -1;// 默认

                // 是否推荐1非推荐2推荐
                $reDataArr['recommendStatus'] =  Vods::$recommendStatusArr;
                $reDataArr['defaultRecommendStatus'] = $recommend_status;// $info['recommend_status'] ?? -1;// 列表页默认状态
...
////            // $reDataArr = $this->reDataArr;
////            $reDataArr = array_merge($reDataArr, $this->reDataArr);
////            return view('web.QualityControl.Site.Vods.add', $reDataArr);
////
////        }, $this->errMethod, $reDataArr, $this->errorView);
//
//        $pageNum = ($id > 0) ? 64 : 16;
//        return $this->exeDoPublicFun($request, $pageNum, 1,'web.QualityControl.Site.Vods.add', true
...
//    }
    ...
    {
        return $this->exeDoPublicFun($request, 17179869184, 1,'web.QualityControl.Site.Vods.info', false
            , 'doInfoPage', ['id' => $id], function (&$reDataArr) use ($request){
     ...
     * @author zouyan(305463219@qq.com)
     */
    public function create_order(Request $request,$id = 0)
    {
        return $this->exeDoPublicFun($request, 0, 8,'web.QualityControl.Site.Vods.create_order', false
            , '', ['id' => $id], function (&$reDataArr) use ($request, &$id){

                // 获得当前视频课程的权限
                $vodPowerNo =static::getVodPower( $request,$reDataArr, $id);
                if(($vodPowerNo & 1) == 1){//  1：没有登录
                    return redirect('web/login');
                }
                if($vodPowerNo <= 0){// 已经购买
                    return redirect('web/vods/info/'. $id);
                }

                $extParams = [
                    // 'handleKeyArr' => $handleKeyArr,//一维数组，数数据需要处理的标记，每一个或类处理，根据情况 自定义标记，然后再处理函数中处理数据。
                    'relationFormatConfigs'=> CTAPIVodsBusiness::getRelationConfigs($request, $this, ['resource_list' => '', 'vod_content' => ''
                        , 'type_name' =>'', 'invoice_template_name' => '', 'invoice_project_template_name' => ''
                        , 'vod_video' => ''], []),
                ...
                if(!in_array($open_status, [1])) throws('课程非开启状态，不可访问');
                $reDataArr['info'] = $info;
                $reDataArr['show_name'] = ($this->user_info['admin_type'] == 2) ? $this->user_info['company_name'] : $this->user_info['real_name'];
                // pr($reDataArr);
            });
    }
    ...
    /**
     * @OA\Get(
     *     path="/api/web/vods/ajax_info",
     *     tags={"前台-视频点播-点播课程"},
     *     summary="点播课程--详情",
     *     description="根据单个id,查询详情记录......",
     *     operationId="webSiteQualityControlVodsAjax_info",
     *     deprecated=false,
...
//    }
    ...
    /**
     * @OA\Post(
     *     path="/api/web/vods/ajax_save",
     *     tags={"前台-视频点播-点播课程"},
     *     summary="点播课程--新加/修改",
     *     description="根据单个id,新加/修改记录(id>0:修改；id=0:新加)......",
     *     operationId="webSiteQualityControlVodsAjax_save",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_vods_id_required"),
...
//        $this->InitParams($request);
    ...
    /**
     * @OA\Get(
     *     path="/api/web/vods/ajax_alist",
     *     tags={"前台-视频点播-点播课程"},
     *     summary="点播课程--列表",
     *     description="点播课程--列表......",
     *     operationId="webSiteQualityControlVodsAjax_alist",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
...
//    }
    ...
    /**
     * @OA\Post(
     *     path="/api/web/vods/ajax_del",
     *     tags={"前台-视频点播-点播课程"},
     *     summary="点播课程--删除",
     *     description="根据单个id,删除记录......",
     *     operationId="webSiteQualityControlVodsAjax_del",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_vods_id_required"),
...
//        $reDataArr['defaultAdminType'] = -1;// 列表页默认状态
        ...
        CTAPIVodsBusiness::incDecByQueyCTL($request, $this, '', 'inc',  'hit_num', 1, [], $incQueryParams, 1, []);
        ...
        // 获得课程目录及课件
        $vod_video = $info['vod_video'] ?? [];
        $levelDataList = Tool::getFormatLevelList($vod_video, '', '', 0, 0, 'parent_video_id', 1, 'id', 'ids', 'level_no', 'video_name', '&nbsp;&nbsp;&nbsp;&nbsp;|', '__');
        $info['vod_video'] = $levelDataList;
        ...
        // 获得推荐课程
        $recommend_list = CTAPIVodsBusiness::getFVFormatList( $request,  $this, 2, 10
            ,  ['recommend_status' => 2, 'open_status' => 1], false,[]
            , array_merge([
                'sqlParams' => [
                   // 'where' => [['is_perfect', 2], ['open_status', 2], ['account_status', 1]],
                    'orderBy' => CTAPIVodsBusiness::$orderBy
                ]
            ], $extParams));
        $reDataArr['recommend_list'] = $recommend_list;
        // pr($reDataArr);
    }
    // **************公用方法********************结束*********************************
