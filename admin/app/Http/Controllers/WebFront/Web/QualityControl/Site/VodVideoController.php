...
//            return view('web.QualityControl.Site.VodVideo.index', $reDataArr);
//
//        }, $this->errMethod, $reDataArr, $this->errorView);
        return $this->exeDoPublicFun($request, 1, 1, 'web.QualityControl.Site.VodVideo.index', true
...
//    {
//        return $this->exeDoPublicFun($request, 0, 8,'web.QualityControl.Site.VodVideo.test', false
...
//            return view('web.QualityControl.Site.VodVideo.select', $reDataArr);
//
//        }, $this->errMethod, $reDataArr, $this->errorView);
//        return $this->exeDoPublicFun($request, 2048, 1, 'web.QualityControl.Site.RrrDddd.select', true
//            , 'doListPage', [], function (&$reDataArr) use ($request){
//
//            });
//    }
     ...
     * @author zouyan(305463219@qq.com)
     */
    public function videos(Request $request, $vod_id = 0, $video_type = 0, $pagesize = 20, $page = 1)
    {
        return $this->exeDoPublicFun($request, 0, 8, 'web.QualityControl.Site.VodVideo.index', false
            , '', [], function (&$reDataArr) use ($request, &$vod_id, &$video_type, &$pagesize, &$page){

                $keyArr = CTAPIVodVideoBusiness::getVodsList($request, $this, $reDataArr, 'web/vod_video/', $vod_id, $video_type, $pagesize, $page, 1);
                $reDataArr['key_str'] = implode(',', $keyArr);

                // 视频类型1上传视频；2网络视频地址
                $reDataArr['videoType'] =  VodVideo::$videoTypeArr;
                $reDataArr['defaultVideoType'] = $video_type;// $info['recommend_status'] ?? -1;// 列表页默认状态
...
//    {
//        $pageNum = ($id > 0) ? 64 : 16;
//        return $this->exeDoPublicFun($request, $pageNum, 1,'web.QualityControl.Site.VodVideo.addDir', true
...
////            // $reDataArr = $this->reDataArr;
////            $reDataArr = array_merge($reDataArr, $this->reDataArr);
////            return view('web.QualityControl.Site.VodVideo.add', $reDataArr);
////
////        }, $this->errMethod, $reDataArr, $this->errorView);
//
//        $pageNum = ($id > 0) ? 64 : 16;
//        return $this->exeDoPublicFun($request, $pageNum, 1,'web.QualityControl.Site.VodVideo.add', true
//            , 'doInfoPage', ['id' => $id], function (&$reDataArr) use ($request){
//
//        });
//    }
    ...
    {
        return $this->exeDoPublicFun($request, 17179869184, 1,'web.QualityControl.Site.VodVideo.info', false
            , 'doInfoPage', ['id' => $id], function (&$reDataArr) use ($request){
    ...
    /**
     * @OA\Get(
     *     path="/api/web/vod_video/ajax_info",
     *     tags={"前台-视频点播-点播课程视频目录"},
     *     summary="点播课程视频目录--详情",
     *     description="根据单个id,查询详情记录......",
     *     operationId="webSiteQualityControlVodVideoAjax_info",
     *     deprecated=false,
...
//    }
    ...
    /**
     * @OA\Post(
     *     path="/api/web/vod_video/ajax_save",
     *     tags={"前台-视频点播-点播课程视频目录"},
     *     summary="点播课程视频目录--新加/修改",
     *     description="根据单个id,新加/修改记录(id>0:修改；id=0:新加)......",
     *     operationId="webSiteQualityControlVodVideoAjax_save",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_vod_video_id_required"),
...
//    }
    ...
    /**
     * @OA\Get(
     *     path="/api/web/vod_video/ajax_alist",
     *     tags={"前台-视频点播-点播课程视频目录"},
     *     summary="点播课程视频目录--列表",
     *     description="点播课程视频目录--列表......",
     *     operationId="webSiteQualityControlVodVideoAjax_alist",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
...
//    {
//        return $this->exeDoPublicFun($request, 34359738368, 8, 'web.QualityControl.Site.SmsTemplate.sms_send', true
//            , '', [], function (&$reDataArr) use ($request){
...
//    }
    ...
    /**
     * @OA\Post(
     *     path="/api/web/vod_video/ajax_del",
     *     tags={"前台-视频点播-点播课程视频目录"},
     *     summary="点播课程视频目录--删除",
     *     description="根据单个id,删除记录......",
     *     operationId="webSiteQualityControlVodVideoAjax_del",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_vod_video_id_required"),
...
//        $reDataArr['defaultAdminType'] = -1;// 列表页默认状态
        ...
        $reDataArr['vod_kv'] = CTAPIVodsBusiness::getListKV($request, $this, ['key' => 'id', 'val' => 'vod_name'], [
            'sqlParams' => ['where' => [['open_status', 1]], 'orderBy' => ['recommend_status' => 'desc', 'id' => 'desc']]
        ]);
    ...
    public function doInfoPage(Request $request, &$reDataArr, $extendParams = []){
        // text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8
         ...
         // 因为ckplay播放器，居然会再自己请求一次，所以这里处理一下--拦截一下
        if(strpos($accept, 'text/html,application/xhtml+xml') === false){
            return false;
        }
...
//        $reDataArr['defaultAdminType'] = -1;// 列表页默认状态
        ...
        $status_online = $info['status_online'] ?? 0;
        if(!in_array($status_online, [1])) throws('课件非开启状态，不可访问');
        // 获得课程信息
        $tem_vod_id = $info['vod_id'] ?? 0;
        // 获得当前视频课程的权限
        $vodPowerNo =static::getVodPower( $request,$reDataArr, $tem_vod_id);
        if(($vodPowerNo & 1) == 1){//  1：没有登录
            return redirect('web/login');
        }
        if(($vodPowerNo & 2) == 2){// 2：没有购买
            return redirect('web/vods/create_order/'. $tem_vod_id);
        }
        $courseInfo = CTAPIVodsBusiness::getInfoData($request, $this, $tem_vod_id, [], '', []);
        if(empty($courseInfo))  throws('课程记录不存在！');
        $course_open_status = $courseInfo['open_status'] ?? 0;
        if(!in_array($course_open_status, [1])) throws('课程非开启状态，不可访问');
        // 浏览数自增
        $incQueryParams = Tool::getParamQuery(['id' => $id]);
        CTAPIVodVideoBusiness::incDecByQueyCTL($request, $this, '', 'inc',  'hit_num', 1, [], $incQueryParams, 1, []);
        // $incQueryParams = Tool::getParamQuery(['id' => $id]);
        CTAPIVodVideoBusiness::incDecByQueyCTL($request, $this, '', 'inc',  'play_num', 1, [], $incQueryParams, 1, []);

        // $reDataArr = array_merge($reDataArr, $resultDatas);
        $reDataArr['info'] = $info;
        $reDataArr['operate'] = $operate;
        $video_type = $info['video_type'] ?? 0;// 视频类型1上传视频；2网络视频地址
        if($video_type == 2){
            $video_url = $info['video_url'] ?? '';
        }else{
            $video_url = $info["resource_list_video"][0]["resource_url"] ?? "" ;
        }
        $reDataArr['video_url'] = $video_url;
