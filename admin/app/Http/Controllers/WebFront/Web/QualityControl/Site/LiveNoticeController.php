...
//            return view('web.QualityControl.Site.LiveNotice.index', $reDataArr);
//
//        }, $this->errMethod, $reDataArr, $this->errorView);
        return $this->exeDoPublicFun($request, 1, 1, 'web.QualityControl.Site.LiveNotice.index', true
...
//            return view('web.QualityControl.Site.LiveNotice.select', $reDataArr);
//
//        }, $this->errMethod, $reDataArr, $this->errorView);
...
//    }
     ...
     * @author zouyan(305463219@qq.com)
     */
    public function list(Request $request, $pagesize = 20, $page = 1)
    {
        return $this->exeDoPublicFun($request, 0, 8, 'web.QualityControl.Site.LiveNotice.index', false
            , '', [], function (&$reDataArr) use ($request, &$vod_type_id, &$recommend_status, &$pagesize, &$page){

                $keyArr = CTAPILiveNoticeBusiness::getWebDataList($request, $this, $reDataArr, 'web/live_notice/', $pagesize, $page, 1);
                $reDataArr['key_str'] = implode(',', $keyArr);

                // 获得最新的
                $newest_list = CTAPILiveNoticeBusiness::getFVFormatList( $request,  $this, 2, 10
                    ,  ['open_status' => 1], false,[]
                    , array_merge([
                        'sqlParams' => [
                            // 'where' => [['is_perfect', 2], ['open_status', 2], ['account_status', 1]],
                            'orderBy' => CTAPILiveNoticeBusiness::$orderBy
                        ]
                    ], []));
                $reDataArr['newest_list'] = $newest_list;
...
//            // $reDataArr = $this->reDataArr;
//            $reDataArr = array_merge($reDataArr, $this->reDataArr);
//            return view('web.QualityControl.Site.LiveNotice.add', $reDataArr);
//
//        }, $this->errMethod, $reDataArr, $this->errorView);

        $pageNum = ($id > 0) ? 64 : 16;
        return $this->exeDoPublicFun($request, $pageNum, 1,'web.QualityControl.Site.LiveNotice.add', true
    ...
    {
        return $this->exeDoPublicFun($request, 17179869184, 1,'web.QualityControl.Site.LiveNotice.info', false
            , 'doInfoPage', ['id' => $id], function (&$reDataArr) use ($request){
    ...
    /**
     * @OA\Get(
     *     path="/api/web/live_notice/ajax_info",
     *     tags={"前台-直播管理-直播公告"},
     *     summary="直播公告--详情",
     *     description="根据单个id,查询详情记录......",
     *     operationId="webQualityControlLiveNoticeAjax_info",
     *     deprecated=false,
...
//        });
    ...
    /**
     * @OA\Post(
     *     path="/api/web/live_notice/ajax_save",
     *     tags={"前台-直播管理-直播公告"},
     *     summary="直播公告--新加/修改",
     *     description="根据单个id,新加/修改记录(id>0:修改；id=0:新加)......",
     *     operationId="webQualityControlLiveNoticeAjax_save",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_live_notice_id_required"),
...
//        }
    ...
    /**
     * @OA\Get(
     *     path="/api/web/live_notice/ajax_alist",
     *     tags={"前台-直播管理-直播公告"},
     *     summary="直播公告--列表",
     *     description="直播公告--列表......",
     *     operationId="webQualityControlLiveNoticeAjax_alist",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
...
//    }
    ...
    /**
     * @OA\Post(
     *     path="/api/web/live_notice/ajax_del",
     *     tags={"前台-直播管理-直播公告"},
     *     summary="直播公告--删除",
     *     description="根据单个id,删除记录......",
     *     operationId="webQualityControlLiveNoticeAjax_del",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_live_notice_id_required"),
...
//        $reDataArr['defaultAdminType'] = -1;// 列表页默认状态
        ...
        if(!in_array($open_status, [1])) throws('非开启状态，不可访问');
