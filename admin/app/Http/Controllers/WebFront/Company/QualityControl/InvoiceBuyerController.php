...
use App\Models\QualityControl\InvoiceBuyer;
use App\Services\Request\CommonRequest;
use App\Services\Tool;
use Illuminate\Http\Request;

class InvoiceBuyerController extends BasicController
{
...
//            return view('company.QualityControl.InvoiceBuyer.index', $reDataArr);
//
//        }, $this->errMethod, $reDataArr, $this->errorView);
        return $this->exeDoPublicFun($request, 1, 1, 'company.QualityControl.InvoiceBuyer.index', true
...
//            // $reDataArr = $this->reDataArr;
//            $reDataArr = array_merge($reDataArr, $this->reDataArr);
//            $reDataArr['province_kv'] = CTAPIInvoiceBuyerBusiness::getCityByPid($request, $this,  0);
//            $reDataArr['province_kv'] = CTAPIInvoiceBuyerBusiness::getChildListKeyVal($request, $this, 0, 1 + 0, 0);
//            $reDataArr['province_id'] = 0;
//            return view('company.QualityControl.InvoiceBuyer.select', $reDataArr);
//
//        }, $this->errMethod, $reDataArr, $this->errorView);
...
//            // $reDataArr = $this->reDataArr;
//            $reDataArr = array_merge($reDataArr, $this->reDataArr);
//            return view('company.QualityControl.InvoiceBuyer.add', $reDataArr);
//
//        }, $this->errMethod, $reDataArr, $this->errorView);

        $pageNum = ($id > 0) ? 64 : 16;
        return $this->exeDoPublicFun($request, $pageNum, 1,'company.QualityControl.InvoiceBuyer.add', true
    ...
    {
        return $this->exeDoPublicFun($request, 17179869184, 1,'company.QualityControl.InvoiceBuyer.info', true
            , 'doInfoPage', ['id' => $id], function (&$reDataArr) use ($request){
    ...
    /**
     * @OA\Get(
     *     path="/api/company/invoice_buyer/ajax_info",
     *     tags={"大后台-电子发票管理-发票配置购买方"},
     *     summary="发票配置购买方--详情",
     *     description="根据单个id,查询详情记录......",
     *     operationId="companyQualityControlInvoiceBuyerAjax_info",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_invoice_buyer_id_required"),
     *     @OA\Response(response=200,ref="#/components/responses/Response_QualityControl_info_invoice_buyer"),
     *     @OA\Response(response=400,ref="#/components/responses/common_Response_err_400"),
     *     @OA\Response(response=404,ref="#/components/responses/common_Response_err_404"),
     * )
     *     请求主体对象
     *     requestBody={"$ref": "#/components/requestBodies/RequestBody_QualityControl_info_invoice_buyer"}
     */
    /**
     * ajax获得详情数据
     *
...
//        $info = CTAPIInvoiceBuyerBusiness::getInfoData($request, $this, $id, [], '', []);
...
//        return ajaxDataArr(1, $resultDatas, '');
    ...
    /**
     * @OA\Post(
     *     path="/api/company/invoice_buyer/ajax_save",
     *     tags={"大后台-电子发票管理-发票配置购买方"},
     *     summary="发票配置购买方--新加/修改",
     *     description="根据单个id,新加/修改记录(id>0:修改；id=0:新加)......",
     *     operationId="companyQualityControlInvoiceBuyerAjax_save",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_invoice_buyer_id_required"),
...
//        $this->InitParams($request);
        ...
        return $this->exeDoPublicFun($request, $pageNum, 4,'', true
            , '', [], function (&$reDataArr) use ($request){
                ...
                $gmf_mc = CommonRequest::get($request, 'gmf_mc');
                $gmf_nsrsbh = CommonRequest::get($request, 'gmf_nsrsbh');
                $gmf_dz = CommonRequest::get($request, 'gmf_dz');
                $gmf_dh = CommonRequest::get($request, 'gmf_dh');
                $gmf_yh = CommonRequest::get($request, 'gmf_yh');
                $gmf_yhzh = CommonRequest::get($request, 'gmf_yhzh');
                $jff_phone = CommonRequest::get($request, 'jff_phone');
                $jff_email = CommonRequest::get($request, 'jff_email');
...
//                    $saveData = array_merge($saveData, $addNewData);
                ...
                $resultDatas = CTAPIInvoiceBuyerBusiness::replaceById($request, $this, $saveData, $id, $extParams, true);
                return ajaxDataArr(1, $resultDatas, '');
        });
    }
    ...
    /**
     * @OA\Get(
     *     path="/api/company/invoice_buyer/ajax_alist",
     *     tags={"大后台-电子发票管理-发票配置购买方"},
     *     summary="发票配置购买方--列表",
     *     description="发票配置购买方--列表......",
     *     operationId="companyQualityControlInvoiceBuyerAjax_alist",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_invoice_buyer_id_optional"),
     *     @OA\Response(response=200,ref="#/components/responses/Response_QualityControl_list_invoice_buyer"),
     *     @OA\Response(response=400,ref="#/components/responses/common_Response_err_400"),
     *     @OA\Response(response=404,ref="#/components/responses/common_Response_err_404"),
     * )
     *     请求主体对象
...
//        $this->InitParams($request);
//        return  CTAPIInvoiceBuyerBusiness::getList($request, $this, 2 + 4);
        return $this->exeDoPublicFun($request, 4, 4,'', true, '', [], function (&$reDataArr) use ($request){
            ...
            CTAPIInvoiceBuyerBusiness::mergeRequest($request, $this, $mergeParams);
            $extParams = [
                // 'handleKeyArr' => $handleKeyArr,//一维数组，数数据需要处理的标记，每一个或类处理，根据情况 自定义标记，然后再处理函数中处理数据。
                'relationFormatConfigs'=> CTAPIInvoiceBuyerBusiness::getRelationConfigs($request, $this,
                    [
                        'company_info' => '',
                    ], []),
                // 'listHandleKeyArr' => ['priceIntToFloat'],
            ];
            return  CTAPIInvoiceBuyerBusiness::getList($request, $this, 2 + 4, [], [], $extParams);
        });
    }
...
//        $this->InitParams($request);
//        $result = CTAPIInvoiceBuyerBusiness::getList($request, $this, 1 + 0);
...
//        $this->InitParams($request);
//        CTAPIInvoiceBuyerBusiness::getList($request, $this, 1 + 0);
...
//                // 'listHandleKeyArr' => ['priceIntToFloat'],
//            ];
//            CTAPIInvoiceBuyerBusiness::getList($request, $this, 1 + 0, [], [], $extParams);
//        });
//    }
...
//        $this->InitParams($request);
//        CTAPIInvoiceBuyerBusiness::importTemplate($request, $this);
...
//    }
    ...
    /**
     * @OA\Post(
     *     path="/api/company/invoice_buyer/ajax_del",
     *     tags={"大后台-电子发票管理-发票配置购买方"},
     *     summary="发票配置购买方--删除",
     *     description="根据单个id,删除记录......",
     *     operationId="companyQualityControlInvoiceBuyerAjax_del",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_invoice_buyer_id_required"),
    ...
    {
//        $this->InitParams($request);
//        return CTAPIInvoiceBuyerBusiness::delAjax($request, $this);
        ...
        return $this->exeDoPublicFun($request, $pageNum, 4,'', true, '', [], function (&$reDataArr) use ($request){
            ...
            return CTAPIInvoiceBuyerBusiness::delCustomizeAjax($request,  $this, $organize_id, [], '');
            // return CTAPIInvoiceBuyerBusiness::delAjax($request, $this);
        });
    }
...
//        // 获得一级城市信息一维数组[$k=>$v]
//        $childKV = CTAPIInvoiceBuyerBusiness::getCityByPid($request, $this, $parent_id);
//        // $childKV = CTAPIInvoiceBuyerBusiness::getChildListKeyVal($request, $this, $parent_id, 1 + 0);
//
...
//        $resultDatas = CTAPIInvoiceBuyerBusiness::importByFile($request, $this, $fileName);
//        return ajaxDataArr(1, $resultDatas, '');
///
...
//        $reDataArr['defaultAdminType'] = -1;// 列表页默认状态
        ...
        // 开启状态(1开启2关闭)
        $reDataArr['openStatus'] =  InvoiceBuyer::$openStatusArr;
...
//        $reDataArr['defaultAdminType'] = -1;// 列表页默认状态
        ...
        if ($id > 0) { // 获得详情数据
            ...
            $info = CTAPIInvoiceBuyerBusiness::getInfoData($request, $this, $id, [], '', $extParams);
