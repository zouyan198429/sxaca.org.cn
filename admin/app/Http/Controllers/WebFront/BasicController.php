...
//        $userInfo = CTAPIStaffBusiness::getInfoDataBase(\request(), $this,'', $staff_id, [], '', 1);
        ...
        if($errDo &&  ($userInfo['admin_type'] & $this->user_type) <= 0 ){
            $this->delUserInfo();
            throws('非法访问！');
        }

        if($errDo && $userInfo['is_perfect'] == 1 ) {
            if($userInfo['admin_type'] == 2){
                // 待补充资料
                if(!isAjax()) return redirect('web/perfect_company');
                throws('待补充企业资料');
            }else{
                // 待补充资料
                if(!isAjax()) return redirect('web/perfect_user');
                throws('待补充用户资料');
            }
        }
        ...
        if($errDo && $userInfo['open_status'] == 1  && $userInfo['is_perfect'] == 2){
            $this->delUserInfo();
            throws('审核中，请耐心等待！！');
        }
        ...
        if($errDo && $userInfo['open_status'] != 2 && $userInfo['is_perfect'] == 2 ){
            $this->delUserInfo();
            throws('非审核通过！！');
        }
        return $userInfo;
