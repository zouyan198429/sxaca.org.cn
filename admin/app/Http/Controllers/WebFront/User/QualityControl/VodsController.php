...
//            return view('user.QualityControl.Vods.index', $reDataArr);
//
//        }, $this->errMethod, $reDataArr, $this->errorView);
        return $this->exeDoPublicFun($request, 1, 1, 'user.QualityControl.Vods.index', true
...
//            return view('user.QualityControl.Vods.select', $reDataArr);
//
//        }, $this->errMethod, $reDataArr, $this->errorView);
...
////            // $reDataArr = $this->reDataArr;
////            $reDataArr = array_merge($reDataArr, $this->reDataArr);
////            return view('user.QualityControl.Vods.add', $reDataArr);
////
////        }, $this->errMethod, $reDataArr, $this->errorView);
//
//        $pageNum = ($id > 0) ? 64 : 16;
//        return $this->exeDoPublicFun($request, $pageNum, 1,'user.QualityControl.Vods.add', true
...
//    }
    ...
    {
        return $this->exeDoPublicFun($request, 17179869184, 1,'user.QualityControl.Vods.info', true
            , 'doInfoPage', ['id' => $id], function (&$reDataArr) use ($request){
    ...
    /**
     * @OA\Get(
     *     path="/api/user/vods/ajax_info",
     *     tags={"用户中心-视频点播-点播课程"},
     *     summary="点播课程--详情",
     *     description="根据单个id,查询详情记录......",
     *     operationId="userQualityControlVodsAjax_info",
     *     deprecated=false,
...
//        });
    ...
    /**
     * @OA\Post(
     *     path="/api/user/vods/ajax_save",
     *     tags={"用户中心-视频点播-点播课程"},
     *     summary="点播课程--新加/修改",
     *     description="根据单个id,新加/修改记录(id>0:修改；id=0:新加)......",
     *     operationId="userQualityControlVodsAjax_save",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_vods_id_required"),
...
//        $this->InitParams($request);
    ...
    /**
     * @OA\Get(
     *     path="/api/user/vods/ajax_alist",
     *     tags={"用户中心-视频点播-点播课程"},
     *     summary="点播课程--列表",
     *     description="点播课程--列表......",
     *     operationId="userQualityControlVodsAjax_alist",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
...
//        return  CTAPIVodsBusiness::getList($request, $this, 2 + 4);
        return $this->exeDoPublicFun($request, 4, 4,'', true, '', [], function (&$reDataArr) use ($request){
            ...
            // 设置参数
            $mergeParams = [
                'open_status' => 1,// 开启状态1已启用2已关闭
            ];
            CTAPIVodsBusiness::mergeRequest($request, $this, $mergeParams);

            $selCompanyIds = [$this->user_id];
            $selCompanyId = $this->user_info['company_id'] ?? 0;
            if(is_numeric($selCompanyId) && $selCompanyId > 0) array_push($selCompanyIds, $selCompanyId);
            $extendParams = [
                'vod_orders' => [
                    'fieldValParams' => [
                        'company_status' => 1,
                        'company_id'=> $selCompanyIds,
                        'pay_status' => [1,2,4,8],
                    ],
//                    'sqlParams' => [
//                        'where' => [
...
//                    ]
            ...
            $extParams = [
                ...
                'relationFormatConfigs'=> CTAPIVodsBusiness::getRelationConfigs($request, $this, ['resource_list' => ''
                    , 'vod_content' => '', 'type_name' =>'', 'invoice_template_name' => ''
                    , 'invoice_project_template_name' => '', 'vod_orders' => ''], $extendParams),
...
//    }
    ...
    /**
     * @OA\Post(
     *     path="/api/user/vods/ajax_del",
     *     tags={"用户中心-视频点播-点播课程"},
     *     summary="点播课程--删除",
     *     description="根据单个id,删除记录......",
     *     operationId="userQualityControlVodsAjax_del",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_vods_id_required"),
