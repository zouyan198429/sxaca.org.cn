...
////            $this->delUserInfo();
////            return redirect(config('public.webSite.reg.domain') .'web/perfect_user');
////        }
//            return view('user.QualityControl.index', $reDataArr);
//
//        }, $this->errMethod, $reDataArr, $this->errorView);
        return $this->exeDoPublicFun($request, 1, 1, 'user.QualityControl.index', true
...
//    }
    ...
    public function login(Request $request)
    {
        ...
        return Tool::doViewPages($this, $request, function (&$reDataArr) use($request){
            ...
            // $reDataArr = $this->reDataArr;
            $reDataArr = array_merge($reDataArr, $this->reDataArr);
            return view('user.QualityControl.login', $reDataArr);
    ...
    public function password(Request $request)
    {
        ...
        return Tool::doViewPages($this, $request, function (&$reDataArr) use($request){
            ...
            return view('user.QualityControl.admin.password', $reDataArr);
    ...
    public function info(Request $request)
    {
        ...
        return Tool::doViewPages($this, $request, function (&$reDataArr) use($request){
            ...
            $reDataArr = array_merge($reDataArr, $user_info);
            return view('user.QualityControl.admin.info', $reDataArr);
...
//            });
    ...
    /**
     * @OA\Post(
     *     path="/api/user/ajax_login",
     *     tags={"用户中心-帐号注册登录"},
     *     summary="帐号密码登录",
     *     description="通过帐号、密码、图形验证码进行登录",
     *     operationId="userIndexAjax_login",
     *     deprecated=false,
    ...
    public function ajax_login_sms(Request $request)
    {
        // $this->InitParams($request);
        // $company_id = $this->company_id;
        return CTAPIStaffBusiness::loginCaptchaCode($request, $this,4, 2, 2);
    }
...
//        $res = DownFile::downFilePath(2, $publicPath . $qrcode_url_old, 1024, $save_file_name, 1024, $save_file_name);
