...
//            return view('user.QualityControl.PaymentRecord.index', $reDataArr);
//
//        }, $this->errMethod, $reDataArr, $this->errorView);
        return $this->exeDoPublicFun($request, 1, 1, 'user.QualityControl.PaymentRecord.index', true
...
//            return view('user.QualityControl.PaymentRecord.select', $reDataArr);
//
//        }, $this->errMethod, $reDataArr, $this->errorView);
...
//            // $reDataArr = $this->reDataArr;
//            $reDataArr = array_merge($reDataArr, $this->reDataArr);
//            return view('user.QualityControl.PaymentRecord.add', $reDataArr);
//
//        }, $this->errMethod, $reDataArr, $this->errorView);

        $pageNum = ($id > 0) ? 64 : 16;
        return $this->exeDoPublicFun($request, $pageNum, 1,'user.QualityControl.PaymentRecord.add', true
    ...
    {
        return $this->exeDoPublicFun($request, 17179869184, 1,'user.QualityControl.PaymentRecord.info', true
            , 'doInfoPage', ['id' => $id], function (&$reDataArr) use ($request){
    ...
    {
        return $this->exeDoPublicFun($request, 0, 8, 'user.QualityControl.PaymentRecord.pay', true
...
//                $class_id = CommonRequest::getInt($request, 'class_id');
                $pay_show_id = $this->personal_id;// CommonRequest::getInt($request, 'pay_show_id');// 报名记录的企业或用户id
    ...
    {
        return $this->exeDoPublicFun($request, 0, 8, 'user.QualityControl.PaymentRecord.pay_save', true
            , '', [], function (&$reDataArr) use ($request){
...
//                $course_id = CommonRequest::getInt($request, 'course_id');
//                $class_id = CommonRequest::getInt($request, 'class_id');
                $pay_show_id = $this->personal_id;// CommonRequest::getInt($request, 'pay_show_id');// 报名用户所属的企业id
...
//                $course_id = CommonRequest::getInt($request, 'course_id');
//                $class_id = CommonRequest::getInt($request, 'class_id');
                $pay_show_id = $this->personal_id;// CommonRequest::getInt($request, 'pay_show_id');// 报名用户id或企业id
                $pay_config_id = CommonRequest::getInt($request, 'pay_config_id');// 支付配置id
                $pay_method = CommonRequest::getInt($request, 'pay_method');// 选择的缴费方式
    ...
    /**
     * @OA\Get(
     *     path="/api/user/payment_record/ajax_info",
     *     tags={"用户中心-付款/收款相关的-付款/收款记录"},
     *     summary="付款/收款记录--详情",
     *     description="根据单个id,查询详情记录......",
     *     operationId="userQualityControlPaymentRecordAjax_info",
     *     deprecated=false,
...
//        });
    ...
    /**
     * @OA\Post(
     *     path="/api/user/payment_record/ajax_save",
     *     tags={"用户中心-付款/收款相关的-付款/收款记录"},
     *     summary="付款/收款记录--新加/修改",
     *     description="根据单个id,新加/修改记录(id>0:修改；id=0:新加)......",
     *     operationId="userQualityControlPaymentRecordAjax_save",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_payment_record_id_required"),
...
//    }
    ...
    /**
     * @OA\Get(
     *     path="/api/user/payment_record/ajax_alist",
     *     tags={"用户中心-付款/收款相关的-付款/收款记录"},
     *     summary="付款/收款记录--列表",
     *     description="付款/收款记录--列表......",
     *     operationId="userQualityControlPaymentRecordAjax_alist",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
...
//        return  CTAPIPaymentRecordBusiness::getList($request, $this, 2 + 4);
        return $this->exeDoPublicFun($request, 4, 4,'', true, '', [], function (&$reDataArr) use ($request){
            ...
            CTAPIPaymentRecordBusiness::mergeRequest($request, $this, $mergeParams);
            $extParams = [
...
//    }
    ...
    /**
     * @OA\Post(
     *     path="/api/user/payment_record/ajax_del",
     *     tags={"用户中心-付款/收款相关的-付款/收款记录"},
     *     summary="付款/收款记录--删除",
     *     description="根据单个id,删除记录......",
     *     operationId="userQualityControlPaymentRecordAjax_del",
     *     deprecated=false,
     *     @OA\Parameter(ref="#/components/parameters/Accept"),
     *     @OA\Parameter(ref="#/components/parameters/Schema_QualityControl_payment_record_id_required"),
