...
class resource_type_self extends BaseDBVerify
{
    public static $model_name = 'QualityControl\ResourceTypeSelf';// 相对于Models的数据模型名称;在子类中定义，使用时用static::,不用self::
    public static $table_name = 'resource_type_self';// 数据表名称
    // 需要从父的去掉的字段  -- 一维数组
        ...
        $tableConfig = [
            'fields' => [
                ...
                'type_name' => [
                    ...
                    'valiDateParam' => [// 参数有值时验证信息；如需要多个验证--此值为二维数组【注意：此时的多语言resources\lang\zh-CN 下具体的表文件中的message也有用一维数组】
                        // "input"=>$_POST["market_id"],"require"=>"false","var_name" => "queue" ,
                        "validator"=>"length","min"=>"0","max"=>"50","message" => '',// '{fieldName}' . ($langModel['valueLenIs'] ?? '') . ' 0~ 50 ' . ($langModel['numsCharacters'] ?? '') . '!'
                    ],
                    'powerNum' => 0,// (2 | 4)  特殊权限[小心设置] 2新加时必填;8修改时如果有下标则判断必填[主键不要设置，不然新建不了] 4不可修改
                ],
                'sort_num' => [
                    "field_name" => '',//  $langModel['field_names']['sort_num'] ?? '', // '排序',
                    'data_type'=>'int',//数据类型 int
                    // 'must_message' => '数据校验不通过，没有{fieldName}数据值！',// 必填时，无值的错误提示
                    'valiDateParam' => [// 参数有值时验证信息；如需要多个验证--此值为二维数组【注意：此时的多语言resources\lang\zh-CN 下具体的表文件中的message也有用一维数组】
                        // "input"=>$_POST["market_id"],"require"=>"false","var_name" => "queue" ,
     ...
     * 资源分类自定义属性-类型名称
     * @OA\Schema(
     *   schema="Schema_QualityControl_resource_type_self_type_name",
     *   type="string",
     *   title="资源分类自定义属性-类型名称",
     *   description="资源分类自定义属性-类型名称",
     *   default="",
     *   minLength=0,
     *   maxLength=30,
     *   example="",
     ...
     * 上面字段对应的查询参数--可填
     * @ OA\Parameter(
     *      parameter="Schema_QualityControl_resource_type_self_brand_name_optional",
     *      name="type_name",
     *      in="query",
     *      description="类型名称",
     *      required=false,
     *      deprecated=false,
     *      @ OA\Schema(ref="#/components/schemas/Schema_QualityControl_resource_type_self_brand_name")
     * ),
     *
     *
     */
     ...
     * 上面字段对应的查询参数--必填
     * @ OA\Parameter(
     *      parameter="Schema_QualityControl_resource_type_self_brand_name_required",
     *      name="type_name",
     *      in="query",
     *      description="类型名称",
     *      required=true,
     ...
     * 资源分类自定义属性-id
     * @OA\Schema(
     *   schema="Schema_QualityControl_resource_type_self_id",
     *   type="integer",
     *   format="int32",
     *   title="资源分类自定义属性-id",
     *   description="资源分类自定义属性-id",
     *   default=0,
     *   minimum="0",
     *   example="1",
     * )
     ...
     * 上面字段对应的查询参数--可填
     * @OA\Parameter(
     *      parameter="Schema_QualityControl_resource_type_self_id_optional",
     *      name="id",
     *      in="query",
     *      description="模板库分类-id",
     *      required=false,
     *      deprecated=false,
     *      @OA\Schema(ref="#/components/schemas/Schema_QualityControl_resource_type_self_id")
     * ),
     *
     *
     */
     ...
     * 上面字段对应的查询参数--必填
     * @OA\Parameter(
     *      parameter="Schema_QualityControl_resource_type_self_id_required",
     *      name="id",
     *      in="query",
     *      description="模板库分类-id",
     *      required=true,
     ...
     * 上面字段对应的查询参数--可填
     * @OA\Parameter(
     *      parameter="Schema_QualityControl_resource_type_self_id_optional_quote",
     ...
     * 上面字段对应的查询参数--必填
     * @OA\Parameter(
     *      parameter="Schema_QualityControl_resource_type_self_id_required_quote",
     *      name="template_type_id",
     *      in="query",
     *      description="模板库分类-id",
     *      required=true,
     ...
     * 模糊查询字段名--具体的表模型用
     * @OA\Parameter(
     *      parameter="Parameter_QualityControl_resource_type_self_field",
     *      name="field",
     *      in="query",
     *      description="模糊查询字段名",
     *      required=false,
    ...
    /**
     * @OA\Schema(
     *     schema="Schema_QualityControl_resource_type_self_obj",
     *     title="资源分类自定义",
     *     description="资源分类自定义列表",
     *     required={},
     ...
     *
     * @OA\RequestBody(
     *     request="RequestBody_QualityControl_info_resource_type_self",
     *     description="单个增加",
     *     required=true,
     *     @OA\JsonContent(ref="#/components/schemas/Schema_QualityControl_resource_type_self_obj"),
     *     @OA\MediaType(
     *         mediaType="application/xml",
     *         @OA\Schema(
     *              @OA\Property(property="info", ref="#/components/schemas/Schema_QualityControl_resource_type_self_obj"),
     *              @OA\Xml(
     *                  name="root",
     *                  wrapped=true
     *              ),
     ...
     *
     * @OA\RequestBody(
     *     request="RequestBody_QualityControl_multi_resource_type_self",
     *     description="批量增加",
     *     required=true,
     *     @OA\JsonContent(
     *          type="object",
     ...
     *              property="data_list",
     *              type="array",
     *              @OA\Items(ref="#/components/schemas/Schema_QualityControl_resource_type_self_obj"),
     *          ),
     *     ),
     *     @OA\MediaType(
     *         mediaType="application/xml",
     ...
     * 列表页
     *     @OA\Response(
     *         response="Response_QualityControl_list_resource_type_self",
     *         description="操作成功返回",
     *         @OA\JsonContent(
     *              type="object",
     ...
     * }
     *     @OA\Response(
     *         response="Response_QualityControl_info_resource_type_self",
     *         description="操作成功返回",
     *         @OA\JsonContent(
     *              type="object",
     *              @OA\Property(property="apistatus", ref="#/components/schemas/common_Schema_apistatus"),
     ...
     * }
     *     @OA\Response(
     *         response="Response_QualityControl_result_resource_type_self",
     *         description="操作成功返回",
     *         @OA\JsonContent(
     *              type="object",
     *              @OA\Property(property="apistatus", ref="#/components/schemas/common_Schema_apistatus"),
     *              @OA\Property(property="result",ref="#/components/schemas/Schema_QualityControl_resource_type_self_obj"),
     *              @OA\Property(property="errorMsg", ref="#/components/schemas/common_Schema_errorMsg"),
     *        ),
     *        @OA\XmlContent(
     *              type="object",
