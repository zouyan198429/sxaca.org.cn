...
class resource_module extends BaseDBVerify
{
    public static $model_name = 'QualityControl\ResourceModule';// 相对于Models的数据模型名称;在子类中定义，使用时用static::,不用self::
    public static $table_name = 'resource_module';// 数据表名称
    // 需要从父的去掉的字段  -- 一维数组
    ...
    public static function getTableConfig($dbFileTag = 'models', $langModel = [], $langModelDB = [], $langTable = []){
        $tableConfig = [
            'fields' => [
                ...
                'resource_id_history' => [
                    "field_name" => '',//  $langModel['field_names']['resource_id_history'] ?? '', // '资源历史',
                    'data_type'=>'int',//数据类型 int
                    // 'must_message' => '数据校验不通过，没有{fieldName}数据值！',// 必填时，无值的错误提示
                    'valiDateParam' => [// 参数有值时验证信息；如需要多个验证--此值为二维数组【注意：此时的多语言resources\lang\zh-CN 下具体的表文件中的message也有用一维数组】
                        // "input"=>$_POST["market_id"],"require"=>"false","var_name" => "queue" ,
                ...
                'module_type' => [
                    ...
                    'valiDateParam' => [// 参数有值时验证信息；如需要多个验证--此值为二维数组【注意：此时的多语言resources\lang\zh-CN 下具体的表文件中的message也有用一维数组】
                        // "input"=>$_POST["market_id"],"require"=>"false","var_name" => "queue" ,
                        "validator"=>"length","min"=>"0","max"=>"80","message" => '',// '{fieldName}' . ($langModel['valueLenIs'] ?? '') . ' 0~ 80 ' . ($langModel['numsCharacters'] ?? '') . '!'
                    ],
                    'powerNum' => 0,// (2 | 4)  特殊权限[小心设置] 2新加时必填;8修改时如果有下标则判断必填[主键不要设置，不然新建不了] 4不可修改
                ],
                'operate_staff_id' => [
     ...
     * 资源模块使用属性-模块标识
     * @OA\Schema(
     *   schema="Schema_QualityControl_resource_module_module_type",
     *   type="string",
     *   title="资源模块使用属性-模块标识",
     *   description="资源模块使用属性-模块标识",
     *   default="",
     *   minLength=0,
     *   maxLength=80,
     *   example="",
     ...
     * 资源模块使用属性-模块id
     * @OA\Schema(
     *   schema="Schema_QualityControl_resource_module_module_id",
     *   type="integer",
     *   format="int32",
     *   title="资源模块使用属性-模块id",
     *   description="资源模块使用属性-模块id",
     *   default=0,
     *   minimum="0",
     *   example="1",
     * )
     ...
     * 资源模块使用属性-id
     * @OA\Schema(
     *   schema="Schema_QualityControl_resource_module_id",
     *   type="integer",
     *   format="int32",
     *   title="资源模块使用属性-id",
     *   description="资源模块使用属性-id",
     *   default=0,
     *   minimum="0",
     *   example="1",
     * )
     ...
     * 模糊查询字段名--具体的表模型用
     * @OA\Parameter(
     *      parameter="Parameter_QualityControl_resource_module_field",
     *      name="field",
     *      in="query",
     *      description="模糊查询字段名",
     *      required=false,
    ...
    /**
     * @OA\Schema(
     *     schema="Schema_QualityControl_resource_module_obj",
     *     title="资源模块使用",
     *     description="资源模块使用列表",
     *     required={},
     ...
     *
     * @OA\RequestBody(
     *     request="RequestBody_QualityControl_info_resource_module",
     *     description="单个增加",
     *     required=true,
     *     @OA\JsonContent(ref="#/components/schemas/Schema_QualityControl_resource_module_obj"),
     *     @OA\MediaType(
     *         mediaType="application/xml",
     *         @OA\Schema(
     *              @OA\Property(property="info", ref="#/components/schemas/Schema_QualityControl_resource_module_obj"),
     *              @OA\Xml(
     *                  name="root",
     *                  wrapped=true
     *              ),
     ...
     *
     * @OA\RequestBody(
     *     request="RequestBody_QualityControl_multi_resource_module",
     *     description="批量增加",
     *     required=true,
     *     @OA\JsonContent(
     *          type="object",
     ...
     *              property="data_list",
     *              type="array",
     *              @OA\Items(ref="#/components/schemas/Schema_QualityControl_resource_module_obj"),
     *          ),
     *     ),
     *     @OA\MediaType(
     *         mediaType="application/xml",
     ...
     * 列表页
     *     @OA\Response(
     *         response="Response_QualityControl_list_resource_module",
     *         description="操作成功返回",
     *         @OA\JsonContent(
     *              type="object",
     ...
     * }
     *     @OA\Response(
     *         response="Response_QualityControl_info_resource_module",
     *         description="操作成功返回",
     *         @OA\JsonContent(
     *              type="object",
     *              @OA\Property(property="apistatus", ref="#/components/schemas/common_Schema_apistatus"),
     ...
     * }
     *     @OA\Response(
     *         response="Response_QualityControl_result_resource_module",
     *         description="操作成功返回",
     *         @OA\JsonContent(
     *              type="object",
     *              @OA\Property(property="apistatus", ref="#/components/schemas/common_Schema_apistatus"),
     *              @OA\Property(property="result",ref="#/components/schemas/Schema_QualityControl_resource_module_obj"),
     *              @OA\Property(property="errorMsg", ref="#/components/schemas/common_Schema_errorMsg"),
     *        ),
     *        @OA\XmlContent(
     *              type="object",
