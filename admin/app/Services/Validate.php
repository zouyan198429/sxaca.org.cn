...
Class Validate{
     ...
     * 需要验证的内容
     *
     * @param array $validateparam array("var_name" => "验证变量名[可为空]" ,"input"=>"","require"=>"","validator"=>"","regexp"=>"","operator"=>"","to"=>"","min"=>"","max"=>"",message=>"")
     * input要验证的值
    ...
    private function getError(){
        ...
        foreach($this->validateparam as $k=>$v){
            if ($v['result'] == false){//返回错误提示
                array_push($errMsg, $v['message']);
                if(isset($v['var_name']) && strlen($v['var_name']) > 0) {
                    if(!isset($varErrMsg[$v['var_name']])) $varErrMsg[$v['var_name']] = [];
                    array_push($varErrMsg[$v['var_name']], $v['message']);
                }
                if(strlen($firstErr) <= 0) $firstErr = $v['message'];
                // return $v['message'];
            }
        }
        if(empty($errMsg)) return null;
        $errs = [
            'firstErr' => $firstErr,
            'errMsg' => $errMsg,
            'varErrMsg' => $varErrMsg,
