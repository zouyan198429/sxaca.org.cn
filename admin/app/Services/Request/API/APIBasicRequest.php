...
//    }
    ...
    public static function compareHistoryOrUpdateVersionApi($modelName, $primaryVal, $historyObj, $HistoryTableName, $historySearch, $ignoreFields = [], $forceIncVersion= 1, $companyId = null , $notLog = 0){
        ...
        // pr($requestTesUrl);// echo $requestTesUrl;die();

        if((static::$request_mode & 2) == 2){// 2 访问本地数据库，获得数据
            $dbObj = null ;
            $modelObj = null;
            $historyObjReal = null;
            // if(static::$dataFromType == 1) CommonDB::getObjByModelName($historyObj, $historyObjReal);
            CommonDB::getObjByModelName($historyObj, $historyObjReal);
            return Tool::objectToArray(CommonAPIFromDBBusiness::exeCompareHistoryOrUpdateVersion($primaryVal, $HistoryTableName, $historySearch, $ignoreFields, $forceIncVersion, $dbObj, $historyObj, $historyObjReal, $modelName, $modelObj));
        }
        // 通过api获得数据
        return HttpRequest::HttpRequestApi($url, $requestData, [], 'POST', static::getHeadersByConfig());
    }
    ...
    public static function getHistoryIdApi($modelName, $primaryVal, $historyObj, $HistoryTableName, $historySearch, $ignoreFields = [], $companyId = null , $notLog = 0){
        ...
        if((static::$request_mode & 2) == 2){// 2 访问本地数据库，获得数据
            ...
            // if(static::$dataFromType == 1) CommonDB::getObjByModelName($historyObj, $historyObjReal);
            CommonDB::getObjByModelName($historyObj, $historyObjReal);
            return Tool::objectToArray(CommonAPIFromDBBusiness::getHistoryId($primaryVal, $HistoryTableName, $historySearch, $ignoreFields, $dbObj, $historyObj, $historyObjReal, $modelName, $modelObj));
        }
        // 通过api获得数据
        return HttpRequest::HttpRequestApi($url, $requestData, [], 'POST', static::getHeadersByConfig());
    }
...
//        $url = config('public.apiUrl') . config('apiUrl.common.firstOrCreateApi');
        ...
        if((static::$request_mode & 2) == 2){// 2 访问本地数据库，获得数据
            $modelObj = null;
            return Tool::objectToArray(CommonAPIFromDBBusiness::getFirstOrCreate($searchConditon, $updateFields, $modelName, $modelObj));
        }
        // 通过api获得数据
        return HttpRequest::HttpRequestApi($url, $requestData, [], 'POST', static::getHeadersByConfig());
    }
...
//        $url = config('public.apiUrl') . config('apiUrl.common.updateOrCreateApi');
        ...
        if((static::$request_mode & 2) == 2){// 2 访问本地数据库，获得数据
            $modelObj = null;
            return Tool::objectToArray(CommonAPIFromDBBusiness::exeUpdateOrCreate($searchConditon, $updateFields, $modelName, $modelObj));
        }
        // 通过api获得数据
        return HttpRequest::HttpRequestApi($url, $requestData, [], 'POST', static::getHeadersByConfig());
    }
...
//        $url = config('public.apiUrl') . config('apiUrl.common.getinfoApi');
        ...
        if((static::$request_mode & 2) == 2){// 2 访问本地数据库，获得数据
            ...
            return Tool::objectToArray(CommonAPIFromDBBusiness::getInfoByID($id, $selectParams, $relations, $modelName, $modelObj));
        }
        // 通过api获得数据
        return HttpRequest::HttpRequestApi($url, $requestData, [], 'POST', static::getHeadersByConfig());
    }
...
//        $url = config('public.apiUrl') . config('apiUrl.common.getinfoApi');
        ...
        if((static::$request_mode & 2) == 2){// 2 访问本地数据库，获得数据}
            ...
            return Tool::objectToArray(CommonAPIFromDBBusiness::getInfoByQuery($pagesize, $queryParams, $relations, $modelName, $modelObj));
        }
        // 通过api获得数据
        return HttpRequest::HttpRequestApi($url, $requestData, [], 'POST', static::getHeadersByConfig());
    }
...
//        }
        ...
        if((static::$request_mode & 2) == 2){// 2 访问本地数据库，获得数据
            ...
            return Tool::objectToArray(CommonAPIFromDBBusiness::getKV($kvParams, $selectParams, $queryParams, $modelName, $modelObj));
        }
        // 通过api获得数据
        return HttpRequest::HttpRequestApi($url, $requestData, [], 'POST', static::getHeadersByConfig());
    }
...
//        }
        ...
        if((static::$request_mode & 2) == 2){// 2 访问本地数据库，获得数据
            $modelObj = null;
            return Tool::objectToArray(CommonAPIFromDBBusiness::getAttr($attrName, $isStatic, $modelName, $modelObj));
        }
        // 通过api获得数据
        return HttpRequest::HttpRequestApi($url, $requestData, [], 'POST', static::getHeadersByConfig());
    }
...
//        }
        ...
        if((static::$request_mode & 2) == 2){// 2 访问本地数据库，获得数据
            ...
            return Tool::objectToArray(CommonAPIFromDBBusiness::exeMethod($methodName, $params, $modelName, $modelObj));
        }
        // 通过api获得数据
        return HttpRequest::HttpRequestApi($url, $requestData, [], 'POST', static::getHeadersByConfig());
    }
...
//        }
        ...
        if((static::$request_mode & 2) == 2){// 2 访问本地数据库，获得数据
            $modelObj = null;
            return Tool::objectToArray(CommonAPIFromDBBusiness::getBusinessDBAttr($attrName, $isStatic, $modelName, $modelObj));
        }
        // 通过api获得数据
        return HttpRequest::HttpRequestApi($url, $requestData, [], 'POST', static::getHeadersByConfig());
    }
...
//        }
        ...
        if((static::$request_mode & 2) == 2){// 2 访问本地数据库，获得数据
            $modelObj = null;
            if (!is_array($params)) $params =[];
            return Tool::objectToArray(CommonAPIFromDBBusiness::exeBusinessDBMethod($methodName, $params, $modelName, $modelObj));
        }
        // 通过api获得数据
        return HttpRequest::HttpRequestApi($url, $requestData, [], 'POST', static::getHeadersByConfig());
    }
...
//        }
        ...
        if((static::$request_mode & 2) == 2){// 2 访问本地数据库，获得数据
            $modelObj = null;
            return Tool::objectToArray(CommonAPIFromBusiness::getBusinessAttr($attrName, $isStatic, $modelName, $modelObj));
        }
        // 通过api获得数据
        return HttpRequest::HttpRequestApi($url, $requestData, [], 'POST', static::getHeadersByConfig());
    }
...
//        }
        ...
        if((static::$request_mode & 2) == 2){// 2 访问本地数据库，获得数据
            $modelObj = null;
            // 特别注意；如果方法的第一个参数是 Request $request,Controller $Controller ，则可以放在参数的最前面--处理好再传入进来
//            array_unshift($params,$Controller); --参考
//            array_unshift($params,$request);--参考

            if (!is_array($params)) $params =[];
            return Tool::objectToArray(CommonAPIFromBusiness::exeBusinessMethod($methodName, $params, $modelName, $modelObj));
        }
        // 通过api获得数据
        return HttpRequest::HttpRequestApi($url, $requestData, [], 'POST', static::getHeadersByConfig());
    }
...
//        $url = config('public.apiUrl') . config('apiUrl.common.delApi');
        ...
        if((static::$request_mode & 2) == 2){// 2 访问本地数据库，获得数据
            $modelObj = null;
            return Tool::objectToArray(CommonAPIFromDBBusiness::exeDel($queryParams, $modelName, $modelObj));
        }
        // 通过api获得数据
        return HttpRequest::HttpRequestApi($url, $requestData, [], 'POST', static::getHeadersByConfig());
    }
     ...
     * @param object $modelObj 当前模型对象
     * @param int $companyId 企业id
     * @param array $pageParams --- 这个参数没有用到了
         [
            'page' => $page,
            'pagesize' => $pagesize,
            'total' => $total,
...
//        $url = config('public.apiUrl') . config('apiUrl.common.getAllApi');
        ...
        if((static::$request_mode & 2) == 2){// 2 访问本地数据库，获得数据
            $modelObj = null;
            return Tool::objectToArray(CommonAPIFromDBBusiness::getAllDataByModel($queryParams, $relations, $modelName, $modelObj));
        }
        // 通过api获得数据
        return HttpRequest::HttpRequestApi($url, $requestData, [], 'POST', static::getHeadersByConfig());
    }
     ...
     * @author zouyan(305463219@qq.com)
     */
    public static function ajaxGetQueryListApi($modelName, $companyId = null,$queryParams='' ,$relations = '', $notLog = 0){
        $relations = RelationDB::getDBRelation($relations);// 获得关系数组,为请求api使用
        $requestData = [
            // 'company_id' => $companyId,
            'Model_name' => $modelName, // 模型
...
//        $url = config('public.apiUrl') . config('apiUrl.common.getListQueryApi');
        ...
        if((static::$request_mode & 2) == 2){// 2 访问本地数据库，获得数据
            $modelObj = null;
            return Tool::objectToArray(CommonAPIFromDBBusiness::getDataByQuery($queryParams, $relations, $modelName, $modelObj));
        }
        // 通过api获得数据
        return HttpRequest::HttpRequestApi($url, $requestData, [], 'POST', static::getHeadersByConfig());
    }
...
//        $url = config('public.apiUrl') . config('apiUrl.common.getlistApi');
        ...
        if((static::$request_mode & 2) == 2){// 2 访问本地数据库，获得数据
            ...
            return Tool::objectToArray(CommonAPIFromDBBusiness::getListDataByModel($page, $pagesize, $total, $queryParams, $relations, $modelName, $modelObj));
        }
        // 通过api获得数据
        return HttpRequest::HttpRequestApi($url, $requestData, [], 'POST', static::getHeadersByConfig());
    }
...
//        $url = config('public.apiUrl') . config('apiUrl.common.addnewApi');
        ...
        if((static::$request_mode & 2) == 2){// 2 访问本地数据库，获得数据
            $modelObj = null;
            return Tool::objectToArray(CommonAPIFromDBBusiness::exeCreate($saveData, $modelName, $modelObj));
        }
        // 通过api获得数据
        return HttpRequest::HttpRequestApi($url, $requestData, [], 'POST', static::getHeadersByConfig());
    }
...
//        $url = config('public.apiUrl') . config('apiUrl.common.addnewBathApi');
        ...
        if((static::$request_mode & 2) == 2){// 2 访问本地数据库，获得数据
            $modelObj = null;
            return Tool::objectToArray(CommonAPIFromDBBusiness::exeCreateBath($saveData, $modelName, $modelObj));
        }
        // 通过api获得数据
        return HttpRequest::HttpRequestApi($url, $requestData, [], 'POST', static::getHeadersByConfig());
    }
...
//        $url = config('public.apiUrl') . config('apiUrl.common.addnewBathByIdApi');
        ...
        if((static::$request_mode & 2) == 2){// 2 访问本地数据库，获得数据
            $modelObj = null;
            return Tool::objectToArray(CommonAPIFromDBBusiness::exeCreateBathByPrimaryKey($saveData, $primaryKey, $modelName, $modelObj));
        }
        // 通过api获得数据
        return HttpRequest::HttpRequestApi($url, $requestData, [], 'POST', static::getHeadersByConfig());
    }
...
//        $url = config('public.apiUrl') . config('apiUrl.common.saveApi');
        ...
        if((static::$request_mode & 2) == 2){// 2 访问本地数据库，获得数据
            $modelObj = null;
            return Tool::objectToArray(CommonAPIFromDBBusiness::exeUpdate($saveData, $queryParams, $modelName, $modelObj));
        }
        // 通过api获得数据
...
//        $url = config('public.apiUrl') . config('apiUrl.common.saveBathById');
        ...
        if((static::$request_mode & 2) == 2){// 2 访问本地数据库，获得数据
            $modelObj = null;
            return Tool::objectToArray(CommonAPIFromDBBusiness::exeBatchSaveByPrimaryKey($saveData, $primaryKey, $modelName, $modelObj));
        }
        // 通过api获得数据
        return HttpRequest::HttpRequestApi($url, $requestData, [], 'POST', static::getHeadersByConfig());
    }
...
//        $url = config('public.apiUrl') . config('apiUrl.common.saveByIdApi');
        ...
        if((static::$request_mode & 2) == 2){// 2 访问本地数据库，获得数据
            ...
            return Tool::objectToArray(CommonAPIFromDBBusiness::exeSave($saveData, $id, $modelName, $modelObj));
        }
        // 通过api获得数据
        return HttpRequest::HttpRequestApi($url, $requestData, [], 'POST', static::getHeadersByConfig());
    }
     ...
     * @author zouyan(305463219@qq.com)
     */
    public static function incDecByQueyApi($modelName, $queryParams='', $incDecType = 'dec', $incDecField = '', $incDecVal = 0, $saveData= [], $companyId = null, $notLog = 0, $modifFields = [] ){
        ...
        $requestData = [
            ...
            'modifFields' => $modifFields,// 修改的其它字段 -没有，则传空数组
        ];
        if (is_numeric($companyId) && $companyId > 0) {
            $requestData['company_id'] = $companyId ;
        }
...
//        $url = config('public.apiUrl') . config('apiUrl.common.saveDecIncByQueryApi');
        ...
        if((static::$request_mode & 2) == 2){// 2 访问本地数据库，获得数据
            ...
            return Tool::objectToArray(CommonAPIFromDBBusiness::exeSaveDecIncByQuery($incDecField, $incDecVal, $incDecType, $queryParams, $modifFields, $modelName, $modelObj));
        }
        // 通过api获得数据
        return HttpRequest::HttpRequestApi($url, $requestData, [], 'POST', static::getHeadersByConfig());
    }
...
//        $url = config('public.apiUrl') . config('apiUrl.common.saveDecIncByArrApi');
        ...
        if((static::$request_mode & 2) == 2){// 2 访问本地数据库，获得数据
            $modelObj = null;
            return Tool::objectToArray(CommonAPIFromDBBusiness::exeBatchSaveDecIncByPrimaryKey($saveData, $modelName, $modelObj));
        }
        // 通过api获得数据
        return HttpRequest::HttpRequestApi($url, $requestData, [], 'POST', static::getHeadersByConfig());
    }
...
//        $url = config('public.apiUrl') . config('apiUrl.common.saveSyncByIdApi');
        ...
        if((static::$request_mode & 2) == 2){// 2 访问本地数据库，获得数据
            ...
            return Tool::objectToArray(CommonAPIFromDBBusiness::exeSync($id, $syncParams, $modelName, $modelObj));
        }
        // 通过api获得数据
        return HttpRequest::HttpRequestApi($url, $requestData, [], 'POST', static::getHeadersByConfig());
    }
...
//        $url = config('public.apiUrl') . config('apiUrl.common.detachApi');
        ...
        if((static::$request_mode & 2) == 2){// 2 访问本地数据库，获得数据
            ...
            return Tool::objectToArray(CommonAPIFromDBBusiness::exeDetach($id, $detachParams, $modelName, $modelObj));
        }
        // 通过api获得数据
        return HttpRequest::HttpRequestApi($url, $requestData, [], 'POST', static::getHeadersByConfig());
    }
