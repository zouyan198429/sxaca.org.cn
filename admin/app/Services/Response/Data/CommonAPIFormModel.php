...
//        }
//        return $requestData;
        return static::getAllDataByModel($queryParams, $relations, static::requestGetModelName($request), $modelObj);
    }

    // 上面方法的直接执行（已拿到相关参数）-- 方便其它地方有参数时，直接可以调用
    public static function getAllDataByModel($queryParams, $relations, $modelName = '', &$modelObj = null){
        // 获得对象
...
//        }
//        return $requestData;
        return static::getDataByQuery($queryParams, $relations, static::requestGetModelName($request), $modelObj);
    }

    // 上面方法的直接执行（已拿到相关参数）-- 方便其它地方有参数时，直接可以调用
    public static function getDataByQuery($queryParams, $relations, $modelName = '', &$modelObj = null){
        // 获得对象
        static::getBusinessDBObjByModelName($modelName, $modelObj );
...
//        }
//        return $requestData;
        return static::getListDataByModel($page, $pagesize, $total, $queryParams, $relations, static::requestGetModelName($request), $modelObj);
    }

    // 上面方法的直接执行（已拿到相关参数）-- 方便其它地方有参数时，直接可以调用
    public static function getListDataByModel($page, $pagesize, $total, $queryParams, $relations, $modelName = '', &$modelObj = null){
        // 获得对象
        static::getBusinessDBObjByModelName($modelName, $modelObj );
...
//        }
//        return  $requestData;
        return static::getInfoByID($id, $selectParams, $relations, static::requestGetModelName($request), $modelObj);
    }
...
//        }
//        return  $requestData;
        return static::getInfoByQuery($pagesize, $queryParams, $relations, static::requestGetModelName($request), $modelObj);
    }

    // 上面方法的直接执行（已拿到相关参数）-- 方便其它地方有参数时，直接可以调用
    public static function getInfoByQuery($pagesize, $queryParams, $relations, $modelName = '', &$modelObj = null){
        // 获得对象
        static::getBusinessDBObjByModelName($modelName, $modelObj );
...
//        }
//        return $requestData;
        return static::exeDel($queryParams, static::requestGetModelName($request), $modelObj);
    }

    // 上面方法的直接执行（已拿到相关参数）-- 方便其它地方有参数时，直接可以调用
    public static function exeDel($queryParams, $modelName = '', &$modelObj = null){
        // 获得对象
        static::getBusinessDBObjByModelName($modelName, $modelObj );
...
//
//        return  $successRels;
        return static::exeSync($id, $synces, static::requestGetModelName($request), $modelObj);
    }

    // 上面方法的直接执行（已拿到相关参数）-- 方便其它地方有参数时，直接可以调用
    public static function exeSync($id, $synces, $modelName = '', &$modelObj = null){
        // 获得对象
        static::getBusinessDBObjByModelName($modelName, $modelObj );
...
//        }
//        return  $successRels;
        return static::exeDetach($id, $detaches, static::requestGetModelName($request), $modelObj);
    }

    // 上面方法的直接执行（已拿到相关参数）-- 方便其它地方有参数时，直接可以调用
    public static function exeDetach($id, $detaches, $modelName = '', &$modelObj = null){
        // 获得对象
        static::getBusinessDBObjByModelName($modelName, $modelObj );
...
//        }
//        return $requestData;
        return static::exeCreate($dataParams, static::requestGetModelName($request), $modelObj);
    }

    // 上面方法的直接执行（已拿到相关参数）-- 方便其它地方有参数时，直接可以调用
    public static function exeCreate($dataParams, $modelName = '', &$modelObj = null){
        // 获得对象
        static::getBusinessDBObjByModelName($modelName, $modelObj );
...
//        }
//        return $requestData;
        return static::exeCreateBath($dataParams, static::requestGetModelName($request), $modelObj);
    }

    // 上面方法的直接执行（已拿到相关参数）-- 方便其它地方有参数时，直接可以调用
    public static function exeCreateBath($dataParams, $modelName = '', &$modelObj = null){
        // 获得对象
        static::getBusinessDBObjByModelName($modelName, $modelObj );
...
//        }
//        return $newIds;
        return static::exeCreateBathByPrimaryKey($dataParams, $primaryKey, static::requestGetModelName($request), $modelObj);
    }

    // 上面方法的直接执行（已拿到相关参数）-- 方便其它地方有参数时，直接可以调用
    public static function exeCreateBathByPrimaryKey($dataParams, $primaryKey, $modelName = '', &$modelObj = null){
        // 获得对象
        static::getBusinessDBObjByModelName($modelName, $modelObj );
...
//        }
//        return $result;
        return static::exeSave($dataParams, $id, static::requestGetModelName($request), $modelObj);
    }

    // 上面方法的直接执行（已拿到相关参数）-- 方便其它地方有参数时，直接可以调用
    public static function exeSave($dataParams, $id, $modelName = '', &$modelObj = null){
        // 获得对象
        static::getBusinessDBObjByModelName($modelName, $modelObj );
...
//        return  $successRels;
        return static::exeBatchSaveByPrimaryKey($dataParams, $primaryKey, $modelName, $modelObj);

    }

    // 上面方法的直接执行（已拿到相关参数）-- 方便其它地方有参数时，直接可以调用
    public static function exeBatchSaveByPrimaryKey($dataParams, $primaryKey, $modelName = '', &$modelObj = null){
...
//            $requestData = CommonDB::updateQuery($modelObj, $dataParams, $queryParams, true);
//        }else{// 调用中间层的方法来获得数据
//            $requestData = $modelObj->save($dataParams, $queryParams);
//        }
//        return $requestData;
        return static::exeUpdate($dataParams, $queryParams, static::requestGetModelName($request), $modelObj);
    }

    // 上面方法的直接执行（已拿到相关参数）-- 方便其它地方有参数时，直接可以调用
    public static function exeUpdate($dataParams, $queryParams, $modelName = '', &$modelObj = null){
        // 获得对象
        static::getBusinessDBObjByModelName($modelName, $modelObj );
...
//        if(static::$dataFromType == 1){// 直接通过数据模型获得数据
//            $requestData = CommonDB::saveDecInc($modelObj, $incDecField, $incDecVal, $incDecType, $queryParams, $modifFields, true);
//        }else{// 调用中间层的方法来获得数据
//            $requestData = $modelObj->saveDecIncByQuery($incDecField, $incDecVal, $incDecType, $queryParams, $modifFields);
//        }
//        return $requestData;
        return static::exeSaveDecIncByQuery($incDecField, $incDecVal, $incDecType, $queryParams, $modifFields, static::requestGetModelName($request), $modelObj);

    }

    // 上面方法的直接执行（已拿到相关参数）-- 方便其它地方有参数时，直接可以调用
    public static function exeSaveDecIncByQuery($incDecField, $incDecVal, $incDecType, $queryParams, $modifFields, $modelName = '', &$modelObj = null){
        // 获得对象
        static::getBusinessDBObjByModelName($modelName, $modelObj );
...
//        return  $successRels;
        return static::exeBatchSaveDecIncByPrimaryKey($dataParams, static::requestGetModelName($request), $modelObj);
    }

    // 上面方法的直接执行（已拿到相关参数）-- 方便其它地方有参数时，直接可以调用
    public static function exeBatchSaveDecIncByPrimaryKey($dataParams, $modelName = '', &$modelObj = null){
...
//        }
//        return  $requestData;
        return static::getKV($kvParams, $selectParams, $queryParams, static::requestGetModelName($request), $modelObj);
    }

    // 上面方法的直接执行（已拿到相关参数）-- 方便其它地方有参数时，直接可以调用
    public static function getKV($kvParams, $selectParams, $queryParams, $modelName = '', &$modelObj = null){
        // 获得对象
        static::getBusinessDBObjByModelName($modelName, $modelObj );
...
//        return  $historyObj->id;
        $dbObj = null ;
        return static::getHistoryId($primaryVal, $historyTableName, $historySearch, $ignoreFields, $dbObj, $historyObjName, $historyObj, static::requestGetModelName($request), $modelObj);

    }

    // 上面方法的直接执行（已拿到相关参数）-- 方便其它地方有参数时，直接可以调用
    public static function getHistoryId($primaryVal, $historyTableName, $historySearch, $ignoreFields, &$dbObj = null, $historyObjName = '', &$historyObj = null, $modelName = '', &$modelObj = null){
        // 获得对象
        static::getBusinessDBObjByModelName($modelName, $modelObj );
...
//        return  $diffDataArr;
        $dbObj = null ;
        return static::exeCompareHistoryOrUpdateVersion($primaryVal, $historyTableName, $historySearch, $ignoreFields, $forceIncVersion, $dbObj, $historyObjName, $historyObj, static::requestGetModelName($request), $modelObj);

    }

    // 上面方法的直接执行（已拿到相关参数）-- 方便其它地方有参数时，直接可以调用
    public static function exeCompareHistoryOrUpdateVersion($primaryVal, $historyTableName, $historySearch, $ignoreFields, $forceIncVersion, &$dbObj = null, $historyObjName = '', &$historyObj = null, $modelName = '', &$modelObj = null){
        // 获得对象
        static::getBusinessDBObjByModelName($modelName, $modelObj );
...
//        }
//        return  $modelObj;
        return static::getFirstOrCreate($searchConditon, $updateFields, static::requestGetModelName($request), $modelObj);
    }

    // 上面方法的直接执行（已拿到相关参数）-- 方便其它地方有参数时，直接可以调用
    public static function getFirstOrCreate($searchConditon, $updateFields, $modelName = '', &$modelObj = null){
        // 获得对象
        static::getBusinessDBObjByModelName($modelName, $modelObj );
...
//        if(static::$dataFromType == 1){// 直接通过数据模型获得数据
//            CommonDB::updateOrCreate($modelObj, $searchConditon, $updateFields, true);
//        }else{// 调用中间层的方法来获得数据
//            $dbObj = null ;
...
//        return  $modelObj;
        ...
        return static::exeUpdateOrCreate($searchConditon, $updateFields, static::requestGetModelName($request), $modelObj);
    }

    // 上面方法的直接执行（已拿到相关参数）-- 方便其它地方有参数时，直接可以调用
    public static function exeUpdateOrCreate($searchConditon, $updateFields, $modelName = '', &$modelObj = null){
        // 获得对象
        static::getBusinessDBObjByModelName($modelName, $modelObj );
...
//        }
//        return  $attrVal;
        return static::getAttr($attrName, $isStatic, static::requestGetModelName($request), $modelObj);
    }

    // 上面方法的直接执行（已拿到相关参数）-- 方便其它地方有参数时，直接可以调用
    public static function getAttr($attrName, $isStatic, $modelName = '', &$modelObj = null){
        // 获得对象
        static::getBusinessDBObjByModelName($modelName, $modelObj );
...
//        }
//        return  $result;
        return static::exeMethod($methodName, $params, static::requestGetModelName($request), $modelObj);
    }

    // 上面方法的直接执行（已拿到相关参数）-- 方便其它地方有参数时，直接可以调用
    public static function exeMethod($methodName, $params, $modelName = '', &$modelObj = null){
        // 获得对象
        static::getBusinessDBObjByModelName($modelName, $modelObj );
