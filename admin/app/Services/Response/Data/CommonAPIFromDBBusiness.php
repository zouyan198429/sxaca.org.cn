...
//    }
    ...
    // 实例化数据中间层 ，获得中间层属性--此方法感觉多余，父类已经有同样的方法
...
//        return  $attrVal;
        ...
        return static::getBusinessDBAttr($attrName, $isStatic, $modelName, $modelObj);
    }

    // 上面方法的直接执行（已拿到相关参数）-- 方便其它地方有参数时，直接可以调用
    public static function getBusinessDBAttr($attrName, $isStatic, $modelName = '', &$modelObj = null){
        // 获得对象
        static::getBusinessDBObjByModelName($modelName, $modelObj );
    ...
    // 实例化数据中间层 ，执行中间层方法--此方法感觉多余，父类已经有同样的方法
    //  @param string 必填 $Model_name model名称 或传入 $modelObj 对象
...
//        return  $result;
        $modelName = static::requestGetModelName($request);
        return static::exeBusinessDBMethod($methodName, $params, $modelName, $modelObj);
    }

    // 上面方法的直接执行（已拿到相关参数）-- 方便其它地方有参数时，直接可以调用
    public static function exeBusinessDBMethod($methodName, $params = [], $modelName = '', &$modelObj = null){
        // 获得对象
        static::getBusinessDBObjByModelName($modelName, $modelObj );
        $result = Tool::exeMethod($modelObj, $methodName, $params);
        return  $result;
