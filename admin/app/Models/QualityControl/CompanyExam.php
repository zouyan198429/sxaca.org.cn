...
class CompanyExam extends BasePublicModel
{
    //****************数据据缓存**相关的***开始********************************************
...
//    public static $primaryKeyValType = 1;
    ...
    // 其它具体的类可以在自己的类中 继承或 重写此常量 -- 只对外提供使用
     const ORDER_BY = ['status_no' => 'asc', 'exam_begin_time' => 'asc'];// ['sort_num' => 'desc', 'id' => 'desc'];// ['id' => 'desc'];
    ...
    // review_method 批卷方式1自动批卷；2人工批卷--有需要人工批卷的分类试题
    const REVIEW_METHOD_AUTO = CompanyPaper::REVIEW_METHOD_AUTO;
    const REVIEW_METHOD_HAND = CompanyPaper::REVIEW_METHOD_HAND;
    const REVIEW_METHOD_ARR = CompanyPaper::REVIEW_METHOD_ARR;

    // review_status 批改状态1未批改；2待批改；4批改中；8已批改
    const REVIEW_STATUS_NOT = 1;
    const REVIEW_STATUS_WAIT = 2;
    const REVIEW_STATUS_DOING = 4;
    const REVIEW_STATUS_DOED = 8;
    const REVIEW_STATUS_ARR = [
        self::REVIEW_STATUS_NOT => '未批改',
        self::REVIEW_STATUS_WAIT => '待批改',
        self::REVIEW_STATUS_DOING => '批改中',
        self::REVIEW_STATUS_DOED => '已批改',
    ];

    // status_no 状态1等考试2考试中4已考试8缺考
    const STATUS_NO_WAIT = 1;
    const STATUS_NO_DOING = 2;
    const STATUS_NO_DOED = 4;
    const STATUS_NO_MISS = 8;
    const STATUS_NO_ARR = [
        self::STATUS_NO_WAIT => '待考试',
        self::STATUS_NO_DOING => '考试中',
        self::STATUS_NO_DOED => '已考试',
        self::STATUS_NO_MISS => '缺考',
    ];

    // exam_subject_updated 卷或试题是否有更新【未到考试时可以变动】1没有 ；2试卷的试题有更新【修改或删除】--需要人工去判断是否更新过来；4试卷有更新---需要人工去判断是否更新过来
    const EXAM_SUBJECT_UPDATED_NO = 1;
    const EXAM_SUBJECT_UPDATED_SUBJECT = 2;
    const EXAM_SUBJECT_UPDATED_EXAM = 4;
    const EXAM_SUBJECT_UPDATED_ARR = [
        self::EXAM_SUBJECT_UPDATED_NO => '无更新',
        self::EXAM_SUBJECT_UPDATED_SUBJECT => '试题有更新',
        self::EXAM_SUBJECT_UPDATED_EXAM => '试卷有更新',
    ];

    // 开通状态1开通；2关闭；4作废【过时关闭】；
    const OPEN_STATUS_OPEN = CompanySubject::OPEN_STATUS_OPEN;// 开通
    const OPEN_STATUS_CLOSE = CompanySubject::OPEN_STATUS_CLOSE;// 关闭
    const OPEN_STATUS_CANCEL = CompanySubject::OPEN_STATUS_CANCEL;// 作废
    const OPEN_STATUS_ARR = CompanySubject::OPEN_STATUS_ARR;

    // 表里没有的字段
    protected $appends = ['review_method_text', 'review_status_text', 'status_no_text', 'exam_subject_updated_text', 'open_status_text'];
     ...
     * @return string
     */
    public function getReviewMethodTextAttribute()
    {
        return Tool::getBitVals(static::REVIEW_METHOD_ARR, $this->review_method, '、');
        // return static::REVIEW_METHOD_ARR[$this->review_method] ?? '';
    }
    ...
    {
        return Tool::getBitVals(static::REVIEW_STATUS_ARR, $this->review_status, '、');
        // return static::REVIEW_STATUS_ARR[$this->review_status] ?? '';
    }
     ...
     * @return string
     */
    public function getStatusNoTextAttribute()
    {
        return Tool::getBitVals(static::STATUS_NO_ARR, $this->status_no, '、');
        // return static::STATUS_NO_ARR[$this->status_no] ?? '';
    }
     ...
     * @return string
     */
    public function getExamSubjectUpdatedTextAttribute()
    {
        return Tool::getBitVals(static::EXAM_SUBJECT_UPDATED_ARR, $this->exam_subject_updated, '、');
        // return static::EXAM_SUBJECT_UPDATED_ARR[$this->exam_subject_updated] ?? '';
    }
    ...
    {
        return $this->hasMany('App\Models\QualityControl\CompanyExamStaff', 'exam_id', 'id');
    }
    ...
    {
        return $this->hasMany('App\Models\QualityControl\CompanyExamStaffSubject', 'exam_id', 'id');
    }

    /**
     * 获取分数等级-二维
     */
    public function coreGrade()
    {
        return $this->hasMany('App\Models\QualityControl\CompanyCoreGrade', 'exam_id', 'id');
    }
    ...
    {
        return $this->hasOne('App\Models\QualityControl\CompanyPaper', 'paper_id', 'id');
    }
}
