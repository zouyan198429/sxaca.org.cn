...
class CompanySubject extends BasePublicModel
{
    //****************数据据缓存**相关的***开始********************************************
...
//    public static $primaryKeyValType = 1;
    ...
    const ITEM_MAX_ANSWER_ORD = "K";// 单选或多选时，最多达到的选项 A、B、C、D、E、F、G、H、I、J、K

    const ONE_FONT_WIDTH = 18;// 一个字的宽度 15px
    // 填空题填写位置类型
    const INPUT_SHOW_UNDERLINE = 1;// 下划线
    const INPUT_SHOW_BRACKETS = 2;// 小括号
    const INPUT_SHOW_UNDERLINE_BRACKETS = 4;// 小括号 + 下划线

    const SUBJECT_ITEM_ANSWER_SPLIT = '||->'; // 导出、导入时 单选、多选、判断题时 选项 和  答案分隔符

    // --中间的数字5，代表指定书写位置预留的字符位个数。

    // 32填空题【确切答案】格式的分隔符
    // 下划线格式 : 墙面装饰分为[[{5}]]和[[{5}]]，不同的墙面有着不同的装饰效果和功能。
    //        =>    墙面装饰分为________和________，不同的墙面有着不同的装饰效果和功能。
    const SUBJECT_TYPE_BIG_SPLIT_BEGIN = '[[{';// 值前符号
    const SUBJECT_TYPE_BIG_SPLIT_MID = '<|=|>';// 值中间符号
    const SUBJECT_TYPE_BIG_SPLIT_END = '}]]';// 值后符号
    // 小括号格式  墙面装饰分为[[(5)]]和[[(5)]]，不同的墙面有着不同的装饰效果和功能。
    //         =》 墙面装饰分为(          )和(          )，不同的墙面有着不同的装饰效果和功能。
    const SUBJECT_TYPE_BRACKETS_SPLIT_BEGIN = '[[(';// 值前符号
    const SUBJECT_TYPE_BRACKETS_SPLIT_END = ')]]';// 值后符号

    // 小括号 + 下划线 格式  墙面装饰分为[([{5}])]和[([{5}])]，不同的墙面有着不同的装饰效果和功能。--- 必须第一个优先处理【因为包含了下划线的】
    //                  =》 墙面装饰分为(________)和(________)，不同的墙面有着不同的装饰效果和功能。
    const SUBJECT_TYPE_UNDERLINE_BRACKETS_SPLIT_BEGIN = '[([{';// 值前符号
    const SUBJECT_TYPE_UNDERLINE_BRACKETS_SPLIT_END = '}])]';// 值后符号

    // 所有分隔符--数组
    const SUBJECT_EG_FORMAT = [
        'splitItemAnswer' => self::SUBJECT_ITEM_ANSWER_SPLIT,
        'splitBegin' => self::SUBJECT_TYPE_BIG_SPLIT_BEGIN ,
        'splitEnd' => self::SUBJECT_TYPE_BIG_SPLIT_END ,
        'splitMid' => self::SUBJECT_TYPE_BIG_SPLIT_MID,
        'bracketsBegin' => self::SUBJECT_TYPE_BRACKETS_SPLIT_BEGIN ,
        'bracketsEnd' => self::SUBJECT_TYPE_BRACKETS_SPLIT_END ,
        'bracketsUnderLineBegin' => self::SUBJECT_TYPE_UNDERLINE_BRACKETS_SPLIT_BEGIN ,
        'bracketsUnderLineEnd' => self::SUBJECT_TYPE_UNDERLINE_BRACKETS_SPLIT_END ,
//            'only' => '1+1=' . self::SUBJECT_TYPE_BIG_SPLIT_BEGIN . '2' . self::SUBJECT_TYPE_BIG_SPLIT_END,
//            'oneOfMany' => '1+1>=' . self::SUBJECT_TYPE_BIG_SPLIT_BEGIN . '0' . self::SUBJECT_TYPE_BIG_SPLIT_MID . '1' . self::SUBJECT_TYPE_BIG_SPLIT_MID . '2' . self::SUBJECT_TYPE_BIG_SPLIT_END,
    ];

    // 题目类型1单选；2多选；4判断；8问答【人工打分】；16填空题【人工打分】；32填空题【确切答案】格式：1+1=[[{2}]]；1+1>=[[{0<|=|>1<|=|>2}]]
    const SUBJECT_TYPE_RADIO = 1;// 单选
    const SUBJECT_TYPE_CHECKBOX = 2;// 多选
    const SUBJECT_TYPE_JUDGE = 4;// 判断
    const SUBJECT_TYPE_QA = 8;// 问答[人工阅]
    const SUBJECT_TYPE_COMPLETAION_HAND = 16;// 填空题[人工阅]
    const SUBJECT_TYPE_COMPLETION = 32;// 填空题
    const SUBJECT_TYPE_ARR = [
        self::SUBJECT_TYPE_RADIO => '单选',
        self::SUBJECT_TYPE_CHECKBOX => '多选',
        self::SUBJECT_TYPE_JUDGE => '判断',
        self::SUBJECT_TYPE_COMPLETION => '填空题[确切答案]',
        self::SUBJECT_TYPE_COMPLETAION_HAND => '填空题[人工批阅]',
        self::SUBJECT_TYPE_QA => '问答[人工批阅]',
    ];

    // 正确答案;4判断时：1正确2错误
    const ANSWER_RIGHT = 1;// 正确
    const ANSWER_ERROR = 2;// 错误
    const ANSWER_ARR = [
        self::ANSWER_RIGHT => '正确',
        self::ANSWER_ERROR => '错误',
    ];
    ...
    // 表里没有的字段
    protected $appends = ['subject_type_text', 'answer_text', 'open_status_text'];
     ...
     */
    public static function replace_special_char($string, $replace_type = 1){
...
//        $string = htmlspecialchars($string);
    ...
    {
        return Tool::getBitVals(static::SUBJECT_TYPE_ARR, $this->subject_type, '、');
        // return static::SUBJECT_TYPE_ARR[$this->subject_type] ?? '';
    }
    ...
    {
        return Tool::getBitVals(static::ANSWER_ARR, $this->answer, '、');
        // return static::ANSWER_ARR[$this->answer] ?? '';
    }
    ...
    {
        return $this->hasMany('App\Models\QualityControl\CompanySubjectAnswer', 'subject_id', 'id');
    }
    ...
    {
        return $this->hasMany('App\Models\QualityControl\CompanyPaperSubject', 'subject_id', 'id');
    }

    /**
     * 获取试题分析-二维
     */
//    public function subjectAnalyse()
//    {
//        return $this->hasMany('App\Models\QualityControl\CompanySubjectAnalyse', 'subject_id', 'id');
//    }
    ...
    public function subjectAnalyse()
    {
        return $this->hasOne('App\Models\QualityControl\CompanySubjectAnalyse', 'subject_id', 'id');
    }
}
