...
class VodVideo extends BasePublicModel
{
    //****************数据据缓存**相关的***开始********************************************
...
//    public static $primaryKeyValType = 1;
    ...
    // 其它具体的类可以在自己的类中 继承或 重写此常量 -- 只对外提供使用
     const ORDER_BY = ['vod_id' => 'desc', 'sort_num' => 'desc', 'id' => 'asc'];// ['sort_num' => 'desc', 'id' => 'desc'];// ['id' => 'desc'];
     ...
     * @var string
     */
    protected $table = 'vod_video';

    // 是否视频1目录；2视频
    public static $isVideoArr = [
        '1' => '章节',
        '2' => '视频',
    ];
    ...
    // 状态(1正常(上架中)  2下架)
    public static $statusOnlineArr = [
        '1' => '已上架',
        '2' => '已下架',
    ];

    // 表里没有的字段
    protected $appends = ['is_video_text', 'video_type_text', 'status_online_text'];
     ...
     * @return string
     */
    public function getIsVideoTextAttribute()
    {
        return static::$isVideoArr[$this->is_video] ?? '';
    }
    ...
    {
        return static::$videoTypeArr[$this->video_type] ?? '';
    }
     ...
     * @return string
     */
    public function getStatusOnlineTextAttribute()
    {
        return static::$statusOnlineArr[$this->status_online] ?? '';
    }

    /**
     * 获取点播课程评论-二维
     */
    public function vodComment()
    {
        return $this->hasMany('App\Models\QualityControl\VodComment', 'vod_video_id', 'id');
    }

    /**
     * 获取点播课程学员学习进度-二维
     */
    public function vodRate()
    {
        return $this->hasMany('App\Models\QualityControl\VodRate', 'vod_video_id', 'id');
    }

    /**
     * 获取点播课程视频目录子节点-二维
     */
    public function vodVideoChilds()
    {
        return $this->hasMany('App\Models\QualityControl\VodVideo', 'parent_video_id', 'id');
    }

    /**
     * 获取点播课程视频目录父节点---一维
     */
    public function vodParentVideo()
    {
        return $this->hasOne('App\Models\QualityControl\VodVideo', 'parent_video_id', 'id');
    }
    ...
    {
        return $this->hasOne('App\Models\QualityControl\Vods', 'vod_id', 'id');
    }
}
