...
class Vods extends BasePublicModel
{
    //****************数据据缓存**相关的***开始********************************************
...
//    ];
    ...
    //****************数据据缓存**相关的***结束********************************************
     ...
     public static $IntPriceFields = ['price_member', 'price_general', 'sale_total'];//[有则设置] 表中整型表示价格的字段数组 -- 一维数组，目的：方便统一把数据中的字段转浮点数或转整数
...
//    public static $primaryKeyValType = 1;
    ...
    // 其它具体的类可以在自己的类中 继承或 重写此常量 -- 只对外提供使用
     const ORDER_BY = ['recommend_status' => 'desc', 'id' => 'desc'];// ['sort_num' => 'desc', 'id' => 'desc'];// ['id' => 'desc'];
    ...
    // 是否推荐1非推荐--热门2推荐
    public static $recommendStatusArr = [
        '2' => '推荐',
        '1' => '热门',
    ];

    // 表里没有的字段
    protected $appends = ['open_status_text', 'recommend_status_text', 'effectively_day_text'];
    ...
    {
        return static::$recommendStatusArr[$this->recommend_status] ?? '';
    }
     ...
     * @return string
     */
    public function getEffectivelyDayTextAttribute()
    {
        $effectively_day = $this->effectively_day;
        if($effectively_day <= 0 ) return '长期有效';
        return $effectively_day . '天';
    }
    ...
    {
        return $this->hasMany('App\Models\QualityControl\VodVideo', 'vod_id', 'id');
    }

    /**
     * 获取点播课程订单-二维
     */
    public function vodOrders()
    {
        return $this->hasMany('App\Models\QualityControl\VodOrders', 'vod_id', 'id');
    }
    ...
    {
        return $this->hasOne('App\Models\QualityControl\VodType', 'vod_type_id', 'id');
    }

    /**
     * 获取点播课程内容---一维
     */
    public function vodsContent()
    {
        return $this->hasOne('App\Models\QualityControl\VodsContent', 'vod_id', 'id');
    }
}
