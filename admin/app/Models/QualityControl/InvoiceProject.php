...
class InvoiceProject extends BasePublicModel
{
    //****************数据据缓存**相关的***开始********************************************
...
//    ];
    //****************数据据缓存**相关的***结束********************************************
     ...
     public static $IntPriceFields = ['xmdj', 'xmje', 'sl', 'se'];//[有则设置] 表中整型表示价格的字段数组 -- 一维数组，目的：方便统一把数据中的字段转浮点数或转整数
     public static $IntPriceIndex = ['xmdj' => 6];// $IntPriceFields 字段对应的扩大陪数；没有配置默认为2  扩大或缩小倍数： pow(10,2)；格式：['sl' => 2,]；注：2的可不用配置【只配置特殊的】，因为默认就是2
...
//    public static $primaryKeyValType = 1;
    ...
    // 优惠政策标识 0：不使用，1：使用
    public static $yhzcbsArr = [
        '0' => '不使用',
        '1' => '使用',
    ];

    // 零税率标识 空：非零税率， 1：免税，2：不征收，3普通零税率
    public static $lslbsArr = [
        '' => '非零税率',
        '1' => '免税',
        '2' => '不征收',
        '3' => '普通零税率',
    ...
    // 表里没有的字段
    protected $appends = ['fphxz_text', 'yhzcbs_text', 'lslbs_text'];
     ...
     * @return string
     */
    public function getFphxzTextAttribute()
    {
        return static::$fphxzArr[$this->fphxz] ?? '';
    }
     ...
     * @return string
     */
    public function getYhzcbsTextAttribute()
    {
        return static::$yhzcbsArr[$this->yhzcbs] ?? '';
    }
     ...
     * @return string
     */
    public function getLslbsTextAttribute()
    {
        return static::$lslbsArr[$this->lslbs] ?? '';
    }
    ...
    {
        return $this->belongsTo('App\Models\QualityControl\Invoices', 'invoice_id', 'id');
    }

    /**
     * 获取发票商品项目模板-一维
     */
    public function invoiceProjectTemplate()
    {
        return $this->belongsTo('App\Models\QualityControl\InvoiceProjectTemplate', 'invoice_project_template_id', 'id');
    }

    /**
     * 获取发票商品项目模板历史-一维
     */
    public function invoiceProjectTemplateHistory()
    {
        return $this->belongsTo('App\Models\QualityControl\InvoiceProjectTemplateHistory', 'invoice_project_template_id_history', 'id');
    }
}
