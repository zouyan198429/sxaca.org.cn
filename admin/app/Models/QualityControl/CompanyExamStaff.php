...
class CompanyExamStaff extends BasePublicModel
{
    //****************数据据缓存**相关的***开始********************************************
...
//    public static $primaryKeyValType = 1;
    ...
    // status_no 状态1待审核64审核未通过4等考试8考试中16已考试32缺考
    const STATUS_NO_WAIT_CHECK = 1;
    const STATUS_NO_UNCHECK = 64;
    const STATUS_NO_WAIT = 4;
    const STATUS_NO_DOING = 8;
    const STATUS_NO_DOED = 16;
    const STATUS_NO_MISS = 32;
    const STATUS_NO_ARR = [
        self::STATUS_NO_WAIT_CHECK => '待审核',
        self::STATUS_NO_UNCHECK => '审核未通过',
    ...
    const REVIEW_STATUS_NOT = CompanyExam::REVIEW_STATUS_NOT;
    const REVIEW_STATUS_WAIT = CompanyExam::REVIEW_STATUS_WAIT;
    const REVIEW_STATUS_DOING = CompanyExam::REVIEW_STATUS_DOING;
    const REVIEW_STATUS_DOED = CompanyExam::REVIEW_STATUS_DOED;
    const REVIEW_STATUS_ARR = CompanyExam::REVIEW_STATUS_ARR;

    // 表里没有的字段
    protected $appends = ['status_no_text', 'review_status_text'];
    ...
    {
        return $this->hasOne('App\Models\QualityControl\CompanyExam', 'exam_id', 'id');
    }
}
