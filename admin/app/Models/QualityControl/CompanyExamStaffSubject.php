...
class CompanyExamStaffSubject extends BasePublicModel
{
    //****************数据据缓存**相关的***开始********************************************
...
//    public static $primaryKeyValType = 1;
     ...
     * @var string
     */
    protected $table = 'company_exam_staff_subject';

    // 是否正确答案2错误1正确；4部分得分
    const IS_RIGHT_RIGHT = 1;// 正确
    const IS_RIGHT_ERROR = 2;// 错误
    const IS_RIGHT_PART = 4;// 部分得分
    const IS_RIGHT_ARR = [
        self::IS_RIGHT_RIGHT => '正确',
        self::IS_RIGHT_ERROR => '错误',
        self::IS_RIGHT_PART => '部分得分',
    ];
    ...
    // 表里没有的字段
    protected $appends = ['is_right_text', 'review_status_text'];
     ...
     * @return string
     */
    public function getIsRightTextAttribute()
    {
        return Tool::getBitVals(static::IS_RIGHT_ARR, $this->is_right, '、');
        // return static::IS_RIGHT_ARR[$this->is_right] ?? '';
    }
