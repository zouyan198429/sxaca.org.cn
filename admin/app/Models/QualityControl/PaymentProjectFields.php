...
class PaymentProjectFields extends BasePublicModel
{
    //****************数据据缓存**相关的***开始********************************************
...
//    public static $primaryKeyValType = 1;
     ...
     * @var string
     */
    protected $table = 'payment_project_fields';


//    标示字段值文字  -- 样例
    // 是否必填1可填；2必填
    const REQUIRED_STATUS_NOT_REQUIRED = 1;// 可填
    const REQUIRED_STATUS_REQUIRED = 2;// 必填
    const REQUIRED_STATUS_ARR = [
        self::REQUIRED_STATUS_NOT_REQUIRED => '可填',
        self::REQUIRED_STATUS_REQUIRED => '必填',
    ];

    // 填写终端1客户端；2企业端
    const INPUT_STATUS_USER_INPUT = 1;// 客户端
    const INPUT_STATUS_COMPANY_INPUT = 2;// 企业端
    const INPUT_STATUS_ARR = [
        self::INPUT_STATUS_USER_INPUT => '用户端',
        self::INPUT_STATUS_COMPANY_INPUT => '后台端',
    ];

    // 显示终端1客户端；2企业端
    const SHOW_STATUS_USER = 1;// 用户端
    const SHOW_STATUS_COMPANY = 2;// 后台端
    const SHOW_STATUS_ARR = [
        self::SHOW_STATUS_USER => '用户端',
        self::SHOW_STATUS_COMPANY => '后台端',
    ];

    // 字段值类型1输入框 ；2多行文本 ； 4富文本；8单选框；16复选框
    const VAL_TYPE_INPUT = 1;// 输入框
    const VAL_TYPE_TEXTAREA = 2;// 多行文本
    const VAL_TYPE_RICHTEXT = 4;// 富文本
    const VAL_TYPE_RADIO = 8;// 单选框
    const VAL_TYPE_CHECKBOX = 16;// 复选框
    const VAL_TYPE_ARR = [
        self::VAL_TYPE_INPUT => '文本框',
        self::VAL_TYPE_TEXTAREA => '多行文本',
        self::VAL_TYPE_RICHTEXT => '富文本',
        self::VAL_TYPE_RADIO => '单选框',
        self::VAL_TYPE_CHECKBOX => '复选框',
    ];

    // 表里没有的字段
    protected $appends = ['required_status_text', 'input_status_text', 'show_status_text', 'val_type_text'];
     ...
     * @return string
     */
    public function getRequiredStatusTextAttribute()
    {
        return Tool::getBitVals(static::REQUIRED_STATUS_ARR, $this->required_status, '、');
        // return static::REQUIRED_STATUS_ARR[$this->required_status] ?? '';
    }
     ...
     * @return string
     */
    public function getInputStatusTextAttribute()
    {
        return Tool::getBitVals(static::INPUT_STATUS_ARR, $this->input_status, '、');
        // return static::INPUT_STATUS_ARR[$this->input_status] ?? '';
    }
     ...
     * @return string
     */
    public function getShowStatusTextAttribute()
    {
        return Tool::getBitVals(static::SHOW_STATUS_ARR, $this->show_status, '、');
        // return static::SHOW_STATUS_ARR[$this->show_status] ?? '';
    }
     ...
     * @return string
     */
    public function getValTypeTextAttribute()
    {
        return Tool::getBitVals(static::VAL_TYPE_ARR, $this->val_type, '、');
        // return static::VAL_TYPE_ARR[$this->val_type] ?? '';
    }
    /**
     * 获取付款/收款记录字段-二维
     */
    ...
    {
        return $this->hasMany('App\Models\QualityControl\PaymentRecordFields', 'payment_project_fields_id', 'id');
    }
