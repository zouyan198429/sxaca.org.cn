...
class CompanyPaper extends BasePublicModel
{
    //****************数据据缓存**相关的***开始********************************************
...
//    public static $primaryKeyValType = 1;
    ...
    // subject_order_type 试题顺序1固定顺序2随机顺序
    const SUBJECT_ORDER_TYPE_FIXED = 1;
    const SUBJECT_ORDER_TYPE_RANDOM = 2;
    const SUBJECT_ORDER_TYPE_ARR = [
        self::SUBJECT_ORDER_TYPE_FIXED => '固定顺序',
        self::SUBJECT_ORDER_TYPE_RANDOM => '随机顺序',
    ];

    // has_block 是否设置题目1无题目；2有题目
    const HAS_BLOCK_NO = 1;
    const HAS_BLOCK_YES = 2;
    const HAS_BLOCK_ARR = [
        self::HAS_BLOCK_NO => '无题目',
        self::HAS_BLOCK_YES => '有题目',
    ];

    // subject_updated 试题是否有更新1没有 ；2有更新【修改或删除】--需要人工去判断是否更新过来;4有更新【删除】
    const SUBJECT_UPDATED_NO = 1;
    const SUBJECT_UPDATED_YES = 2;
    const SUBJECT_UPDATED_DELED = 4;
    const SUBJECT_UPDATED_ARR = [
        self::SUBJECT_UPDATED_NO => '无更新',
        self::SUBJECT_UPDATED_YES => '更新-改',
        self::SUBJECT_UPDATED_DELED => '更新-删',
    ];

    // review_method 批卷方式1自动批卷；2人工批卷--有需要人工批卷的分类试题
    const REVIEW_METHOD_AUTO = 1;
    const REVIEW_METHOD_HAND = 2;
    const REVIEW_METHOD_ARR = [
        self::REVIEW_METHOD_AUTO => '自动批卷',
        self::REVIEW_METHOD_HAND => '人工批卷',
    ];
    ...
    // 表里没有的字段
    protected $appends = ['subject_order_type_text', 'has_block_text', 'subject_updated_text', 'review_method_text', 'open_status_text'];
     ...
     * @return string
     */
    public function getSubjectOrderTypeTextAttribute()
    {
        return Tool::getBitVals(static::SUBJECT_ORDER_TYPE_ARR, $this->subject_order_type, '、');
        // return static::SUBJECT_ORDER_TYPE_ARR[$this->subject_order_type] ?? '';
    }
     ...
     * @return string
     */
    public function getHasBlockTextAttribute()
    {
        return Tool::getBitVals(static::HAS_BLOCK_ARR, $this->has_block, '、');
        // return static::HAS_BLOCK_ARR[$this->has_block] ?? '';
    }
     ...
     * @return string
     */
    public function getSubjectUpdatedTextAttribute()
    {
        return Tool::getBitVals(static::SUBJECT_UPDATED_ARR, $this->subject_updated, '、');
        // return static::SUBJECT_UPDATED_ARR[$this->subject_updated] ?? '';
    }
    ...
    {
        return $this->hasMany('App\Models\QualityControl\CompanyPaperCategory', 'paper_id', 'id');
    }
    ...
    {
        return $this->hasMany('App\Models\QualityControl\CompanyPaperSubject', 'paper_id', 'id');
    }
    ...
    {
        return $this->hasMany('App\Models\QualityControl\CompanyExam', 'paper_id', 'id');
    }
}
