...
class PaymentRecord extends BasePublicModel
{
    //****************数据据缓存**相关的***开始********************************************
...
//    ];
    ...
    //****************数据据缓存**相关的***结束********************************************
     ...
     public static $IntPriceFields = ['pay_amount', 'discount_amount', 'real_amount', 'wait_refund_amount', 'refunded_amount', 'final_amount'];//[有则设置] 表中整型表示价格的字段数组 -- 一维数组，目的：方便统一把数据中的字段转浮点数或转整数
...
//    标示字段值文字  -- 样例
    ...
    const PAY_STATUS_WAIT = 1;// 待缴费
    const PAY_STATUS_PART_PAY = 2;// 部分缴费
    const PAY_STATUS_PAYED = 4;// 已缴费
    const PAY_STATUS_PART_REFUND = 8;// 部分退费
    const PAY_STATUS_PART_REFUNDED = 16;// 已退费
    const PAY_STATUS_ARR = [
        self::PAY_STATUS_WAIT => '待缴费',
        self::PAY_STATUS_PART_PAY => '部分缴费',
        self::PAY_STATUS_PAYED => '已缴费',
        self::PAY_STATUS_PART_REFUND => '部分退费',
        self::PAY_STATUS_PART_REFUNDED => '已退费',
    ];

    // 记录状态1正常4已作废8已到期
    const RECORD_STATUS_NORMAL = 1;// 正常
    const RECORD_STATUS_CANCEL = 4;// 已作废
    const RECORD_STATUS_EXPIRE = 8;// 已到期
    const RECORD_STATUS_ARR = [
        self::RECORD_STATUS_NORMAL => '正常',
        self::RECORD_STATUS_CANCEL => '已作废',
        self::RECORD_STATUS_EXPIRE => '已到期',
    ];

    // 有效时长1长期有效【可再次付费】；2长期有效【不可再次付费】；4指定有效时长
    const VALID_LIMIT_FOREVER_REPEAT = PaymentProject::VALID_LIMIT_FOREVER_REPEAT;// 长期有效【可再次付费】
    const VALID_LIMIT_FIXED_NOT_REPEAT = PaymentProject::VALID_LIMIT_FIXED_NOT_REPEAT;// 长期有效【不可再次付费】
    const VALID_LIMIT_FIXED = PaymentProject::VALID_LIMIT_FIXED;// 指定有效时长
    const VALID_LIMIT_ARR = PaymentProject::VALID_LIMIT_ARR;

    // 记录处理状态1待处理；2处理中；4已处理
    const HANDLE_STATUS_WAIT = 1;// 待处理
    const HANDLE_STATUS_DOING = 2;// 处理中
    const HANDLE_STATUS_DOED = 4;// 已处理
    const HANDLE_STATUS_ARR = [
        self::HANDLE_STATUS_WAIT => '待处理',
        self::HANDLE_STATUS_DOING => '处理中',
        self::HANDLE_STATUS_DOED => '已处理',
    ];

    // 表里没有的字段
    protected $appends = ['pay_status_text', 'record_status_text', 'valid_limit_text', 'handle_status_text'];
    ...
    {
        return Tool::getBitVals(static::RECORD_STATUS_ARR, $this->record_status, '、');
        // return static::RECORD_STATUS_ARR[$this->record_status] ?? '';
    }
     ...
     * @return string
     */
    public function getHandleStatusTextAttribute()
    {
        return Tool::getBitVals(static::HANDLE_STATUS_ARR, $this->handle_status, '、');
        // return static::HANDLE_STATUS_ARR[$this->handle_status] ?? '';
    }
    ...
    {
        return $this->hasMany('App\Models\QualityControl\PaymentRecordFlow', 'payment_record_id', 'id');
    }
    ...
    {
        return $this->hasMany('App\Models\QualityControl\PaymentRecordFields', 'payment_record_id', 'id');
    }
    ...
    {
        return $this->hasMany('App\Models\QualityControl\PaymentRecordLog', 'payment_record_id', 'id');
    }
    ...
    {
        return $this->hasOne('App\Models\QualityControl\PaymentProject', 'payment_project_id', 'id');
    }
}
