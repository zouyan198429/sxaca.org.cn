...
class PaymentRecordFields extends BasePublicModel
{
    //****************数据据缓存**相关的***开始********************************************
...
//    public static $primaryKeyValType = 1;
     ...
     * @var string
     */
    protected $table = 'payment_record_fields';

//    标示字段值文字  -- 样例
    // 字段值类型1输入框 ；2多行文本 ； 4富文本；8单选框；16复选框
    const VAL_TYPE_INPUT = PaymentProjectFields::VAL_TYPE_INPUT;// 输入框
    const VAL_TYPE_TEXTAREA = PaymentProjectFields::VAL_TYPE_TEXTAREA;// 多行文本
    const VAL_TYPE_RICHTEXT = PaymentProjectFields::VAL_TYPE_RICHTEXT;// 富文本
    const VAL_TYPE_RADIO = PaymentProjectFields::VAL_TYPE_RADIO;// 单选框
    const VAL_TYPE_CHECKBOX = PaymentProjectFields::VAL_TYPE_CHECKBOX;// 复选框
    const VAL_TYPE_ARR = PaymentProjectFields::VAL_TYPE_ARR;

    // 表里没有的字段
    protected $appends = ['val_type_text'];
    ...
    {
        return $this->hasOne('App\Models\QualityControl\PaymentRecord', 'payment_record_id', 'id');
    }
    ...
    {
        return $this->hasOne('App\Models\QualityControl\PaymentProjectFields', 'payment_project_fields_id', 'id');
    }
}
