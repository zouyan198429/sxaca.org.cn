...
class Invoices extends BasePublicModel
{
    //****************数据据缓存**相关的***开始********************************************
...
//    ];
    //****************数据据缓存**相关的***结束********************************************
     ...
     public static $IntPriceFields = ['kce', 'hjse', 'hjje', 'jshj'];//[有则设置] 表中整型表示价格的字段数组 -- 一维数组，目的：方便统一把数据中的字段转浮点数或转整数
...
//    public static $primaryKeyValType = 1;
    ...
    // 开票类型 0-蓝字发票；1-红字发票
    public static $kplxArr = [
        '0' => '蓝字发票',
        '1' => '红字发票',
    ];

    // 开票服务商1沪友
    public static $invoiceServiceArr = [
        '1' => '沪友电子发票',
    ];
    ...
    // 表里没有的字段
    protected $appends = ['invoice_status_text', 'upload_status_text', 'kplx_text', 'invoice_service_text', 'itype_text', 'tspz_text', 'zsfs_text'];
     ...
     * @return string
     */
    public function getKplxTextAttribute()
    {
        return static::$kplxArr[$this->kplx] ?? '';
    }
     ...
     * @return string
     */
    public function getItypeTextAttribute()
    {
        return static::$itypeArr[$this->itype] ?? '';
    }
     ...
     * @return string
     */
    public function getTspzTextAttribute()
    {
        return static::$tspzArr[$this->tspz] ?? '';
    }
    ...
    {
        return $this->hasMany('App\Models\QualityControl\InvoiceProject', 'invoice_id', 'id');
    }
    ...
    {
        return $this->belongsTo('App\Models\QualityControl\OrderFlow', 'order_no', 'order_no');
    }
    ...
    {
        return $this->belongsTo('App\Models\QualityControl\InvoiceBuyer', 'invoice_buyer_id', 'id');
    }

    /**
     * 获取发票配置购买方历史-一维
     */
    public function invoiceBuyerHistory()
    {
        return $this->belongsTo('App\Models\QualityControl\InvoiceBuyerHistory', 'invoice_buyer_id_history', 'id');
    }
    ...
    {
        return $this->belongsTo('App\Models\QualityControl\InvoiceSeller', 'invoice_seller_id', 'id');
    }

    /**
     * 获取发票配置销售方历史-一维
     */
    public function invoiceSellerHistory()
    {
        return $this->belongsTo('App\Models\QualityControl\InvoiceSellerHistory', 'invoice_seller_id_history', 'id');
    }
    ...
    {
        return $this->belongsTo('App\Models\QualityControl\InvoiceTemplate', 'invoice_template_id', 'id');
    }

    /**
     * 获取发票开票模板历史-一维
     */
    public function invoiceTemplateHistory()
    {
        return $this->belongsTo('App\Models\QualityControl\InvoiceTemplateHistory', 'invoice_template_id_history', 'id');
    }

    /**
     * 获取蓝票对应的红票---一维
     */
    public function invoiceRed()
    {
        return $this->hasOne('App\Models\QualityControl\Invoices', 'yfp_hm', 'fp_hm');
    }

    /**
     * 获取红票所属的蓝票--一维
     */
    public function invoiceBlue()
    {
