...
class PaymentRecordFlow extends BasePublicModel
{
    //****************数据据缓存**相关的***开始********************************************
...
//    ];
    ...
    //****************数据据缓存**相关的***结束********************************************
     ...
     public static $IntPriceFields = ['pay_price'];//[有则设置] 表中整型表示价格的字段数组 -- 一维数组，目的：方便统一把数据中的字段转浮点数或转整数
...
//    public static $primaryKeyValType = 1;
     ...
     * @var string
     */
    protected $table = 'payment_record_flow';

//    标示字段值文字  -- 样例
    // 支付类型1付款2退款
    const PAY_TYPE_PAYED = 1;// 付款
    const PAY_TYPE_REFUND = 2;// 退款
    const PAY_TYPE_ARR = [
        self::PAY_TYPE_PAYED => '付款',
        self::PAY_TYPE_REFUND => '退款',
    ];
    ...
    {
        return Tool::getBitVals(static::PAY_TYPE_ARR, $this->pay_type, '、');
        // return static::PAY_TYPE_ARR[$this->pay_type] ?? '';
    }
