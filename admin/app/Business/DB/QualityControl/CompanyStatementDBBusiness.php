 ...
 *
 */
class CompanyStatementDBBusiness extends BasePublicDBBusiness
{
...
//                }
                ...
                ResourceDBBusiness::resourceSync(static::thisObj(), 16, $id, $resourceIds, [], $operate_staff_id, $operate_staff_id_history);
            }
            ...
            if(!$isModify){
                ...
                if(!$isBatchOperate && is_numeric($resultDatas['company_id']) && $resultDatas['company_id'] > 0){
                    StaffDBBusiness::updateStatementNum($resultDatas['company_id']);
                }
            }else if($forceCompanyNum && !empty($companyNumIds)){// 修改时 需要强制更新企业能力范围数量
                StaffDBBusiness::updateStatementNum($companyNumIds);
            }
        });
        // ************************************************
        // 保存成功后的处理
     ...
     * @author zouyan(305463219@qq.com)
     */
    public static function getCompanyStatementCount($company_id = 0){
        // 更新班级老师人数
//        $queryParams = [
//            'where' => [
//                ['company_id', $company_id],
...
//        }
        CommonDB::doTransactionFun(function() use( &$id, &$organize_id, &$organizeIds, &$doOperate){
            // 删除资源及文件
            ResourceDBBusiness::delResourceByIds(static::thisObj(), $id, 16, $doOperate);
...
//            ];
            ...
            if(!empty($organizeIds)){
                foreach($organizeIds as $organizeId){
                    // 根据企业id更企业能力范围数
                    StaffDBBusiness::updateStatementNum($organizeId);;
                }
            }
        });
        return $id;
