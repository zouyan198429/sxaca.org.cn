 ...
 *
 */
class PaymentRecordFieldsDBBusiness extends BasePublicDBBusiness
{
...
//        }
        ...
        CommonDB::doTransactionFun(function() use(&$saveData, &$company_id, &$id, &$operate_staff_id, &$modifAddOprate, &$operate_staff_id_history, &$modelObj, &$isModify){
            ...
            // 如果是新的字段，则需要加入字段历史id
            if(!$isModify && isset($saveData['payment_project_fields_id']) && $saveData['payment_project_fields_id'] > 0){
                $saveData['payment_project_fields_id_history'] = PaymentProjectFieldsDBBusiness::getIdHistory($saveData['payment_project_fields_id']);
            }
            // 如果新加时：有项目id,且没有对应的历史id时
            if(!$isModify && isset($saveData['payment_project_id']) && $saveData['payment_project_id'] > 0 && (!isset($saveData['payment_project_id_history']) || $saveData['payment_project_id_history'] <= 0)){
                $saveData['payment_project_id_history'] = PaymentProjectDBBusiness::getIdHistory($saveData['payment_project_id']);
            }
