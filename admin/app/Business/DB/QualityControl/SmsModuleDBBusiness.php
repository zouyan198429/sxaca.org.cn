 ...
 *
 */
class SmsModuleDBBusiness extends BasePublicDBBusiness
{
...
//        }
        ...
        // 参数
        $sms_module_params = [];
        $has_sms_module_params = false;// 是否有 false:没有 ； true:有
        Tool::getInfoUboundVal($saveData, 'sms_module_params', $has_sms_module_params, $sms_module_params, 1);
        ...
        CommonDB::doTransactionFun(function() use(&$saveData, &$company_id, &$id, &$operate_staff_id, &$modifAddOprate, &$operate_staff_id_history, &$modelObj, &$isModify,
            $sms_module_params, $has_sms_module_params){
            ...
            // 短信模板所属模块参数 修改
            if($has_sms_module_params){
                $sms_module_params = nullToEmpty($sms_module_params);// 将数据中的null，改为空
                $module_params_ids = SmsModuleParamsDBBusiness::updateByDataList(['module_id' => $id], ['module_id' => $id]
                    , $sms_module_params, $isModify, $operate_staff_id, $operate_staff_id_history
    ...
    public static function delById($company_id, $id, $operate_staff_id = 0, $modifAddOprate = 0, $extendParams = []){
        ...
        return CommonDB::doTransactionFun(function() use(&$company_id, &$id, &$operate_staff_id, &$modifAddOprate, &$extendParams){
            // 删除短信模板所属模块参数
            $delQueryParams = Tool::getParamQuery(['module_id' => $id],[], []);
            SmsModuleParamsDBBusiness::del($delQueryParams);
            // 删除记录
