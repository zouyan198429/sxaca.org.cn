 ...
 *
 */
class VodTypeDBBusiness extends BasePublicDBBusiness
{
...
//                }
                ...
                ResourceDBBusiness::resourceSync(static::thisObj(), 4096, $id, $resourceIds, [], $operate_staff_id, $operate_staff_id_history);
            }
    ...
    public static function delById($company_id, $id, $operate_staff_id = 0, $modifAddOprate = 0, $extendParams = []){
        ...
        // 判断是不是可以删除--有课程就不能删除
        $vodsList = VodsDBBusiness::getDBFVFormatList(1, 1, ['vod_type_id' => $id],false);
        if(!empty($vodsList)){
            // $vodsFormatList = Tool::arrUnderReset($vodsList, 'vod_type_id', 2, '_');
            $vodTypeIds = Tool::getArrFields($vodsList, 'vod_type_id');
        ...
        return CommonDB::doTransactionFun(function() use(&$company_id, &$id, &$operate_staff_id, &$modifAddOprate, &$extendParams){
            // 删除资源及文件
            ResourceDBBusiness::delResourceByIds(static::thisObj(), $id, 4096);
            // 删除记录
