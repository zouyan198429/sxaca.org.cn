 ...
 *
 */
class CompanyPaperSubjectDBBusiness extends BasePublicDBBusiness
{
    ...
    // 注：历史表不要重置此属性
    public static $ignoreFields = ['paper_subject_id'];
    // 获得记录历史id
    public static function getIdHistory($mainId = 0, &$mainDBObj = null, &$historyDBObj = null){
        // $mainDBObj = null ;
        // $historyDBObj = null ;
        return static::getHistoryId($mainDBObj, $mainId, CompanyPaperSubjectHistoryDBBusiness::$model_name
            , CompanyPaperSubjectHistoryDBBusiness::$table_name, $historyDBObj, ['paper_subject_id' => $mainId], static::$ignoreFields);
    }
    ...
    public static function compareHistory($id = 0, $forceIncVersion = 0, &$mainDBObj = null, &$historyDBObj = null){
        ...
        // $mainDBObj = null ;
        // $historyDBObj = null ;
        return static::compareHistoryOrUpdateVersion($mainDBObj, $id, CompanyPaperSubjectHistoryDBBusiness::$model_name
            , CompanyPaperSubjectHistoryDBBusiness::$table_name, $historyDBObj, $historySearch, static::$ignoreFields, $forceIncVersion);
    }
...
//            }
     ...
     * 判断是否已经使用
     *
     * @param int/string/array $type_no $subject_ids 试题id 多个 ：数组或逗号分隔的字符串
     * @param int  $company_id 企业id
     * @param int  $used_id 已经使用的主记录id
     * @return  boolean 已经使用 true; 未使用false
     * @author zouyan(305463219@qq.com)
     */
    public static function judgeNoUsed($subject_ids, $company_id = 0, &$used_id = 0){

        // 判断在试卷试题中是否使用
        // ----优化，如果读所有的可能太多，所以先读一条记录
        $subjectInfo = static::getDBFVFormatList(4, 1, ['subject_id' => $subject_ids], false, [], [// , 'company_id' => $company_id
//            'sqlParams' => [
//                'where' => [
//                    ['type_no', '&' , $type_no . ' > 0'],
//                ]
//            ]
        ...
        $used_id = $subjectInfo['subject_id'] ?? 0;
