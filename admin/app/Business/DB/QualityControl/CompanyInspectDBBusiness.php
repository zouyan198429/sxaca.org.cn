 ...
 *
 */
class CompanyInspectDBBusiness extends BasePublicDBBusiness
{
    ...
    public static function replaceById($saveData, $company_id, &$id, $operate_staff_id = 0, $modifAddOprate = 0){
        ...
        if(isset($saveData['exist_problem']) && empty($saveData['exist_problem'])  ){
            throws('存在问题不能为空！');
        }
...
//        }
        ...
        CommonDB::doTransactionFun(function() use(&$saveData, &$company_id, &$id, &$operate_staff_id, &$modifAddOprate, &$operate_staff_id_history, &$modelObj, &$isModify
            ...
            if(!$isModify){
                ...
                if(!$isBatchOperate && is_numeric($resultDatas['company_id']) && $resultDatas['company_id'] > 0){
                    StaffDBBusiness::updateCompanyInspectNum($resultDatas['company_id']);
                }
            }else if($forceCompanyNum && !empty($companyNumIds)){// 修改时 需要强制更新企业能力范围数量
                StaffDBBusiness::updateCompanyInspectNum($companyNumIds);
            }
        });
        // ************************************************
        // 保存成功后的处理
     ...
     * @author zouyan(305463219@qq.com)
     */
    public static function getCompanyInspectCount($company_id = 0){
...
//            ];
            ...
            if(!empty($organizeIds)){
                foreach($organizeIds as $organizeId){
                    // 根据企业id更企业能力范围数
                    StaffDBBusiness::updateCompanyInspectNum($organizeId);;
                }
            }
        });
        return $id;
