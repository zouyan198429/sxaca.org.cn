 ...
 *
 */
class LaboratoryAddrDBBusiness extends BasePublicDBBusiness
{
...
//        }
        ...
        CommonDB::doTransactionFun(function() use(&$saveData, &$company_id, &$id, &$operate_staff_id, &$modifAddOprate, &$operate_staff_id_history, &$modelObj, &$isModify
            ...
            if(!$isModify){
                ...
                if(!$isBatchOperate && is_numeric($resultDatas['company_id']) && $resultDatas['company_id'] > 0){
                    StaffDBBusiness::updateLaboratoryNum($resultDatas['company_id']);
                }
            }else if($forceCompanyNum && !empty($companyNumIds)){// 修改时 需要强制更新企业能力范围数量
                StaffDBBusiness::updateLaboratoryNum($companyNumIds);
            }
        });
        // ************************************************
        // 保存成功后的处理
    ...
    /**
     * 根据企业id,获得企业的机构实验室地址数
     *
     * @param int  $company_id 企业id
     * @return  mixed 能力范围数
     * @author zouyan(305463219@qq.com)
     */
    public static function getLaboratoryAddrCount($company_id = 0){
        // 更新班级老师人数
//        $queryParams = [
//            'where' => [
//                ['company_id', $company_id],
...
//            ];
            ...
            if(!empty($organizeIds)){
                foreach($organizeIds as $organizeId){
                    // 根据企业id更企业能力范围数
                    StaffDBBusiness::updateLaboratoryNum($organizeId);;
                }
            }
        });
        return $id;
    ...
    public static function createOrOpenAddr($company_id, $addr, &$addr_id, $operate_staff_id = 0, $modifAddOprate = 0){
        if(!is_numeric($company_id) || $company_id <=0) return 0;
        $addrObj = null ;
        $searchConditon = [
            'company_id' => $company_id,
            'addr' => $addr,
