 ...
 *
 */
class CompanyPunishDBBusiness extends BasePublicDBBusiness
{
...
//                }
                // ResourceDBBusiness::bathResourceSync(static::thisObj(), 32, [['id' => $id, 'resourceIds' => $resourceIds, 'otherData' => []]], $operate_staff_id, $operate_staff_id_history);
                ResourceDBBusiness::resourceSync(static::thisObj(), 32, $id, $resourceIds, [], $operate_staff_id, $operate_staff_id_history);
            }
            ...
            if(!$isModify){
                ...
                if(!$isBatchOperate && is_numeric($resultDatas['company_id']) && $resultDatas['company_id'] > 0){
                    StaffDBBusiness::updatePunishNum($resultDatas['company_id']);
                }
            }else if($forceCompanyNum && !empty($companyNumIds)){// 修改时 需要强制更新企业能力范围数量
                StaffDBBusiness::updatePunishNum($companyNumIds);
            }
        });
        // ************************************************
        // 保存成功后的处理
     ...
     * @author zouyan(305463219@qq.com)
     */
    public static function getCompanyPunishCount($company_id = 0){
        // 更新班级老师人数
//        $queryParams = [
//            'where' => [
//                ['company_id', $company_id],
...
//            ];
            ...
            if(!empty($organizeIds)){
                foreach($organizeIds as $organizeId){
                    // 根据企业id更企业能力范围数
                    StaffDBBusiness::updatePunishNum($organizeId);;
                }
            }
        });
        return $id;
