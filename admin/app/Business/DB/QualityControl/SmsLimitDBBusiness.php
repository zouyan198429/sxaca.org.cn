 ...
 *
 */
class SmsLimitDBBusiness extends BasePublicDBBusiness
{
...
//        }
        ...
        // 限止键--唯一
        if( isset($saveData['limit_key']) && static::judgeFieldExist($company_id, $id ,"limit_key", $saveData['limit_key'], [],1)){
            throws('限止键已存在！');
        }

        // 限止键--唯一
        if( isset($saveData['limit_code']) && static::judgeFieldExist($company_id, $id ,"limit_code", $saveData['limit_code'], [],1)){
            throws('限止键已存在！');
        }
        if( isset($saveData['limit_code']) && !Tool::isBitNum($saveData['limit_code'], 63)){
            throws('限止键不是用效格式【1、2、4...】！');
        }
