 ...
 *
 */
class LiveNoticeDBBusiness extends BasePublicDBBusiness
{
...
//        }
        ...
        if(isset($saveData['notice_name']) && empty($saveData['notice_name'])  ){
            throws('公告名称不能为空！');
        }

        // 是否有详细内容
        $hasNoticeContent = false;
        $NoticeContent = '';
        Tool::getInfoUboundVal($saveData, 'notice_content', $hasNoticeContent, $NoticeContent, 1);
        ...
        CommonDB::doTransactionFun(function() use(&$saveData, &$company_id, &$id, &$operate_staff_id, &$modifAddOprate, &$operate_staff_id_history
            , &$modelObj, &$isModify, &$hasNoticeContent, &$NoticeContent ){
            ...
            if($hasNoticeContent) {
                $contentObj = null;
                LiveNoticeContentDBBusiness::updateOrCreate($contentObj, ['live_notice_id' => $id], ['notice_content' => $NoticeContent] );
            }
        });
        // ************************************************
        // 保存成功后的处理
    ...
    public static function delById($company_id, $id, $operate_staff_id = 0, $modifAddOprate = 0, $extendParams = []){
        ...
        return CommonDB::doTransactionFun(function() use(&$company_id, &$id, &$operate_staff_id, &$modifAddOprate, &$extendParams){
            // 删除内容
            $contentObj = null;
            LiveNoticeContentDBBusiness::deleteByIds($id, $contentObj,  'live_notice_id');
