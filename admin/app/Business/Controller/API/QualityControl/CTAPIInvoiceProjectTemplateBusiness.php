...
class CTAPIInvoiceProjectTemplateBusiness extends BasicPublicCTAPIBusiness
{
    public static $model_name = 'API\QualityControl\InvoiceProjectTemplateAPI';
...
//                static::getRelationSqlParams(['where' => [['admin_type', 2]]], $extendParams, 'company_info'), '', []),
    ...
    public static function getRelationConfigReturnData(Request $request, Controller $controller, $return_num = 0){
        ...
        if(($return_num & 2) == 2){// 给上一级返回下标名称 invoice_project_template_name => '模板名称'
            $one_field = ['key' => 'template_name', 'return_type' => 2, 'ubound_name' => 'invoice_project_template_name', 'split' => '、'];// 获得名称
    ...
    public static function joinListParams(Request $request, Controller $controller, &$queryParams, $notLog = 0){
        ...
        if(strlen($open_status) > 0 && !in_array($open_status, [0, '-1']))  Tool::appendParamQuery($queryParams, $open_status, 'open_status', [0, '0', ''], ',', false);

        $append_name = CommonRequest::get($request, 'append_name');
        if(strlen($append_name) > 0 && !in_array($append_name, [0, '-1']))  Tool::appendParamQuery($queryParams, $append_name, 'append_name', [0, '0', ''], ',', false);

        $merge_goods = CommonRequest::get($request, 'merge_goods');
        if(strlen($merge_goods) > 0 && !in_array($merge_goods, [0, '-1']))  Tool::appendParamQuery($queryParams, $merge_goods, 'merge_goods', [0, '0', ''], ',', false);
...
//        if(strlen($ids) > 0 && $ids != 0)  Tool::appendParamQuery($queryParams, $ids, 'id', [0, '0', ''], ',', false);
    ...
    public static function handleRelationDataFormat(Request $request, Controller $controller, &$main_list, &$data_list, $handleKeyArr, &$returnFields = []){
        ...
        if(in_array('priceIntToFloat', $handleKeyArr)){
            Tool::bathPriceCutFloatInt($data_list, InvoiceProjectTemplate::$IntPriceFields, InvoiceProjectTemplate::$IntPriceIndex, 2, 2);
        }
