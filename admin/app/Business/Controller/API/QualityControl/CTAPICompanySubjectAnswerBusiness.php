...
class CTAPICompanySubjectAnswerBusiness extends BasicPublicCTAPIBusiness
{
    public static $model_name = 'API\QualityControl\CompanySubjectAnswerAPI';
    public static $table_name = 'company_subject_answer';// 表名称
    public static $record_class = __CLASS__;// 当前的类名称 App\Business\***\***\**\***
    public static $orderBy = CompanySubjectAnswer::ORDER_BY;// ['sort_num' => 'desc', 'id' => 'desc'];// 默认的排序字段数组 ['id' => 'desc']--默认 或 ['sort_num' => 'desc', 'id' => 'desc']
...
//        if(strlen($bbbb) > 0 && !in_array($bbbb, [0, '-1']))  Tool::appendParamQuery($queryParams, $bbbb, 'bbbb', [0, '0', ''], ',', false);
        ...
        $answer_val = CommonRequest::get($request, 'answer_val');
        if(strlen($answer_val) > 0  && !in_array($answer_val, [0, '-1']))  Tool::appendCondition($queryParams, 'answer_val',  $answer_val . '=' . $answer_val, '&');
        ...
        if(strlen($is_right) > 0  && !in_array($is_right, [0, '-1']))  Tool::appendCondition($queryParams, 'is_right',  $is_right . '=' . $is_right, '&'
            , 'where', 1, false, CompanySubjectAnswer::IS_RIGHT_ARR);
