...
use App\Models\QualityControl\PaymentRecordFields;
...
class CTAPIPaymentRecordFieldsBusiness extends BasicPublicCTAPIBusiness
{
    public static $model_name = 'API\QualityControl\PaymentRecordFieldsAPI';
    public static $table_name = 'payment_record_fields';// 表名称
    public static $record_class = __CLASS__;// 当前的类名称 App\Business\***\***\**\***
    public static $orderBy = PaymentRecordFields::ORDER_BY;// ['sort_num' => 'desc', 'id' => 'desc'];// 默认的排序字段数组 ['id' => 'desc']--默认 或 ['sort_num' => 'desc', 'id' => 'desc']
...
//        if(strlen($bbbb) > 0 && !in_array($bbbb, [0, '-1']))  Tool::appendParamQuery($queryParams, $bbbb, 'bbbb', [0, '0', ''], ',', false);
        ...
        $payment_project_fields_id = CommonRequest::get($request, 'payment_project_fields_id');
        if(strlen($payment_project_fields_id) > 0 && !in_array($payment_project_fields_id, [0, '-1']))  Tool::appendParamQuery($queryParams, $payment_project_fields_id, 'payment_project_fields_id', [0, '0', ''], ',', false);

        $val_type = CommonRequest::get($request, 'val_type');
        if(strlen($val_type) > 0  && !in_array($val_type, [0, '-1']))  Tool::appendCondition($queryParams, 'val_type', $val_type . '=' . $val_type, '&', 'where', 1, false, PaymentRecordFields::VAL_TYPE_ARR); // --逻辑 & 操作，且有数组范围
