...
class CTAPIProjectStandardsBusiness extends BasicPublicCTAPIBusiness
{
    public static $model_name = 'API\QualityControl\ProjectStandardsAPI';
...
//                static::getRelationSqlParams(['where' => [['admin_type', 2]]], $extendParams, 'company_info'), '', []),
    ...
    public static function getRelationConfigReturnData(Request $request, Controller $controller, $return_num = 0){
        ...
        if(($return_num & 2) == 2){// 给上一级返回名称 project_standard_id_name = '项目名称' 下标
            $one_field = ['key' => 'name', 'return_type' => 2, 'ubound_name' => 'project_standard_id_name', 'split' => '、'];// 获得名称
            if(!isset($return_data['one_field'])) $return_data['one_field'] = [];
            array_push($return_data['one_field'], $one_field);
        }

        if(($return_num & 4) == 4){// 给上一级返回名称 project_standards_text = '项目名称1<br/>项目名称2' 下标
            $one_field = ['key' => 'name', 'return_type' => 2, 'ubound_name' => 'project_standards_text', 'split' => '<br/>'];// 获得名称
            if(!isset($return_data['one_field'])) $return_data['one_field'] = [];
            array_push($return_data['one_field'], $one_field);
        }

        if(($return_num & 8) == 8){// 给上一级返回 id 、 tag_name 的二维数组 . 如： project_standards = [[ 'id' => 26,  'tag_name' => '方法2'], ... ]
            $many_fields = [ 'ubound_name' => 'project_standards', 'fields_arr'=> ['id' => 'id', 'tag_name' => 'name'],'reset_ubound' => 2];
