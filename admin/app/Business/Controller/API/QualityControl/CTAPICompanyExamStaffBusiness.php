...
use App\Models\QualityControl\CompanyExamStaff;
...
class CTAPICompanyExamStaffBusiness extends BasicPublicCTAPIBusiness
{
    public static $model_name = 'API\QualityControl\CompanyExamStaffAPI';
    public static $table_name = 'company_exam_staff';// 表名称
    public static $record_class = __CLASS__;// 当前的类名称 App\Business\***\***\**\***
    public static $orderBy = CompanyExamStaff::ORDER_BY;// ['sort_num' => 'desc', 'id' => 'desc'];// 默认的排序字段数组 ['id' => 'desc']--默认 或 ['sort_num' => 'desc', 'id' => 'desc']
...
//        if(strlen($bbbb) > 0 && !in_array($bbbb, [0, '-1']))  Tool::appendParamQuery($queryParams, $bbbb, 'bbbb', [0, '0', ''], ',', false);
        ...
        $exam_no = CommonRequest::get($request, 'exam_no');
        if(strlen($exam_no) > 0 && !in_array($exam_no, [0, '-1']))  Tool::appendParamQuery($queryParams, $exam_no, 'exam_no', [0, '0', ''], ',', false);

        $core_grade_id = CommonRequest::get($request, 'core_grade_id');
        if(strlen($core_grade_id) > 0 && !in_array($core_grade_id, [0, '-1']))  Tool::appendParamQuery($queryParams, $core_grade_id, 'core_grade_id', [0, '0', ''], ',', false);

        $core_grade_id_history = CommonRequest::get($request, 'core_grade_id_history');
        if(strlen($core_grade_id_history) > 0 && !in_array($core_grade_id_history, [0, '-1']))  Tool::appendParamQuery($queryParams, $core_grade_id_history, 'core_grade_id_history', [0, '0', ''], ',', false);
        ...
        $status_no = CommonRequest::get($request, 'status_no');
        if(strlen($status_no) > 0  && !in_array($status_no, [0, '-1']))  Tool::appendCondition($queryParams, 'status_no',  $status_no . '=' . $status_no, '&'
            , 'where', 1, false, CompanyExamStaff::STATUS_NO_ARR);
        ...
        if(strlen($review_status) > 0  && !in_array($review_status, [0, '-1']))  Tool::appendCondition($queryParams, 'review_status',  $review_status . '=' . $review_status, '&'
            , 'where', 1, false, CompanyExamStaff::REVIEW_STATUS_ARR);
