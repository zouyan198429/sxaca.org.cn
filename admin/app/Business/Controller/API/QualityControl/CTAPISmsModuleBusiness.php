...
class CTAPISmsModuleBusiness extends BasicPublicCTAPIBusiness
{
    public static $model_name = 'API\QualityControl\SmsModuleAPI';
    public static $table_name = 'sms_module';// 表名称
    public static $record_class = __CLASS__;// 当前的类名称 App\Business\***\***\**\***
    public static $orderBy = SmsModule::ORDER_BY;// ['sort_num' => 'desc', 'id' => 'desc'];// 默认的排序字段数组 ['id' => 'desc']--默认 或 ['sort_num' => 'desc', 'id' => 'desc']
...
//                static::getRelationSqlParams(['where' => [['admin_type', 2]]], $extendParams, 'company_info'), '', []),
            // 获得短信模板所属模块参数
            'module_params' => CTAPISmsModuleParamsBusiness::getTableRelationConfigInfo($request, $controller
                , ['id' => 'module_id']
                , 2, 1
                ,'','',
                CTAPISmsModuleParamsBusiness::getRelationConfigs($request, $controller,
                    static::getUboundRelation($relationArr, 'module_params'),
                    static::getUboundRelationExtendParams($extendParams, 'module_params')),
                static::getRelationSqlParams(['orderBy' => ['sort_num' => 'desc', 'id' => 'desc']], $extendParams, 'module_params'), '', []),
        ];
    ...
    public static function getRelationConfigReturnData(Request $request, Controller $controller, $return_num = 0){
        ...
        if(($return_num & 2) == 2){// 给上一级返回名称 module_name 下标
            $one_field = ['key' => 'module_name', 'return_type' => 2, 'ubound_name' => 'module_name', 'split' => '、'];// 获得名称
