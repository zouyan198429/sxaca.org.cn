...
class CTAPICompanyNewContentBusiness extends BasicPublicCTAPIBusiness
{
    public static $model_name = 'API\QualityControl\CompanyNewContentAPI';
...
//                static::getRelationSqlParams(['where' => [['admin_type', 2]]], $extendParams, 'company_info'), '', []),
    ...
    public static function getRelationConfigReturnData(Request $request, Controller $controller, $return_num = 0){
        ...
        if(($return_num & 2) == 2){// 给上一级返回详细内容 new_content 下标
            $one_field = ['key' => 'new_content', 'return_type' => 2, 'ubound_name' => 'new_content', 'split' => '、'];// 获得名称
    ...
    public static function joinListParams(Request $request, Controller $controller, &$queryParams, $notLog = 0){
        ...
        $new_id = CommonRequest::get($request, 'new_id');
        if(strlen($new_id) > 0 && !in_array($new_id, [0, '-1']))  Tool::appendParamQuery($queryParams, $new_id, 'new_id', [0, '0', ''], ',', false);
