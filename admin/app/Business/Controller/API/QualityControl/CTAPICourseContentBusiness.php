...
class CTAPICourseContentBusiness extends BasicPublicCTAPIBusiness
{
    public static $model_name = 'API\QualityControl\CourseContentAPI';
...
//                static::getRelationSqlParams(['where' => [['admin_type', 2]]], $extendParams, 'company_info'), '', []),
    ...
    public static function getRelationConfigReturnData(Request $request, Controller $controller, $return_num = 0){
        ...
        if(($return_num & 2) == 2){// 给上一级返回详细内容 course_content 下标
            $one_field = ['key' => 'course_content', 'return_type' => 2, 'ubound_name' => 'course_content', 'split' => '、'];// 获得名称
