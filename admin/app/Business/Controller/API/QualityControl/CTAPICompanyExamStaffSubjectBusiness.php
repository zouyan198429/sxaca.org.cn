...
use App\Models\QualityControl\CompanyExamStaffSubject;
...
class CTAPICompanyExamStaffSubjectBusiness extends BasicPublicCTAPIBusiness
{
    public static $model_name = 'API\QualityControl\CompanyExamStaffSubjectAPI';
    public static $table_name = 'company_exam_staff_subject';// 表名称
    public static $record_class = __CLASS__;// 当前的类名称 App\Business\***\***\**\***
    public static $orderBy = CompanyExamStaffSubject::ORDER_BY;// ['sort_num' => 'desc', 'id' => 'desc'];// 默认的排序字段数组 ['id' => 'desc']--默认 或 ['sort_num' => 'desc', 'id' => 'desc']
...
//        if(strlen($bbbb) > 0 && !in_array($bbbb, [0, '-1']))  Tool::appendParamQuery($queryParams, $bbbb, 'bbbb', [0, '0', ''], ',', false);
        ...
        $exam_id = CommonRequest::get($request, 'exam_id');
        if(strlen($exam_id) > 0 && !in_array($exam_id, [0, '-1']))  Tool::appendParamQuery($queryParams, $exam_id, 'exam_id', [0, '0', ''], ',', false);

        $exam_staff_id = CommonRequest::get($request, 'exam_staff_id');
        if(strlen($exam_staff_id) > 0 && !in_array($exam_staff_id, [0, '-1']))  Tool::appendParamQuery($queryParams, $exam_staff_id, 'exam_staff_id', [0, '0', ''], ',', false);

        $subject_id = CommonRequest::get($request, 'subject_id');
        if(strlen($subject_id) > 0 && !in_array($subject_id, [0, '-1']))  Tool::appendParamQuery($queryParams, $subject_id, 'subject_id', [0, '0', ''], ',', false);

        $subject_id_history = CommonRequest::get($request, 'subject_id_history');
        if(strlen($subject_id_history) > 0 && !in_array($subject_id_history, [0, '-1']))  Tool::appendParamQuery($queryParams, $subject_id_history, 'subject_id_history', [0, '0', ''], ',', false);

        $is_right = CommonRequest::get($request, 'is_right');
        if(strlen($is_right) > 0  && !in_array($is_right, [0, '-1']))  Tool::appendCondition($queryParams, 'is_right',  $is_right . '=' . $is_right, '&'
            , 'where', 1, false, CompanyExamStaffSubject::IS_RIGHT_ARR);

        $paper_id = CommonRequest::get($request, 'paper_id');
        if(strlen($paper_id) > 0 && !in_array($paper_id, [0, '-1']))  Tool::appendParamQuery($queryParams, $paper_id, 'paper_id', [0, '0', ''], ',', false);

        $paper_id_history = CommonRequest::get($request, 'paper_id_history');
        if(strlen($paper_id_history) > 0 && !in_array($paper_id_history, [0, '-1']))  Tool::appendParamQuery($queryParams, $paper_id_history, 'paper_id_history', [0, '0', ''], ',', false);

        $review_status = CommonRequest::get($request, 'review_status');
        if(strlen($review_status) > 0  && !in_array($review_status, [0, '-1']))  Tool::appendCondition($queryParams, 'review_status',  $review_status . '=' . $review_status, '&'
            , 'where', 1, false, CompanyExamStaffSubject::REVIEW_STATUS_ARR);
