...
class CTAPIPaymentProjectFieldsBusiness extends BasicPublicCTAPIBusiness
{
    public static $model_name = 'API\QualityControl\PaymentProjectFieldsAPI';
    public static $table_name = 'payment_project_fields';// 表名称
    public static $record_class = __CLASS__;// 当前的类名称 App\Business\***\***\**\***
    public static $orderBy = PaymentProjectFields::ORDER_BY;// ['sort_num' => 'desc', 'id' => 'desc'];// 默认的排序字段数组 ['id' => 'desc']--默认 或 ['sort_num' => 'desc', 'id' => 'desc']
...
//        if(strlen($bbbb) > 0 && !in_array($bbbb, [0, '-1']))  Tool::appendParamQuery($queryParams, $bbbb, 'bbbb', [0, '0', ''], ',', false);
        ...
        if(strlen($required_status) > 0  && !in_array($required_status, [0, '-1']))  Tool::appendCondition($queryParams, 'required_status', $required_status . '=' . $required_status, '&', 'where', 1, false, PaymentProjectFields::REQUIRED_STATUS_ARR); // --逻辑 & 操作，且有数组范围

        $input_status = CommonRequest::get($request, 'input_status');
        if(strlen($input_status) > 0  && !in_array($input_status, [0, '-1']))  Tool::appendCondition($queryParams, 'input_status', $input_status . '=' . $input_status, '&', 'where', 1, false, PaymentProjectFields::INPUT_STATUS_ARR); // --逻辑 & 操作，且有数组范围

        $show_status = CommonRequest::get($request, 'show_status');
        if(strlen($show_status) > 0  && !in_array($show_status, [0, '-1']))  Tool::appendCondition($queryParams, 'show_status', $show_status . '=' . $show_status, '&', 'where', 1, false, PaymentProjectFields::SHOW_STATUS_ARR); // --逻辑 & 操作，且有数组范围

        $val_type = CommonRequest::get($request, 'val_type');
        if(strlen($val_type) > 0  && !in_array($val_type, [0, '-1']))  Tool::appendCondition($queryParams, 'val_type', $val_type . '=' . $val_type, '&', 'where', 1, false, PaymentProjectFields::VAL_TYPE_ARR); // --逻辑 & 操作，且有数组范围
