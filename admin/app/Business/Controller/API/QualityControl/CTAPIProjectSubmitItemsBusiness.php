...
class CTAPIProjectSubmitItemsBusiness extends BasicPublicCTAPIBusiness
{
...
//        }
        ...
        if(($return_num & 4) == 4){// 给上一级返回名称 submit_items_text = '含水量1<br/>含盐量2' 下标
            $one_field = ['key' => 'name', 'return_type' => 2, 'ubound_name' => 'submit_items_text', 'split' => '<br/>'];// 获得名称
            if(!isset($return_data['one_field'])) $return_data['one_field'] = [];
            array_push($return_data['one_field'], $one_field);
        }

        if(($return_num & 8) == 8){// 给上一级返回 id 、 tag_name 的二维数组 . 如： project_standards = [[ 'id' => 26,  'tag_name' => '方法2'], ... ]
            $many_fields = [ 'ubound_name' => 'submit_items', 'fields_arr'=> ['id' => 'id', 'tag_name' => 'name'],'reset_ubound' => 2];
