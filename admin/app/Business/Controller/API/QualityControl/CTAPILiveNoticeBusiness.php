...
class CTAPILiveNoticeBusiness extends BasicPublicCTAPIBusiness
{
    public static $model_name = 'API\QualityControl\LiveNoticeAPI';
    public static $table_name = 'live_notice';// 表名称
    public static $record_class = __CLASS__;// 当前的类名称 App\Business\***\***\**\***
    public static $orderBy = LiveNotice::ORDER_BY;// ['sort_num' => 'desc', 'id' => 'desc'];// 默认的排序字段数组 ['id' => 'desc']--默认 或 ['sort_num' => 'desc', 'id' => 'desc']
...
//                static::getRelationSqlParams(['where' => [['admin_type', 2]]], $extendParams, 'company_info'), '', []),
            // 获得详细内容
            'live_notice_content' => CTAPILiveNoticeContentBusiness::getTableRelationConfigInfo($request, $controller
                , ['id' => 'live_notice_id']
                , 1, 2
                ,'','',
                CTAPILiveNoticeContentBusiness::getRelationConfigs($request, $controller,
                    static::getUboundRelation($relationArr, 'live_notice_content'),
                    static::getUboundRelationExtendParams($extendParams, 'live_notice_content')),
                static::getRelationSqlParams([], $extendParams, 'live_notice_content'), '', []),
        ];
...
//        }
     ...
     * @author zouyan(305463219@qq.com)
     */
    public static function getWebDataList(Request $request, Controller $controller, &$reDataArr, $url_path = 'jigou/list/', $pagesize = 20, $page = 1, $notLog = 0){
...
//        $company_grade = CommonRequest::get($request, 'company_grade');
        ...
        $pathParamStr = $pagesize . '_{page}';// . $page;
