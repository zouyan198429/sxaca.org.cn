...
use App\Models\QualityControl\CompanyPaperCategory;
...
class CTAPICompanyPaperCategoryBusiness extends BasicPublicCTAPIBusiness
{
    public static $model_name = 'API\QualityControl\CompanyPaperCategoryAPI';
    public static $table_name = 'company_paper_category';// 表名称
    public static $record_class = __CLASS__;// 当前的类名称 App\Business\***\***\**\***
    public static $orderBy = CompanyPaperCategory::ORDER_BY;// ['sort_num' => 'desc', 'id' => 'desc'];// 默认的排序字段数组 ['id' => 'desc']--默认 或 ['sort_num' => 'desc', 'id' => 'desc']
...
//        if(strlen($bbbb) > 0 && !in_array($bbbb, [0, '-1']))  Tool::appendParamQuery($queryParams, $bbbb, 'bbbb', [0, '0', ''], ',', false);
        ...
        if(strlen($subject_order_type) > 0  && !in_array($subject_order_type, [0, '-1']))  Tool::appendCondition($queryParams, 'subject_order_type',  $subject_order_type . '=' . $subject_order_type, '&'
            , 'where', 1, false, CompanyPaperCategory::SUBJECT_ORDER_TYPE_ARR);
        ...
        if(strlen($subject_updated) > 0  && !in_array($subject_updated, [0, '-1']))  Tool::appendCondition($queryParams, 'subject_updated',  $subject_updated . '=' . $subject_updated, '&'
            , 'where', 1, false, CompanyPaperCategory::SUBJECT_UPDATED_ARR);
