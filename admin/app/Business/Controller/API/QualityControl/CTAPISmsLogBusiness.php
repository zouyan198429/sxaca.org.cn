...
class CTAPISmsLogBusiness extends BasicPublicCTAPIBusiness
{
    public static $model_name = 'API\QualityControl\SmsLogAPI';
...
//                static::getRelationSqlParams(['where' => [['admin_type', 2]]], $extendParams, 'company_info'), '', []),
            // 获得所属模板名称
            'sms_template' => CTAPISmsTemplateBusiness::getTableRelationConfigInfo($request, $controller
                , ['template_id' => 'id']
                , 1, 2
                ,'','',
                CTAPISmsTemplateBusiness::getRelationConfigs($request, $controller,
                    static::getUboundRelation($relationArr, 'sms_template'),
                    static::getUboundRelationExtendParams($extendParams, 'sms_template')),
                static::getRelationSqlParams([], $extendParams, 'sms_template'), '', []),
        ];
...
//        }
    ...
    public static function joinListParams(Request $request, Controller $controller, &$queryParams, $notLog = 0){
        ...
        $template_id = CommonRequest::get($request, 'template_id');
        if(strlen($template_id) > 0 && !in_array($template_id, [0, '-1']))  Tool::appendParamQuery($queryParams, $template_id, 'template_id', [0, '0', ''], ',', false);
