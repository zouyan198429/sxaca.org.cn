...
class CTAPILiveNoticeContentBusiness extends BasicPublicCTAPIBusiness
{
    public static $model_name = 'API\QualityControl\LiveNoticeContentAPI';
...
//                static::getRelationSqlParams(['where' => [['admin_type', 2]]], $extendParams, 'company_info'), '', []),
    ...
    public static function getRelationConfigReturnData(Request $request, Controller $controller, $return_num = 0){
        ...
        if(($return_num & 2) == 2){// 给上一级返回名称 notice_content 下标
            $one_field = ['key' => 'notice_content', 'return_type' => 2, 'ubound_name' => 'notice_content', 'split' => '、'];// 获得名称
    ...
    public static function joinListParams(Request $request, Controller $controller, &$queryParams, $notLog = 0){
        ...
        $live_notice_id = CommonRequest::get($request, 'live_notice_id');
        if(strlen($live_notice_id) > 0 && !in_array($live_notice_id, [0, '-1']))  Tool::appendParamQuery($queryParams, $live_notice_id, 'live_notice_id', [0, '0', ''], ',', false);
