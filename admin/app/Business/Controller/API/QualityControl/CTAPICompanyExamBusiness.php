...
use App\Models\QualityControl\CompanyExam;
...
class CTAPICompanyExamBusiness extends BasicPublicCTAPIBusiness
{
    public static $model_name = 'API\QualityControl\CompanyExamAPI';
    public static $table_name = 'company_exam';// 表名称
    public static $record_class = __CLASS__;// 当前的类名称 App\Business\***\***\**\***
    public static $orderBy = CompanyExam::ORDER_BY;// ['sort_num' => 'desc', 'id' => 'desc'];// 默认的排序字段数组 ['id' => 'desc']--默认 或 ['sort_num' => 'desc', 'id' => 'desc']
...
//        if(strlen($bbbb) > 0 && !in_array($bbbb, [0, '-1']))  Tool::appendParamQuery($queryParams, $bbbb, 'bbbb', [0, '0', ''], ',', false);
        ...
        $exam_num = CommonRequest::get($request, 'exam_num');
        if(strlen($exam_num) > 0 && !in_array($exam_num, [0, '-1']))  Tool::appendParamQuery($queryParams, $exam_num, 'exam_num', [0, '0', ''], ',', false);
        ...
        if(strlen($review_method) > 0  && !in_array($review_method, [0, '-1']))  Tool::appendCondition($queryParams, 'review_method',  $review_method . '=' . $review_method, '&'
            , 'where', 1, false, CompanyExam::REVIEW_METHOD_ARR);
        ...
        if(strlen($review_status) > 0  && !in_array($review_status, [0, '-1']))  Tool::appendCondition($queryParams, 'review_status',  $review_status . '=' . $review_status, '&'
            , 'where', 1, false, CompanyExam::REVIEW_STATUS_ARR);
        ...
        if(strlen($status_no) > 0  && !in_array($status_no, [0, '-1']))  Tool::appendCondition($queryParams, 'status_no',  $status_no . '=' . $status_no, '&'
            , 'where', 1, false, CompanyExam::STATUS_NO_ARR);

        $exam_subject_updated = CommonRequest::get($request, 'exam_subject_updated');
        if(strlen($exam_subject_updated) > 0  && !in_array($exam_subject_updated, [0, '-1']))  Tool::appendCondition($queryParams, 'exam_subject_updated',  $exam_subject_updated . '=' . $exam_subject_updated, '&'
            , 'where', 1, false, CompanyExam::EXAM_SUBJECT_UPDATED_ARR);
        ...
        if(strlen($open_status) > 0  && !in_array($open_status, [0, '-1']))  Tool::appendCondition($queryParams, 'open_status',  $open_status . '=' . $open_status, '&'
            , 'where', 1, false, CompanyExam::OPEN_STATUS_ARR);
