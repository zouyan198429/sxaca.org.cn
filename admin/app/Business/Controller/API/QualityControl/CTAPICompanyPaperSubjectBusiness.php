...
class CTAPICompanyPaperSubjectBusiness extends BasicPublicCTAPIBusiness
{
    public static $model_name = 'API\QualityControl\CompanyPaperSubjectAPI';
    public static $table_name = 'company_paper_subject';// 表名称
    public static $record_class = __CLASS__;// 当前的类名称 App\Business\***\***\**\***
    public static $orderBy = CompanyPaperSubject::ORDER_BY;// ['sort_num' => 'desc', 'id' => 'desc'];// 默认的排序字段数组 ['id' => 'desc']--默认 或 ['sort_num' => 'desc', 'id' => 'desc']
...
//        if(strlen($bbbb) > 0 && !in_array($bbbb, [0, '-1']))  Tool::appendParamQuery($queryParams, $bbbb, 'bbbb', [0, '0', ''], ',', false);
        ...
        $paper_category_id = CommonRequest::get($request, 'paper_category_id');
        if(strlen($paper_category_id) > 0 && !in_array($paper_category_id, [0, '-1']))  Tool::appendParamQuery($queryParams, $paper_category_id, 'paper_category_id', [0, '0', ''], ',', false);
        ...
        if(strlen($subject_updated) > 0  && !in_array($subject_updated, [0, '-1']))  Tool::appendCondition($queryParams, 'subject_updated',  $subject_updated . '=' . $subject_updated, '&'
            , 'where', 1, false, CompanyPaperSubject::SUBJECT_UPDATED_ARR);
