...
var otheraction = {
    paySave: function(obj) {
        ...
        var pay_show_id = trObj.find('input[name=pay_show_id]').val();
        ...
        var pay_method = trObj.find('input[name=pay_method]:checked').val() || '';
        var judge_seled = judge_validate(1,'缴费方式',pay_method,true,'digit','','');
        if(judge_seled != ''){
            layer_alert("请选择缴费方式",3,0);
        ...
        var index_query = layer.confirm('确定' + operateText + '吗？', {
            ...
            var weburl = PAY_SAVE_URL + '?id='+ ids + '&pay_show_id=' + pay_show_id + '&pay_config_id=' + pay_config_id + '&pay_method=' + pay_method;
            console.log(weburl);
            var tishi = '缴费';//"查看供应商";
...
function ajax_form(){
    ...
    //     return false;
    // }
    if(!judge_validate(4,'待分班学员',id,true,'length',1,3000)){
        return false;
    }
    ...
    var class_id = $('input[name=class_id]:checked').val() || '';
    var judge_seled = judge_validate(1,'分配班级',class_id,true,'digit','','');
    if(judge_seled != ''){
        layer_alert("请选择分配班级",3,0);
        //err_alert('<font color="#000000">' + judge_seled + '</font>');
        return false;
    }
