...
(function() {
   ...
   // document.write("        if( item.issuper==0 ){");
    ...
    document.write("            <td><%=item.pay_company_name%><hr/><%=item.user_staff_name%><\/td>");
    document.write("            <td>¥<%=item.pay_amount%><hr/>¥<%=item.real_amount%><\/td>");
    document.write("            <td>¥<%=item.discount_amount%><hr/><%=item.discount_explain%><\/td>");
    document.write("            <td>¥<%=item.wait_refund_amount%><hr/>¥<%=item.refunded_amount%><\/td>");
    document.write("            <td><%=item.order_no%><hr/><span style=\"color: red;font-size:30px;\">¥<%=item.final_amount%></span><\/td>");
    document.write("            <td><span style=\"color: red;font-size:20px;\"><%=item.pay_status_text%></span><hr/><span style=\"color: red;font-size:20px;\"><%=item.record_status_text%></span><\/td>");
    document.write("            <td>");
    document.write("                <%=item.order_time%><hr/>");
    document.write("                <%=item.pay_valid_second%>秒【<%=item.pay_valid_second_format%>】<hr/><%=item.pay_end_time%><hr/>");
    document.write("                <%=item.pay_time%>");
    document.write("            <\/td>");
    document.write("            <td>");
    document.write("                <%=item.handle_status_text%><hr/>");
    document.write("                <%if( item.valid_limit_second > 0){%>");
    document.write("                   <%=item.valid_limit_second%>秒【<%=item.valid_limit_second_format%>】");
    document.write("                <%}%>");
    document.write("            <\/td>");
    document.write("            <td><%=item.begin_time%><hr/><%=item.finish_time%><\/td>");
    document.write("            <td><%=item.invoice_project_template_name%><hr/><%=item.invoice_template_name%><\/td>");
    ...
    document.write("                <\/a>");
    document.write("                <%}%>");
    document.write("                <%if(can_modify &&  item.record_status == RECORD_STATUS_NORMAL && valInArrOrObj([PAY_STATUS_WAIT, PAY_STATUS_PART_PAY], item.pay_status)){%>");
