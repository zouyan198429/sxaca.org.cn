...
function ajax_form(){
    ...
    var year_no = $('input[name=year_no]').val();
    if(!judge_validate(4,'年',year_no,true,'digit','','')){
        return false;
    }

    var exist_problem = $('input[name=exist_problem]').val();
    if(!judge_validate(4,'存在问题',exist_problem,true,'length',1,200)){
        return false;
    }

    var breach_clause = $('textarea[name=breach_clause]').val();
    if(!judge_validate(4,'违返条款',breach_clause,false,'length',2,2000)){
        return false;
    }

    var link_url = $('input[name=link_url]').val();
    if(!judge_validate(4,'原文链接',link_url,false,'length',1,600)){
        return false;
    }
