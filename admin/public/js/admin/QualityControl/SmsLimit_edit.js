...
function ajax_form(){
    ...
    //     return false;
    // }
    var limit_explain = $('input[name=limit_explain]').val();
    if(!judge_validate(4,'说明',limit_explain,true,'length',1,100)){
        return false;
    }

    var limit_key = $('input[name=limit_key]').val();
    if(!judge_validate(4,'限止键',limit_key,true,'length',1,30)){
        return false;
    }

    var limit_code = $('input[name=limit_code]').val();
    if(!judge_validate(4,'限止编号',limit_code,true,'bitint',63,'')){
        return false;
    }

    var valid_second = $('input[name=valid_second]').val();
    if(!judge_validate(4,'单位时间',valid_second,true,'digit','','')){
        return false;
    }

    var limit_count = $('input[name=limit_count]').val();
    if(!judge_validate(4,'限止次数',limit_count,true,'digit','','')){
        return false;
    }
