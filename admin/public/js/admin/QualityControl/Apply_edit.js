...
var FILE_UPLOAD_OBJ = {
    ...
    'myUploaderLarge':{
        ...
        'upload_file_filters' : {
            // 只允许上传图片或图标（.ico）
            mime_types: commonaction.getMineTypes(['pic']),// FILE_MIME_TYPES.excel.mime_types, 下标配置   如 ['pic','pdf',...]
            // 最大上传文件为 2MB
            max_file_size: '100mb'
            // 不允许上传重复文件
            // prevent_duplicates: true
...
function ajax_form(){
    ...
    var judge_seled = judge_validate(1,'所属企业',staff_id,true,'positive_int','','');
    if(judge_seled != ''){
    ...
    if(!judge_validate(4,'应用名称',app_name,true,'length',1,50)){
        return false;
    }
...
function popSelectInit(){
    ...
    $('.select_close').each(function(){
        ...
        console.log('==select_id=', idObj.val());
