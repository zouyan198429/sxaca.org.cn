...
//业务逻辑部分
var otheraction = {
    copyLog :function(thisObj) {
        var btnObj = $(thisObj);
        var htmlStr = btnObj.closest('tr').find('.content').html();
...
(function() {
   ...
   // document.write("        if( item.issuper==0 ){");
    ...
    document.write("            <td><%=item.log_no%><hr/><%=item.action_method%><\/td>");
    document.write("            <td><%=item.app_name%><hr/><%=item.system_no%><\/td>");
    document.write("            <td><%=item.model_no%><hr/><%=item.fun_no%><\/td>");
    document.write("            <td><%=item.begin_time%><hr/><%=item.end_time%><\/td>");
    document.write("            <td><%=item.exe_time%><hr/><%=item.log_content.results_content%><\/td>");
    document.write("            <td><%=item.do_status_text%><hr/><%=item.response_level_text%><\/td>");
    // document.write("            <td><%=item.log_content.request_content%><\/td>");
    document.write("            <td>");
    document.write("            <%=unicode2string(item.log_content.request_content)%>");
    document.write("            <span class=\"content\" style=\"display: none;\"><%=item.log_content.request_content%></span>");
    document.write("            &nbsp;&nbsp;<input type=\"button\" class=\"btn btn-success  btn-xs export_excel\"  value=\"复制\"  onclick=\"otheraction.copyLog(this)\">");
    document.write("            <\/td>");
    // document.write("            <td>");
