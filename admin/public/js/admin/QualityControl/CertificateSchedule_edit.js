...
function ajax_form(){
    ...
    if(!judge_validate(4,'二级名称',project_name,false,'length',1,100)){
        return false;
    }

    var three_name = $('input[name=three_name]').val();
    if(!judge_validate(4,'三级名称',three_name,false,'length',1,100)){
        return false;
    }

    var four_name = $('input[name=four_name]').val();
    if(!judge_validate(4,'四级名称',four_name,false,'length',1,100)){
        return false;
    }
    ...
    var method_name = $('textarea[name=method_name]').val();
    if(!judge_validate(4,'标准（方法）名称',method_name,true,'length',2,500)){
        return false;
    }

    var limit_range = $('textarea[name=limit_range]').val();
    if(!judge_validate(4,'限制范围',limit_range,false,'length',2,2000)){
        return false;
    }

    var explain_text = $('textarea[name=explain_text]').val();
    if(!judge_validate(4,'说明',explain_text,false,'length',2,2000)){
        return false;
    }
