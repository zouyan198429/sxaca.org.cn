...
$(function(){
    ...
    //切换参数类型
    $(document).on("change",'input[name=param_type]',function(){
...
function showHide(param_type) {

    if(param_type == 1){
        $('.tr_date_time_format').show();
        $('.tr_fixed_val').hide();
    }else if(param_type == 2){
        $('.tr_date_time_format').hide();
        $('.tr_fixed_val').show();
    }else{
        $('.tr_date_time_format').hide();
        $('.tr_fixed_val').hide();
...
function ajax_form(){
    ...
    if(!judge_validate(4,'参数名称',param_name,true,'length',1,30)){
        return false;
    }

    var param_code = $('input[name=param_code]').val();
    if(!judge_validate(4,'参数代码',param_code,true,'length',1,50)){
        return false;
    }

    var param_type = $('input[name=param_type]:checked').val() || '';
    var judge_seled = judge_validate(1,'参数类型',param_type,true,'custom',/^[12]$/,"");
    if(judge_seled != ''){
        layer_alert("请选择参数类型",3,0);
        //err_alert('<font color="#000000">' + judge_seled + '</font>');
        return false;
    }

    if(param_type == 1){
        var date_time_format = $('input[name=date_time_format]').val();
        if(!judge_validate(4,'日期时间格式化',date_time_format,true,'length',1,20)){
            return false;
        }
        $('input[name=fixed_val]').val('');
    }else{
        var fixed_val = $('input[name=fixed_val]').val();
        if(!judge_validate(4,'固定值',fixed_val,true,'length',1,500)){
            return false;
        }
        $('input[name=date_time_format]').val('');
    }
