...
function ajax_form(){
    ...
    var search_json = $('textarea[name=search_json]').val();
    if(!judge_validate(4,'查询能力范围json',search_json,true,'length',2,20000)){
        return false;
    }

    var schedule_json = $('textarea[name=schedule_json]').val();
    if(!judge_validate(4,'更新能力范围json',schedule_json,true,'length',2,20000)){
        return false;
    }
