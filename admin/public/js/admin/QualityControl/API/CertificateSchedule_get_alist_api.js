...
var otheraction = {
    copyLog :function(thisObj) {
        var btnObj = $(thisObj);
        var htmlStr = $('#api_result_block').html();// btnObj.closest('tr').find('.content').html();
...
function ajax_form(){
    ...
    if(!judge_validate(4,'当前页码',page,true,'positive_int','','')){
        return false;
    }

    var pagesize = $('input[name=pagesize]').val();
    if(!judge_validate(4,'每页显示数量',pagesize,false,'positive_int','','')){
        return false;
    }
            ...
            if(!ret.apistatus){//失败
                ...
                layer.msg('操作成功！', {
                    ...
                    if( (hidden_option & 8192) != 8192){
                        ...
                        $('#api_result_block').html(JSON.stringify(ret));
                        SUBMIT_FORM = true;//标记为未提交过
                    }else{
