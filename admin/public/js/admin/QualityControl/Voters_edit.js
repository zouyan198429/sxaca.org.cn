...
function ajax_form(){
    ...
    var judge_seled = judge_validate(1,'组序号',group_type,true,'custom',/^([1248]|16)$/,"");
    if(judge_seled != ''){
        layer_alert("请选择组序号",3,0);
        //err_alert('<font color="#000000">' + judge_seled + '</font>');
        return false;
    }

    var group_no = $('input[name=group_no]').val();
    if(!judge_validate(4,'组号',group_no,true,'length',1,60)){
        return false;
    }

    var real_name = $('input[name=real_name]').val();
    if(!judge_validate(4,'姓名',real_name,true,'length',1,30)){
        return false;
    }

    var sex_name = $('input[name=sex_name]').val();
    if(!judge_validate(4,'性别',sex_name,false,'length',1,10)){
        return false;
    }

    var id_number = $('input[name=id_number]').val();
    if(!judge_validate(4,'身份证号',id_number,false,'length',1,25)){
        return false;
    }

    var birth_date = $('input[name=birth_date]').val();
    if(!judge_validate(4,'出生日期',birth_date,false,'length',1,30)){
        return false;
    }

    var relation_name = $('input[name=relation_name]').val();
    if(!judge_validate(4,'与户主关系',relation_name,false,'length',1,15)){
        return false;
    }

    var politics_name = $('input[name=politics_name]').val();
    if(!judge_validate(4,'政治面貌',politics_name,false,'length',1,30)){
        return false;
    }

    var addr = $('input[name=addr]').val();
    if(!judge_validate(4,'家庭住址',addr,false,'length',1,200)){
        return false;
    }

    var contact = $('input[name=contact]').val();
    if(!judge_validate(4,'联系方式',contact,false,'length',1,100)){
        return false;
    }
