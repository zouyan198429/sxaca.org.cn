...
function ajax_form(){
    ...
    var vod_type_id = $('input[name=vod_type_id]:checked').val() || '';
    var judge_seled = judge_validate(1,'所属分类',vod_type_id,true,'digit',"","");
    if(judge_seled != ''){
    ...
    var vod_name = $('input[name=vod_name]').val();
    if(!judge_validate(4,'课程名称',vod_name,true,'length',1,50)){
        return false;
    }
    ...
    var vod_content = $('textarea[name=vod_content]').val();
    if(!judge_validate(4,'详细内容',vod_content,true,'length',1,200000)){
        return false;
    }
    ...
    if(!judge_validate(4,'收费标准(会员)',price_member,true,'doublepositive','','')){
        return false;
    }

    var price_general = $('input[name=price_general]').val();
    if(!judge_validate(4,'收费标准(非会员)',price_general,true,'doublepositive','','')){
        return false;
    }

    var effectively_day = $('input[name=effectively_day]').val();
    if(!judge_validate(4,'有效期时长',effectively_day,false,'digit','','')){
        return false;
    }
    ...
    var recommend_status = $('input[name=recommend_status]:checked').val() || '';
    var judge_seled = judge_validate(1,'推荐状态',recommend_status,true,'custom',/^[12]$/,"");
    if(judge_seled != ''){
        layer_alert("请选择推荐状态",3,0);
        //err_alert('<font color="#000000">' + judge_seled + '</font>');
        return false;
    }
