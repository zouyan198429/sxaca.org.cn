...
function ajax_form(){
    ...
    var pay_company_name = $('input[name=pay_company_name]').val();
    if(!judge_validate(4,'收款企业名称',pay_company_name,true,'length',1,50)){
        return false;
    }

    var pay_key = $('input[name=pay_key]').val();
    if(!judge_validate(4,'收款关键字',pay_key,true,'length',1,20)){
        return false;
    }
    ...
    if(!judge_validate(4,'备注',remarks,false,'length',2,500)){
        return false;
    }
