...
function ajax_form(){
    ...
    // let hasVals = false;
        ...
        let input_num = 0;
        $('#samples_list').find('input[name="items_samples[]"]').each(function () {
            let inputObj = $(this);
            ...
            var judge_seled = judge_validate(1,samples_num_txt + input_num,input_val,isMust,'digit','','');
            // 第一个必填 ，其它的可填, 但是要按顺序填
            // 值格式有错 或  前面已经有空值，但是后面又有值
            let input_err_str = '请按顺序填写【' + samples_num_txt + input_num + '】';
            if( judge_seled != '' || (has_empty_val && trim(input_val) !== '')){
                if((has_empty_val && trim(input_val) !== '')){
                    input_err_str = '请按顺序填写【' + samples_num_txt + '】';
                }
        ...
        // 没有值
        if(!input_has_val){
            layer_alert("请按顺序填写【" + samples_num_txt + "】",3,0);
            //err_alert('<font color="#000000">' + judge_seled + '</font>');
            has_err = true;
            return false;
        }
