...
function ajax_form(){
    ...
    if(!judge_validate(4,'销售方名称',xsf_mc,true,'length',1,100)){
        return false;
    }

    var xsf_dz = $('input[name=xsf_dz]').val();
    if(!judge_validate(4,'销售方地址',xsf_dz,false,'length',1,100)){
        return false;
    }

    var xsf_dh = $('input[name=xsf_dh]').val();
    if(!judge_validate(4,'销售方电话',xsf_dh,false,'length',1,50)){
        return false;
    }

    var xsf_nsrsbh = $('input[name=xsf_nsrsbh]').val();
    if(!judge_validate(4,'销售方纳税人识别号',xsf_nsrsbh,true,'length',1,50)){
        return false;
    }

    var xsf_yh = $('input[name=xsf_yh]').val();
    if(!judge_validate(4,'销售方银行',xsf_yh,false,'length',1,100)){
        return false;
    }

    var xsf_yhzh = $('input[name=xsf_yhzh]').val();
    if(!judge_validate(4,'销售方银行账号',xsf_yhzh,false,'length',1,50)){
        return false;
    }
