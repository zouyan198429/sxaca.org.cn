...
function ajax_form(){
    ...
    var import_no = $('input[name=import_no]').val();
    if(!judge_validate(4,'导入批次',import_no,true,'length',1,30)){
        return false;
    }

    // 导入时间
    var begin_date = $('input[name=import_time]').val();
    if(!judge_validate(4,'导入时间',begin_date,true,'date','','')){
        return false;
    }

    var import_content = $('textarea[name=import_content]').val();
    if(!judge_validate(4,'内容',import_content,true,'length',2,50000)){
        return false;
    }

    var success_num = $('input[name=success_num]').val();
    if(!judge_validate(4,'成功导入数',success_num,false,'digit','','')){
        return false;
    }

    var fail_num = $('input[name=fail_num]').val();
    if(!judge_validate(4,'失败导入数',fail_num,false,'digit','','')){
        return false;
    }
