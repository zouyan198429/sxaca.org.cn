...
var FILE_UPLOAD_OBJ = {
    ...
    'videoUploaderLarge':{
        ...
        'operate_auth': 1 |  2 | 4,// 1 | 操作权限 1 查看 ；2 下载 ；4 删除
        'icon':'file-o',// 非图片时文件显示的图标--自定义的初始化用，重上传【不管】的会自动生成。 可以参考 common.js 中的变量 FILE_MIME_TYPES file-o  默认
        'file_upload_url': UPLOAD_VIDEO_LARGE_URL,// "{ { url('api/admin/upload') }}",// 文件上传提交地址 'your/file/upload/url'
        'file_down_url': DOWN_VIDEO_FILE_URL,// 文件下载url--自定义格式化数据时可用
        'pic_del_url' : DEL_VIDEO_FILE_URL,// 删除图片url
        'del_fun_pre' : 'large_del',// 自定义的删除前的方法名，返回要加入删除时的参数对象,无参数 ，需要时自定义方法
        'multipart_params' : {pro_unit_id:'0'},// 附加参数	函数或对象，默认 {}
        'lang' : 'zh_cn', // 界面语言 默认情况下设置为空值，会从浏览器 <html lang=""> 属性上获取语言设置，但有也可以手动指定为以下选项：'zh_cn'：简体中文；'zh_tw'：繁体中文；
        'file_data_name' : 'photo', //	文件域在表单中的名称	默认 'file'
        'checkbox_name' : 'resource_id_video[]',// //'large_id[]',// 上传后文件id的复选框名称
        'limit_files_count' : 1,//   限制文件上传数目	false（默认）或数字
        'mulit_selection' : true,//  是否可用一次选取多个文件	默认 true false
        'auto_upload' : false,//  当选择文件后立即自动进行上传操作 true / false
        'upload_file_filters' : {
            // 只允许上传图片或图标（.ico）
            mime_types: commonaction.getMineTypes(['video', 'audio']),//  FILE_MIME_TYPES.excel.mime_types, 下标配置   如 ['pic','pdf',...] , 'excel', 'pdf', 'doc'
            // 最大上传文件为 2MB
        ...
        'baidu_tem_pic_list' : 'baidu_template_upload_pic',// 上传列表格式数的百度数据格式化模型id baidu_template_pic_show
        'flash_swf_url' : FLASH_VIDEO_SWF_URL,// "{ {asset('dist/lib/uploader/Moxie.swf') }}",// flash 上传组件地址  默认为 lib/uploader/Moxie.swf
        'silverlight_xap_url' : SILVERLIGHT_VIDEO_XAP_URL, // "{ {asset('dist/lib/uploader/Moxie.xap') }}",// silverlight_xap_url silverlight 上传组件地址  默认为 lib/uploader/Moxie.xap  请确保在文件上传页面能够通过此地址访问到此文件。
        'self_upload' : true,//  是否自己触发上传 TRUE/1自己触发上传方法 FALSE/0控制上传按钮
        'file_upload_method' : 'initPic()',// 单个上传成功后执行方法 格式 aaa();  或  空白-没有
        'file_upload_complete' : '',  // 所有上传成功后执行方法 格式 aaa();  或  空白-没有
        'file_resize' : {quuality: 40},
        ...
        //     // preserve_headers: false // 是否保留图片的元数据，默认为 true 保留，如果为 false 不保留。
        // },
        'pic_list_json': {'data_list': RESOURCE_VIDEO_LIST_LARGE }// piclistJson 数据列表json对象格式  {‘data_list’:[{'id':1,'resource_name':'aaa.jpg','resource_url':'picurl','created_at':'2018-07-05 23:00:06'}]}
        // 不使用了，会自动格式化 pic_list_json 来 匹配  'static_files':@ json($info['static_files'] ?? [])// 初始静态文件对象数组 ； self_upload = false 使用； 格式 [{remoteData:{id:197}, name: 'zui.js', size: 216159, url: 'http://openzui.com'},...]
    },
    'coursewareUploaderLarge':{
        ...
        'operate_auth': 1 |  2 | 4,// 1 | 操作权限 1 查看 ；2 下载 ；4 删除
        'icon':'file-o',// 非图片时文件显示的图标--自定义的初始化用，重上传【不管】的会自动生成。 可以参考 common.js 中的变量 FILE_MIME_TYPES file-o  默认
        'file_upload_url': UPLOAD_COURSEWARE_LARGE_URL,// "{ { url('api/admin/upload') }}",// 文件上传提交地址 'your/file/upload/url'
        'file_down_url': DOWN_COURSEWARE_FILE_URL,// 文件下载url--自定义格式化数据时可用
        'pic_del_url' : DEL_COURSEWARE_FILE_URL,// 删除图片url
        'del_fun_pre' : 'large_del',// 自定义的删除前的方法名，返回要加入删除时的参数对象,无参数 ，需要时自定义方法
        'multipart_params' : {pro_unit_id:'0'},// 附加参数	函数或对象，默认 {}
        'lang' : 'zh_cn', // 界面语言 默认情况下设置为空值，会从浏览器 <html lang=""> 属性上获取语言设置，但有也可以手动指定为以下选项：'zh_cn'：简体中文；'zh_tw'：繁体中文；
        'file_data_name' : 'photo', //	文件域在表单中的名称	默认 'file'
        'checkbox_name' : 'resource_id_courseware[]',// //'large_id[]',// 上传后文件id的复选框名称
        'limit_files_count' : 9,//   限制文件上传数目	false（默认）或数字
        'mulit_selection' : true,//  是否可用一次选取多个文件	默认 true false
        'auto_upload' : false,//  当选择文件后立即自动进行上传操作 true / false
        'upload_file_filters' : {
            // 只允许上传图片或图标（.ico）
            mime_types: commonaction.getMineTypes(['pic', 'excel', 'pdf', 'doc', 'ppt']),//  FILE_MIME_TYPES.excel.mime_types, 下标配置   如 ['pic','pdf',...] , 'excel', 'pdf', 'doc'
            // 最大上传文件为 2MB
            max_file_size: '200mb'
            // 不允许上传重复文件
            // prevent_duplicates: true
        ...
        'baidu_tem_pic_list' : 'baidu_template_upload_pic',// 上传列表格式数的百度数据格式化模型id baidu_template_pic_show
        'flash_swf_url' : FLASH_COURSEWARE_SWF_URL,// "{ {asset('dist/lib/uploader/Moxie.swf') }}",// flash 上传组件地址  默认为 lib/uploader/Moxie.swf
        'silverlight_xap_url' : SILVERLIGHT_COURSEWARE_XAP_URL, // "{ {asset('dist/lib/uploader/Moxie.xap') }}",// silverlight_xap_url silverlight 上传组件地址  默认为 lib/uploader/Moxie.xap  请确保在文件上传页面能够通过此地址访问到此文件。
        'self_upload' : true,//  是否自己触发上传 TRUE/1自己触发上传方法 FALSE/0控制上传按钮
        'file_upload_method' : 'initPic()',// 单个上传成功后执行方法 格式 aaa();  或  空白-没有
        'file_upload_complete' : '',  // 所有上传成功后执行方法 格式 aaa();  或  空白-没有
        'file_resize' : {quuality: 40},
        ...
        //     // preserve_headers: false // 是否保留图片的元数据，默认为 true 保留，如果为 false 不保留。
        // },
        'pic_list_json': {'data_list': RESOURCE_COURSEWARE_LIST_LARGE }// piclistJson 数据列表json对象格式  {‘data_list’:[{'id':1,'resource_name':'aaa.jpg','resource_url':'picurl','created_at':'2018-07-05 23:00:06'}]}
        // 不使用了，会自动格式化 pic_list_json 来 匹配  'static_files':@ json($info['static_files'] ?? [])// 初始静态文件对象数组 ； self_upload = false 使用； 格式 [{remoteData:{id:197}, name: 'zui.js', size: 216159, url: 'http://openzui.com'},...]
    }
    // 'myUploaderGrid':{
    //     'files_type': 1,// 0 图片文件 1 其它文件
...
$(function(){
    ...
    $(document).on("change",'input[name=video_type]',function(){
        ...
        changeVideoTypeShow(video_type);
    });
});

function changeVideoTypeShow(video_type) {
    if(video_type == 1){// 上传
        $('#video_url_id').hide();
        $('#video_upload_id').show();
    }else{
        $('#video_url_id').show();
        $('#video_upload_id').hide();
    }
}
...
function ajax_form(){
    ...
    if(!judge_validate(4,'课件名称',video_name,true,'length',1,100)){
        return false;
    }
    ...
    if( (!judge_list_checked(imgObj,3)) && filesCount <=0 ) {//没有选中的
        layer_alert('请选择要上传的课程分类图片！',3,0);
        return false;
    }


    var video_type = $('input[name=video_type]:checked').val() || '';
    var judge_seled = judge_validate(1,'视频类型',video_type,true,'custom',/^[12]$/,"");
    if(judge_seled != ''){
        layer_alert("请选择视频类型",3,0);
        //err_alert('<font color="#000000">' + judge_seled + '</font>');
        return false;
    }

    // 判断是否上传视频
    var uploaderVideo = $('#videoUploaderLarge').data('zui.uploader');
    var filesVideo = uploaderVideo.getFiles();
    var filesCountVideo = filesVideo.length;
    var videoObj = $('#videoUploaderLarge').closest('.resourceBlock').find(".upload_img");

    if(video_type == 1){
        if( (!judge_list_checked(videoObj,3)) && filesCountVideo <=0 ) {//没有选中的
            layer_alert('请选择要上传的视频！',3,0);
            return false;
        }
    }else{
        var video_url = $('input[name=video_url]').val();
        if(!judge_validate(4,'视频网络地址',video_url,true,'length',1,800)){
            return false;
        }
    }
    ...
    // 判断是否上传附件资料文件
    var uploaderCourseware = $('#coursewareUploaderLarge').data('zui.uploader');
    var filesCourseware = uploaderCourseware.getFiles();
    var filesCountCourseware = filesCourseware.length;
    var CoursewareObj = $('#coursewareUploaderLarge').closest('.resourceBlock').find(".upload_img");

    // if( (!judge_list_checked(CoursewareObj,3)) && filesCountCourseware <=0 ) {//没有选中的
    //     layer_alert('请选择要上传的附件资料！',3,0);
    //     return false;
    // }
    ...
    // 上传图片
    if(filesCount > 0 || filesCountVideo > 0  || filesCountCourseware > 0 ){
        var layer_index = layer.load();
        uploader.start();
        uploaderVideo.start();
        uploaderCourseware.start();
        var intervalId = setInterval(function(){
            var status = uploader.getState();
            var statusVideo = uploaderVideo.getState();
            var statusCourseware = uploaderCourseware.getState();
            console.log('获取上传队列状态代码',uploader.getState());
            if(status == 1 && statusVideo == 1 && statusCourseware == 1){
                layer.close(layer_index);//手动关闭
                clearInterval(intervalId);
                if(commonaction.isUploadSuccess(uploader) && commonaction.isUploadSuccess(uploaderVideo) && commonaction.isUploadSuccess(uploaderCourseware)){// 都上传成功
                    ajax_save(id);
                }
            }
        },1000);
