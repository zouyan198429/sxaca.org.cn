...
// 清空已上传数据
// upload_id 上传控制id
function cls_upload_file(upload_id){
    console.log('upload_id=', upload_id);
    var layer_index = layer.load();
    var uploadObj = $('#'+ upload_id);
    var resourceBlockObj = uploadObj.closest('.resourceBlock');
    var uploadImgObj = resourceBlockObj.find(".upload_img");
    uploadImgObj.find('.resource').each(function(){
        let obj = $(this);
        // 删除资源记录
        var resource_id = obj.data('id');
        // var upload_id = obj.closest('.resourceBlock').find('.uploader').attr('id');
        delResource(resource_id, 2, obj , upload_id);
    });
    layer.close(layer_index);//手动关闭
}

// 清空已上传数据--需要确认，二能操作
function cls_sure_upload_file(upload_id){

    var index_query = layer.confirm('您确定清空吗？不可恢复!', {
        btn: ['确定','取消'] //按钮
    }, function(){
        // 删除资源记录
        layer.close(index_query);
        cls_upload_file(upload_id);
    }, function(){
    });
    return false;
}
