        ...
        'success' : function(ret){
            ...
            if(!ret.apistatus){//失败
                ...
                layer.msg('操作成功！', {
                    ...
                    var vodOrderId = ret.result;
                    consoleLogs(['-vodOrderId-', vodOrderId]);
                    // 通过父页面弹出支付页面
                    // payOrderStaff(vodOrderId);
                    otheraction.payByIds(null, vodOrderId);
                });
...
// 通过父页面弹出支付页面
// function payOrderStaff(vodOrderId){
//     consoleLogs(['-payOrderStaff-vodOrderId-', vodOrderId]);
//     // 执行父类的支付方法
//     window.parent.otheraction.payByIds(null, vodOrderId);
//     // 关闭当前页面
//     parent_reset_list();
// }

function payCompleteUrl() {
    go(LIST_URL);
}
