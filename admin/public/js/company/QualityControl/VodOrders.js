...
var otheraction = {
    ...
    payByIds: function(obj, ids) {
        ...
        //获得表单各name的值
        var weburl = PAY_URL + '?vod_order_id='+ ids;
        console.log(weburl);
        // go(SHOW_URL + id);
        // location.href='/pms/Supplier/show?supplier_id='+id;
        // var weburl = SHOW_URL + id;
...
(function() {
   ...
   // document.write("        if( item.issuper==0 ){");
    ...
    document.write("            <td><%=item.company_name%>(<%=item.company_grade_text%>)<hr/><%=item.order_no%><\/td>");
    document.write("            <td><%=item.contacts%>（<%=item.tel%>）<hr/><%=item.invoice_project_template_name%><\/td>");
    document.write("            <td><%=item.vod_name%><hr/><%=item.invoice_template_name%><\/td>");
    document.write("            <td>￥<%=item.price%><hr/>￥<%=item.price_total%><\/td>");
    document.write("            <td><%=item.company_status_text%><hr/><%=item.order_time%><\/td>");
    document.write("            <td><%=item.pay_status_text%><hr/><%=item.pay_time%><\/td>");
    document.write("            <td><%=item.effectively_day_text%><hr/><%=item.effectively_time%><\/td>");
    ...
    document.write("                <\/a>");
    document.write("                <%}%>");
    // document.write("                <%if( item.company_status == 1 && [2,4,8].indexOf(item.pay_status) != -1 ){%>");
    document.write("                <%if( item.company_status == 1){%>");
    document.write("                <a href=\"javascript:void(0);\" class=\"btn btn-mini btn-success\"  onclick=\"otheraction.show(<%=item.vod_id%>)\">");
    document.write("                    <i class=\"ace-icon  fa fa-eye  bigger-60\"> 查看<\/i>");
    document.write("                <\/a>");
    document.write("                <%}%>");
    document.write("                <%if( false){%>");
